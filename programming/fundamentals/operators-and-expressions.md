...
# Bug — operator precedence causes wrong result
total = price - price * discount_rate + price * tax_rate
print(total)  # 88.0

# Intended: apply discount first, then add tax
total = (price - price * discount_rate) * (1 + tax_rate)
print(total)  # 86.4
```

```javascript
let price = 100, discountRate = 0.2, taxRate = 0.08;
let total = price - price * discountRate + price * taxRate;
console.log(total);  // 88
let correct = (price - price * discountRate) * (1 + taxRate);
console.log(correct);  // 86.4
```

When combining different operator types, use parentheses to enforce the intended order.

### Case 2: Short-Circuit for Safe Access
...
# Unsafe
name = response["data"]["user"]["profile"]["name"]  # crashes if missing

# Safe with manual checks
...
name = safe_get(response, "data", "user", "profile", "name")
print(name)  # "Alice"

not_found = safe_get(response, "data", "user", "nonexistent", "field")
print(not_found)  # None
```

```javascript
const response = { data: { user: { profile: { name: "Alice" } } } };

// Optional chaining (cleaner than manual checks)
const name = response?.data?.user?.profile?.name;
console.log(name);  // "Alice"

const notFound = response?.data?.user?.nonexistent?.field;
console.log(notFound);  // undefined
```

### Case 3: Floating-Point Comparison

```python
computed = 0.1 + 0.2
print(computed)           # 0.30000000000000004
print(computed == 0.3)    # False

# Fix: tolerance
EPSILON = 1e-9
print(abs(computed - 0.3) < EPSILON)  # True
...
console.log(computed === 0.3);  // false

const EPSILON = 1e-9;
console.log(Math.abs(computed - 0.3) < EPSILON);  // true
```

This is a consequence of IEEE 754 binary floating-point — decimal fractions like `0.1` cannot be represented exactly in binary.
...
```python
result = (5 + 3) * 2 ** 3 // 4 - 1  # 15
```

Step-by-step: `2**3=8`, `5+3=8`, `8*8=64`, `64//4=16`, `16-1=15`.

### Precedence Table

| Level | Category | Operators | Example | Evaluation |
|-------|----------|-----------|---------|------------|
| 18 | Parentheses | `()` | `(2+3)*4` | `5*4=20` |
| 15 | Exponentiation | `**` | `2*3**2` | `2*9=18` |
| 14 | Multiplicative | `* / // %` | `2+3*4` | `2+12=14` |
| 13 | Additive | `+ -` | `10-2+3` | `(10-2)+3=11` |
| 10 | Equality | `== !=` | `2+2==5-1` | `4==4=True` |
| 6 | Logical AND | `and/&&` | `T or F and F` | `T or (F and F)=T` |
| 5 | Logical OR | `or/\|\|` | `F and T or T` | `(F and T) or T=T` |
| 2 | Assignment | `=` | `x=y=5` | `x=(y=5)` |
...
| `undefined` | N/A | Falsy |
| `NaN` | N/A | Falsy |
| `[]` (empty array) | Falsy | Truthy |
| `{}` (empty dict/object) | Falsy | Truthy |
| `False` | Falsy | Falsy |
