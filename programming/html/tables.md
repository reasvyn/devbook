...
**Case 1: The inaccessible comparison table.**

A developer creates a product comparison table but uses `<td>` for the header column:
...
A screen reader hears "Price $10 $15" without understanding which product corresponds to which price because there are no `<th>` elements and no `scope` attributes.

The fix: use `<th>` with `scope` for all headers:
...
**Case 2: The multi-page printed table.**

A table with 100+ rows prints across multiple pages. Without `<thead>`, the header row appears only on the first page, making subsequent pages impossible to understand.

The fix: wrap headers in `<thead>`:
...
When printed, `<thead>` content repeats at the top of every page.

**Case 3: The slow table on mobile.**

A table with 12 columns is completely unusable on a 375px phone screen. The text shrinks to 8px and each cell contains only 2-3 visible characters.

The fix: wrap the table in a scrollable container:
...
The table retains its full width and users can scroll horizontally to see all columns.

**Case 4: Inconsistent cell count from colspan misuse.**

A developer adds `colspan="2"` to a cell but forgets to reduce the cell count in that row, causing the table layout to break:
...
This row has 3 logical cells (A spans 2 columns, B is in column 3, C is in column 4). An adjacent row with 4 regular cells will misalign.

## Examples

**Example 1: Employee list with row headers.**
...
**Example 2: Complex header structure with colspan and rowspan.**

```html
<table>
    <caption>2026 Department Budget</caption>
    <thead>
        <tr>
