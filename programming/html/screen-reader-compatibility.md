...
### Fixing a Non-Accessible Navigation Menu

**Problem:** A CSS-only mega menu that appears on hover cannot be used by keyboard users.
...
Fix with keyboard and screen reader support:
```html
<nav aria-label="Main">
    <ul>
        <li class="dropdown">
            ...
            <ul id="products-menu" class="submenu" role="menu" hidden>
                <li role="none">
                    <a role="menuitem" href="/web">Web</a>
                </li>
                <li role="none">
                    <a role="menuitem" href="/mobile">Mobile</a>
                </li>
            </ul>
        </li>
    </ul>
...
Key changes:
- `<a>` replaced with `<button>` for the toggle (semantically correct)
- `aria-expanded` communicates open/close state
- `aria-controls` associates the toggle with the menu
- `role="menu"` and `role="menuitem"` for screen reader consistency
- Hidden attribute hides menu from accessibility tree when closed

### Making a Data Chart Accessible

**Problem:** A complex SVG chart with no text alternative is invisible to screen readers.

```html
<!-- Before: no screen reader access -->
<div class="chart">
    <svg>...</svg>
</div>
```
...
```html
<!-- After: accessible chart -->
<div class="chart" role="img" aria-label="Revenue by quarter 2026">
    ...
    <table class="visually-hidden">
        ...
        <tr><td>Q3</td><td>$110K</td></tr>
        <tr><td>Q4</td><td>$150K</td></tr>
    </table>
</div>
```
...
### Example 1: Accessible loading spinner
...
Screen reader: *"Loading search results..."* (announced once, not repeatedly)

### Example 2: Status update without ARIA live region

```html
<button onclick="updateCount()">Add to cart</button>
<span id="cart-count" role="status">0 items</span>
```

```javascript
function updateCount() {
    const count = document.getElementById('cart-count');
    count.textContent = `${++itemCount} items in cart`;
}
```

`role="status"` ensures the count change is announced without interrupting the user's current activity.

### Example 3: Keyboard-navigable autocomplete

```html
<label for="search">Search</label>
<input type="text" id="search" role="combobox" aria-expanded="false"
       aria-controls="results" aria-autocomplete="list">
<ul id="results" role="listbox" aria-label="Suggestions" hidden></ul>
```

When the input gains focus and the user types, the results list appears. `aria-expanded` toggles, and arrow keys navigate options using `aria-activedescendant`.
