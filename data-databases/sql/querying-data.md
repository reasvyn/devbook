...
### Case 1: Finding duplicates with GROUP BY and HAVING

**Problem:** Find employees sharing the same name and department.
...
The sample has no duplicates. Insert a test duplicate:

```sql
INSERT INTO employees VALUES (11, 'Alice', 'Engineering', 'Backend', 120000, '2024-01-15', 'a2@example.com');
SELECT name, department, COUNT(*) AS cnt FROM employees GROUP BY name, department HAVING COUNT(*) > 1;
DELETE FROM employees WHERE id = 11;
```

**Key insight:** `HAVING COUNT(*) > 1` is the standard duplicate detection pattern. Only GROUP BY columns define what makes a duplicate.

### Case 2: Cursor-based pagination

**Problem:** `LIMIT 20 OFFSET 10000` forces the database to scan 10000 discarded rows, getting slower for deep pages.

**Solution:** Cursor-based pagination uses WHERE to seek directly to the last seen row:

```sql
-- Page 1
SELECT id, name, salary FROM employees ORDER BY id LIMIT 3;
-- Page 2: last id = 3
SELECT id, name, salary FROM employees WHERE id > 3 ORDER BY id LIMIT 3;
-- Page 3: last id = 6
SELECT id, name, salary FROM employees WHERE id > 6 ORDER BY id LIMIT 3;
```

For multi-column sorts, use a tuple cursor:

```sql
-- Sort by salary DESC, id
SELECT id, name, salary FROM employees ORDER BY salary DESC, id LIMIT 3;
-- Next page using (salary, id) as cursor
SELECT id, name, salary
FROM employees
WHERE salary < 130000 OR (salary = 130000 AND id > 5)
ORDER BY salary DESC, id
LIMIT 3;
```

**Key insight:** Cursor pagination uses index seeks (O(1) per page) instead of scans + offset (O(N)). Trade-off: no random-access page numbers.

### Case 3: Counting related records

**Problem:** Count orders per customer.

```sql
CREATE TABLE customers (id INTEGER PRIMARY KEY, name TEXT NOT NULL);
CREATE TABLE orders (id INTEGER PRIMARY KEY, customer_id INTEGER NOT NULL REFERENCES customers(id), total REAL NOT NULL, ordered_at TEXT NOT NULL);

INSERT INTO customers VALUES (1, 'Alice'), (2, 'Bob'), (3, 'Charlie');
INSERT INTO orders VALUES
    (1, 1, 50.00, '2024-01-15'), (2, 1, 30.00, '2024-02-10'),
    (3, 2, 100.00, '2024-03-05'), (4, 1, 20.00, '2024-03-20'),
    (5, 3, 75.00, '2024-04-01');
```
...
With LEFT JOIN, customers with zero orders appear. Use INNER JOIN to exclude them:
...
```sql
SELECT c.name, COUNT(o.id) AS cnt, COALESCE(SUM(o.total), 0) AS total, ROUND(AVG(o.total), 2) AS avg_order
FROM customers c
LEFT JOIN orders o ON c.id = o.customer_id
GROUP BY c.id, c.name;
```

**Key insight:** JOIN type determines whether zero-count rows appear. GROUP BY must uniquely identify each customer — grouping by primary key (`c.id`) is sufficient.
