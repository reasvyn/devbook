...
**Case 1: The page that loads JavaScript in `<head>` without defer.**

A developer puts a large script in `<head>` without `defer`:
...
<body>
    <!-- Content visible only after large-bundle.js downloads -->
</body>
```

The page is blank until the 500KB bundle downloads and executes. The fix:

```html
<script src="large-bundle.js" defer></script>
```

**Case 2: The scripts that execute in the wrong order.**

A developer uses `async` for scripts with dependencies:
...
`app.js` depends on `library.js`, but `app.js` might download faster and execute first, causing errors.

The fix: use `defer` instead of `async`:
...
**Case 3: The module that cannot be imported.**

A developer tries to import a module using a bare specifier:

```javascript
import _ from 'lodash';
```

The browser throws an error: "Failed to resolve module specifier 'lodash'".

The fix: use an import map:
...
**Example 1: Modern script loading setup.**
...
<head>
    ...
    <title>Modern Script Loading</title>
    ...
    <!-- Deferred application scripts (preserve order) -->
    <script src="vendor.js" defer></script>
    <script src="app.js" defer></script>

    <!-- Async third-party scripts (independent) -->
    <script src="https://cdn.example.com/analytics.js" async crossorigin="anonymous"
            integrity="sha384-abc123..."></script>
</head>
<body>
    <!-- Page content -->

    <!-- Blocking polyfill for old browsers (rarely needed) -->
    <script>
        if (!('fetch' in window)) {
...
**Example 2: Dynamic import for code splitting.**
...
**Example 3: ES module with import map.**

```html
<script type="importmap">
{
    "imports": {
        "htm": "https://cdn.example.com/htm/htm.js",
        "preact": "https://cdn.example.com/preact/preact.js",
        "preact/hooks": "https://cdn.example.com/preact/hooks.js"
    }
}
</script>
...
**Example 4: Conditional polyfill loading.**

```html
<script>
    // Load IntersectionObserver polyfill only if needed
    if (!('IntersectionObserver' in window)) {
        const script = document.createElement('script');
        script.src = '/polyfills/intersection-observer.js';
