...
Alice and Bob agree on a prime p and generator g. They exchange public values computed from their private keys.

```python
def diffie_hellman(p: int, g: int, a_private: int, b_private: int) -> tuple[int, int]:
    """Simulate Diffie-Hellman key exchange."""
    A = pow(g, a_private, p)  # Alice public
    B = pow(g, b_private, p)  # Bob public
    
    shared_a = pow(B, a_private, p)  # Alice computes shared secret
    shared_b = pow(A, b_private, p)  # Bob computes shared secret
...
print(f"Alice public: {pow(g, alice_private, p)}")
print(f"Bob public: {pow(g, bob_private, p)}")
print(f"Shared secret (Alice): {shared_a}")
print(f"Shared secret (Bob): {shared_b}")
print(f"Match: {shared_a == shared_b}")
```

### Case 2: ElGamal Encryption

ElGamal is a public-key cryptosystem based on DLP.

```python
def elgamal_keygen(p: int, g: int) -> tuple[int, int, int]:
    """Generate ElGamal keys. Returns (public_key, private_key, p, g)."""
    import random
...
def elgamal_decrypt(ciphertext: tuple, priv: int, p: int) -> int:
...
# Example
p = 467  # prime
g = 2    # primitive root modulo 467

h, x, p, g = elgamal_keygen(p, g)
m = 123
c1, c2 = elgamal_encrypt(m, (h, p, g))
m2 = elgamal_decrypt((c1, c2), x, p)
print(f"Original: {m}, Decrypted: {m2}")
print(f"Match: {m == m2}")
```

## Examples

### Example 1: Finding All Primitive Roots Modulo a Prime
...
for p in [7, 13, 17]:
    ...
    print(f"Primitive roots mod {p}: {roots}")
    print(f"  Count: {len(roots)} = phi(phi({p})) = {phi(phi(p))}")
```

### Example 2: Checking Quadratic Residue Status

```python
def qr_status(a: int, p: int) -> str:
    """Return human-readable QR status."""
    ls = legendre_symbol(a, p)
    if ls == 0:
        return f"{a} is divisible by {p}"
    sqrt = [x for x in range(1, p) if (x * x) % p == a % p]
    return f"{a} is {'a QR' if ls == 1 else 'a QNR'} mod {p}, sqrt: {sqrt}"

for a in range(1, 12):
    print(qr_status(a, 11))
```

### Example 3: DLP with Different Bases

```python
def dlp_table(g: int, n: int) -> dict[int, int]:
    """Create a table mapping h -> log_g(h) for all h in the group."""
    table = {}
    cur = 1
    for x in range(n):
        if cur not in table:
...
print("DLP table base 2 mod 11:")
table = dlp_table(2, 11)
for h, log in sorted(table.items()):
    print(f"  log_2({h}) = {log}")
```

### Example 4: Using Legendre Symbol for Solvability

```python
def solve_quadratic_congruence(a: int, b: int, c: int, p: int) -> list[int]:
    """
    Solve ax^2 + bx + c = 0 (mod p) for odd prime p.
    Returns list of solutions.
    """
    if p == 2:
        return [x for x in range(2) if (a*x*x + b*x + c) % 2 == 0]
    
    # Multiply by 4a to complete the square
    ...
    # Find sqrt of D modulo p
    # For small p, use brute force
    for r in range(1, p):
        if (r * r) % p == D:
            inv_2a = pow(2 * a, -1, p)
            x1 = ((-b + r) * inv_2a) % p
...
print(solve_quadratic_congruence(1, -3, 2, 7))  # x^2 - 3x + 2 = 0 mod 7
```
