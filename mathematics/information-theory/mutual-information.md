...
### Case 1: Feature Selection for a Classification Task

Given 100 features and a binary target, use mutual information to select the top 10 features:
...
y = (X[:, 2] * X[:, 5] + 0.1 * np.random.randn(500) > 0).astype(int)
...
scores_selected = cross_val_score(clf, X_selected, y, cv=5)

print(f"Accuracy with all features: {scores_all.mean():.3f}")
print(f"Accuracy with top 10 MI features: {scores_selected.mean():.3f}")
```

### Case 2: Detecting Non-Linear Dependence

Pearson correlation misses non-linear relationships. Mutual information detects them:

```python
np.random.seed(42)
n = 1000
x = np.random.uniform(-2, 2, n)
y_circular = x**2 + 0.1 * np.random.randn(n)
y_linear = 0.8 * x + 0.2 * np.random.randn(n)

corr_circular = np.corrcoef(x, y_circular)[0, 1]
corr_linear = np.corrcoef(x, y_linear)[0, 1]
mi_circular = mutual_info_regression(x.reshape(-1, 1), y_circular, random_state=42)[0]
mi_linear = mutual_info_regression(x.reshape(-1, 1), y_linear, random_state=42)[0]

print(f"Circular: corr={corr_circular:.3f}, MI={mi_circular:.4f}")
print(f"Linear: corr={corr_linear:.3f}, MI={mi_linear:.4f}")
```

### Case 3: Fano's Bound on Classification Error

For a 10-class problem with conditional entropy $H(Y|X) = 2.1$ bits:

```python
import math

def fano_lower_bound(H_YgX, num_classes):
    return (H_YgX - 1) / math.log2(num_classes)

H_YgX = 2.1
k = 10
pe_bound = fano_lower_bound(H_YgX, k)
print(f"Fano lower bound: P_e >= {pe_bound:.3f}")
print(f"Even a perfect classifier cannot beat {pe_bound*100:.1f}% error")
```

## Examples

### Example 1: Computing MI by Hand
...
# Verify: H(X) + H(Y) - H(X,Y)
H_X = -np.sum(p_x * np.log2(p_x))
H_Y = -np.sum(p_y * np.log2(p_y))
H_XY = -np.sum(p_xy * np.log2(p_xy + 1e-12))
print(f"H(X) + H(Y) - H(X,Y) = {H_X + H_Y - H_XY:.4f} bits")
```

### Example 2: MI for Gaussian Variables
...
for rho in [0.0, 0.3, 0.6, 0.9, 1.0]:
    print(f"rho={rho:.1f}: MI={mi_gaussian(rho):.4f} bits")
```

### Example 3: Conditional MI and Independence

```python
# Create data where X and Y are marginally dependent but conditionally independent given Z
n = 1000
...
# Marginal MI
mi_marginal = mutual_info_regression(X.reshape(-1, 1), Y, random_state=42)[0]

# Conditional MI (compute for each Z value, then average)
mi_conditional = 0
for z in [0, 1]:
    mask = Z == z
    mi_z = mutual_info_regression(X[mask].reshape(-1, 1), Y[mask], random_state=42)[0]
    mi_conditional += np.mean(mask) * mi_z

print(f"I(X;Y)     = {mi_marginal:.4f}")
print(f"I(X;Y|Z)   = {mi_conditional:.4f} (near 0 if conditional independence holds)")
```

### Example 4: Data Processing Inequality in Action
...
W = Z ** 2  # non-linear post-processing

I_XY = mutual_info_regression(X.reshape(-1, 1), Y, random_state=42)[0]
I_XZ = mutual_info_regression(X.reshape(-1, 1), Z, random_state=42)[0]
I_XW = mutual_info_regression(X.reshape(-1, 1), W, random_state=42)[0]

print(f"I(X;Y) = {I_XY:.4f}")
print(f"I(X;Z) = {I_XZ:.4f} (<= I(X;Y))")
print(f"I(X;W) = {I_XW:.4f} (<= I(X;Z))")
```
