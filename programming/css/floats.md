...
This is the primary remaining use case for floats. The text flows naturally around the image.
...
A pull quote floats inside the article text. Content wraps around it.

### Magazine-Style Initial Cap with Float
...
Creates a drop cap effect without pseudo-elements.

## Examples

### Example 1: Simple text wrap
...
.media-body {
    overflow: hidden;           /* Also creates BFC — prevents wrapping under float */
}
```

### Example 2: Grid of floated items (historic)
...
### Example 3: Circle-shaped image wrap
