...
### Case 1: Evaluating a Benchmark Claim

A developer reads a blog post titled "Database X is 50x faster than PostgreSQL for geospatial queries."

ACCAB assessment:

- Authority: Author is a Database X employee. High bias.
- Accuracy: No benchmark code provided. Claims cannot be verified.
- Currency: Published 3 years ago. Database X and PostgreSQL have released multiple versions since.
- Bias: Author works for Database X. Blog is on the company website.
- Basis: Claims are not supported by citations. No comparison methodology described.

Further investigation: The developer finds the claim originates from a vendor benchmark that compares Database X (optimized for geospatial, tuned by vendor engineers) against PostgreSQL with default settings and no spatial indexes.

Verdict: Reject. The claim is marketing, not evidence.

### Case 2: Citation Chaining

A developer reads: "Studies show that most developers spend 60% of their time debugging [1]."

Reference [1] is a blog post that says: "A Microsoft study found that developers spend 60% of their time debugging [2]."

Reference [2] is a Microsoft Research paper from 2005: "Debugging as a Development Activity."

The developer reads the original paper:

- The study surveyed 50 developers at Microsoft
- The 60% figure was self-reported ("What percentage of your time do you spend debugging?")
- The paper notes: "These numbers are self-reported and may be imprecise"
- The paper also found that debugging time varies widely by role (15% for frontend, 70% for systems)

The citation chain reveals:

- The claim is based on a limited study (50 developers, one company, self-reported)
- The claim oversimplifies (ignores variance by role)
- The blog post cited the paper but the developer citing the blog post did not check the original

The developer decides: The claim is not reliable enough to make decisions about tooling or process.

### Case 3: Official Documentation vs. Blog Post

A developer is configuring Nginx for a high-traffic site. A popular blog post recommends settings that differ from the official Nginx documentation.

Blog post recommends: `worker_connections 4096; worker_processes auto;`

Official docs recommend: `worker_connections 768; worker_processes 1;`

ACCAB assessment:

- Blog post: Expert author, but published 4 years ago. Recommends settings for high traffic, but does not explain the trade-offs.
- Official docs: From the Nginx project. Current with the latest release. Conservative defaults prioritize stability.

The developer:

- Checks the official docs for the specific version they are deploying
- Reads the blog post's reasoning (high traffic, many concurrent connections)
- Tests both configurations under realistic load
- Finds the blog post's settings work but consume more memory
- Chooses the official defaults initially, tunes later based on monitoring

Verdict: Start with official docs, verify blog post advice against them, and test before deploying.

### Case 4: Detecting AI Hallucination

A developer uses an AI assistant to generate code for a relatively new API. The AI generates code that uses a function that does not exist in the real API.

Detection:

- The developer has not seen this function in the official documentation
- The function name follows a plausible pattern but no one else uses it online
- The AI's confidence is high, but there are no examples or references

Verification:

- Check the official API documentation — function is not listed
- Search the codebase — no usage of this function
- Search the web for the function name — no results
- Check the library's source code on GitHub — function does not exist

Result: AI hallucination. The developer confirms the correct API call from the official documentation and reports the hallucination.

## Examples

### Example 1: ACCAB Framework Applied

Source: "Why Go is Better Than Java for Microservices" (Medium post, no author bio, no date)

```text
Authority:   No author name or bio. Unknown credentials. — LOW
Accuracy:    Claims like "Go is 3x faster than Java" without benchmarks.
             No code examples or reproducible data. — LOW
Currency:    No date. What version of Go? What version of Java? — UNKNOWN
Bias:        Title says "Better Than" — adversarial framing.
             No discussion of Java's advantages (ecosystem, tooling). — HIGH
Basis:       No citations, no references, no links to data. — LOW

Overall:     Not a reliable source. Do not use for decision-making.
```
...
Primary source (RFC 9113):
```text
"HTTP/2 allows multiple streams to be active simultaneously on a single
TCP connection. Streams are identified by integer stream identifiers.
Concurrent stream processing means that the server processes multiple
requests in an unpredictable order."
```

Secondary source (tutorial):
```text
"HTTP/2 lets you send multiple requests at once over a single connection,
unlike HTTP/1.1 which required one connection per request."
```

Difference: The secondary source is simpler and explains the practical benefit. The primary source is more precise about stream identifiers and processing order. Both are useful but serve different purposes.

### Example 3: Reproducing Results

A paper claims their database achieves 1M writes per second on a single node.

To reproduce:

1. Check if the code is available (GitHub link in the paper)
2. Set up the same hardware (or closest available)
3. Run the same workload (if the workload generator is provided)
4. Compare results

If results differ significantly:

- Environment differences (different CPU, storage, network)
- Software version differences
- Configuration differences (tuning parameters not mentioned in the paper)
- The paper's results may be inaccurate

### Example 4: Holistic Source Evaluation

A developer evaluating whether to use a new JavaScript framework:

Sources consulted:

1. **Official documentation** — well-structured, complete API reference, getting started guide — HIGH
2. **GitHub repository** — 50K stars, active development, 500 contributors — HIGH
3. **Blog post by core team** — explains design decisions, trade-offs — MEDIUM-HIGH
4. **Hacker News discussion** — mixed opinions, some valid criticism — MEDIUM
5. **Stack Overflow questions** — few questions, limited community — LOW (early stage)
6. **Vendor benchmarks** — claim 5x faster than React without methodology — LOW

Verdict: The framework has strong documentation and community, but the low Stack Overflow presence and vendor benchmarks with no methodology suggest it is early-stage. The developer decides to use it for a small project first, not production.
