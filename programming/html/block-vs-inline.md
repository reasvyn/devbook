...
**Case 1: The navigation that would not go horizontal.**

A developer writes navigation links:
...
Each link appears on its own line because `<a>` elements are inline and wrap naturally, but the developer wants them side by side. The developer tries `width: 100px` on the links — it does not work because `<a>` is inline. The developer adds `display: inline-block`:
...
Now each link has a fixed width, sits horizontally, and respects padding. But there is a 4px gap between them due to whitespace in the HTML. The developer switches to flexbox:
...
The navigation works as expected with no gaps.

**Case 2: The invisible margin that pushed content.**

A developer has a page with two sections:
...
The developer sets `margin-bottom: 40px` on `.section-a` and `margin-top: 30px` on `.section-b`. The space between sections is 40px, not 70px — margin collapse has taken effect. The developer expects 70px and the layout looks too tight.

The developer removes one of the margins (margin collapse chooses the larger value) or uses only `margin-bottom` on `.section-a` with 40px. Alternatively, using flexbox or grid layout avoids margin collapse entirely.

**Case 3: The span that would not be square.**

A developer wants a small colored square: `<span class="indicator"></span>` with `width: 16px; height: 16px;`. Nothing appears — `width`/`height` do not apply to inline elements. Adding `display: inline-block` fixes it:
...
The square appears and flows inline with surrounding text.

**Case 4: The paragraph that swallowed the div.**

```html
<p>Click this button: <div class="btn">Submit</div></p>
```

The `<div>` closes the `<p>` implicitly. The browser renders two separate elements. Using an inline element like `<button>` instead keeps everything inside the paragraph.

## Examples

**Example 1: Comparing block, inline, and inline-block.**
...
<p>
    <span class="demo block">Block 1</span>
    <span class="demo block">Block 2</span>
</p>

<p>
    <span class="demo inline-block">Inline-Block 1</span>
    <span class="demo inline-block">Inline-Block 2</span>
    <span class="demo inline-block">Inline-Block 3</span>
</p>
```

The inline spans sit on the same line. The block spans stack vertically. The inline-block spans sit on the same line but have explicit dimensions.

**Example 2: Replaced vs non-replaced inline.**

```html
<p>
    This is text with an
    <img src="icon.png" alt="Icon" style="width: 20px; height: 20px;">
    image inside. The image respects width and height even though it is inline.
</p>

<p>
    This is text with a
    <span style="width: 20px; height: 20px;">span</span>
    inside. The span ignores width and height — the span dimensions collapse.
</p>
```

**Example 3: Using `display: block` on links for large click targets.**
...
Each link becomes a full-width block with generous padding for easy clicking.
