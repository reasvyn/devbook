...
### Case 1: Adapting Content for Two Audiences

```text
For end-users (consultative):
Deploying your service is straightforward. Click the Deploy
button in the dashboard, and we will handle the rest. If
something goes wrong, the dashboard shows you what happened.

For engineers (formal, precise):
Deployment creates a container instance from the specified image
tag. The orchestrator performs a rolling update with a 10% canary
before full promotion. If the health check fails, the deployment
is automatically rolled back.
```

### Case 2: Rewriting a Document with Inconsistent Tone

```text
Original:
The API gateway routes requests. You can configure rate limiting.
The developer shall enable logging. It is super important to set
the right timeout values. Good luck!

Revised (consistent consultative):
The API gateway routes requests. Configure rate limiting per
endpoint. Enable logging for debugging. Set timeout values that
match your service's expected response times. Without proper
timeouts, requests may queue up and degrade performance.
```

### Case 3: Handling Tone for Error Messages

```text
Too formal:  Authentication failed. Access denied.
Too casual:  Oops! That key does not look right. ;)
Appropriate: Authentication failed. The API key you entered is
             invalid. Generate a new key in the dashboard.
```

## Examples

### Example 1: Same Content, Three Registers

```text
Formal:  The configuration file shall be located in /etc/app.
         The application must be restarted after modifications.
Casual:  You will find the config file in /etc/app. Restart the
         app after changing it.
```

### Example 2: Second Person vs. Third Person

```text
Second person: You can customize the color scheme in Preferences.
Third person:  The user can customize the color scheme.
```

### Example 3: Tone in API Documentation

```text
Friendly (Stripe): Use the Customers API to manage customer data.
Formal (AWS):      The Customers API provides operations for
                   managing customer resources.
```

### Example 4: Urgent Tone for Security Advisories

```text
Normal: The database backup runs daily at 2:00 AM UTC.
Urgent: Update your application immediately. A critical
        vulnerability (CVE-2024-1234) affects all versions
        prior to 2.1.0.
```

### Example 5: Internal vs. External Documentation

```text
Old brand voice (corporate, formal):
Our platform facilitates the seamless integration of disparate
data sources through a unified API. Organizations can leverage
this capability to streamline their data pipelines and enhance
operational efficiency.

New brand voice (developer-focused, direct):
Connect any data source through a single API. Pipe data from
databases, SaaS tools, and files into your workflow without
