...
```css
h1 {
    /* Linear scale: 2rem at 320px → 4rem at 1440px */
...
The formula: `clamp(min, preferred, max)` where preferred = `base + slope * viewport`.
...
:root {
    ...
    --primary-200: color-mix(in oklch, var(--primary) 35%, white);
    ...
    --primary-800: color-mix(in oklch, var(--primary) 45%, black);
    --primary-900: color-mix(in oklch, var(--primary) 25%, black);
}
```

## Examples

### Example 1: Responsive grid with repeat, minmax, and auto-fill
...
Four lines of CSS creates a responsive grid that adds columns as space allows.

### Example 2: Frosted glass with backdrop-filter
...
### Example 3: Image optimization with image-set
