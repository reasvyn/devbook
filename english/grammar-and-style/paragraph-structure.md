...
### Case 1: Rewriting a Disorganized Paragraph

```text
Original — no clear structure, ideas jump around
The application uses Redis for caching. It can also be used as a
message broker. The installation process requires the Redis server
to be running. Redis supports different data structures like lists
and sets. You should configure the maxmemory setting.

Revised — MEAL plan structure
Redis serves as both a cache and a message broker in the
application stack. (Main idea)
It stores frequently accessed data in memory for fast retrieval,
and its pub/sub feature enables asynchronous communication between
services. (Evidence)
Using Redis for both purposes reduces operational overhead because
the team manages one data store instead of two. (Analysis)
The following sections cover installation and configuration. (Link)
```

### Case 2: Fixing Pronoun Ambiguity

```text
Original:
The function reads the configuration and validates the schema.
If it contains errors, it returns an error message.

Revised:
The function reads the configuration file and validates the
schema against the expected format. If the schema contains errors,
the function returns a message describing the validation failure.
```

### Case 3: Breaking Up a Wall of Text

```text
Original — single paragraph covering token lifecycle, refresh tokens,
and security trade-offs in 200 words.

Revised — three focused paragraphs:

The authentication system uses JWT tokens generated at login.
The token contains the user ID, expiration time, and a digital
signature. The server verifies the signature using a secret key.
If the token is valid, the server processes the request.

Refresh tokens are long-lived tokens stored securely on the client.
When the access token expires, the client requests a new one using
the refresh token. This process happens transparently.

This two-token system balances security and user experience.
Short-lived access tokens limit the damage if a token is
compromised. Long-lived refresh tokens reduce login frequency.
```

## Examples

### Example 1: Topic Sentence Positions

```text
First sentence (standard):
Recursion is useful for traversing tree-like data structures. Each
node is processed by calling the same function on its children.
The base case terminates recursion at a leaf node.
```

### Example 2: Paragraph Flow with Transitions

```text
The application fetches data from the primary database. However,
this database is optimized for writes, not reads. Therefore,
frequent read queries cause performance degradation. As a result,
a read replica was added to handle query traffic.
```

### Example 3: Comparison-Contrast Paragraphs

```text
Relational databases enforce a fixed schema. Every row must conform
to defined column types and constraints. This ensures data integrity
but requires migrations for schema changes.

In contrast, NoSQL databases use flexible schemas. Each document
can have different fields. This accelerates development but places
data integrity responsibility on the application layer.
```

### Example 4: General-to-Specific Order

```text
Distributed systems face the challenge of data consistency across
nodes. When multiple nodes accept writes, data may diverge.

The CAP theorem states that a distributed system can provide only
two of three guarantees: consistency, availability, partition
tolerance. Most systems choose availability and partition tolerance.

Eventual consistency accepts temporary divergence. All nodes
eventually converge to the same value if no new writes occur.
```

### Example 5: Before-and-After Paragraph Repair

```text
Before (no topic sentence, multiple ideas):
The server runs on port 3000. The database stores user profiles.
You can configure the port in the settings file. The database
connection string is also in the settings file. PostgreSQL is the
default database. Redis is used for caching session data.

After (clear structure, one idea per paragraph):
The application server runs on port 3000 by default. You can change
this port in the settings file using the SERVER_PORT option.

The application supports PostgreSQL as its primary database. The
database connection string is configured in the settings file
using the DATABASE_URL option.

Redis provides session caching. Configure the Redis connection in
the CACHE section of the settings file.
```
