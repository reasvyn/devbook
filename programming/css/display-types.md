...
Before `display: flow-root`, floats required a clearfix:
...
```css
.container {
    display: flow-root;         /* Contains floats, no extra markup */
}
```

### Removing Whitespace Gap
...
```css
.nav {
    display: flex;              /* Clean, modern */
    gap: 1.5rem;
    list-style: none;
    padding: 0;
}

/* Pre-flexbox approach */
.nav--old {
    overflow: hidden;           /* BFC */
}
.nav--old li {
...
### Example 2: Visually hidden utility
...
### Example 3: Equal-height columns with table display
