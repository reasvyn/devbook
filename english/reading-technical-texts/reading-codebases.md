...
### Case 1: Onboarding to a New Microservice

A developer joins a team operating a payment processing microservice. They have never seen the codebase.

Approach:
1. Read the README and service documentation (top-down)
2. Read the Dockerfile and CI configuration
3. Read the entry point (main.go) to understand server setup
4. Read the router configuration to understand the API surface
5. Pick one endpoint (POST /api/charges) and trace the execution path
6. Read the tests for that endpoint
7. Write notes on the data flow and dependencies

Outcome: After 2-3 reading sessions, the developer can describe the architecture, knows where to find each component, and can make their first contribution.

### Case 2: Debugging a Performance Regression

A production incident: search endpoint response time increased from 50ms to 2s after a recent deployment.

Approach:
1. Use git bisect to find the offending commit
2. Read the diff — the commit changes a database query from `SELECT *` to a subquery with a JOIN
3. Check database indexes — no index was added for the new JOIN column
4. The test did not catch this because the test database has 100 rows, while production has 10 million

Outcome: The developer adds the missing index. Response time returns to 50ms. A performance test with realistic data volume is added.

### Case 3: Learning Express.js by Reading Its Source

A developer wants to understand how Express.js middleware works by reading the source code.

Approach:
1. Read the README and getting started guide (top-down)
2. Find the entry point (lib/express.js)
3. Trace `app.use()` — how middleware is registered
4. Trace the request handling pipeline — how middleware and routes are executed in order
5. Read the Layer and Route classes — how the router matches requests

Key insights:
- Middleware is stored as an array of Layer objects
- Each Layer holds a path pattern and a handler function
- Error-handling middleware (4 arguments) is skipped unless an error propagates

Outcome: The developer understands Express deeply enough to debug middleware issues and write custom middleware with confidence.

## Examples

### Example 1: Tracing a Request Through an MVC App
...
Trace: Route `GET /products/:id` maps to `ProductsController.show` -> extracts `id` from params -> calls `Product.findById(id)` -> if not found, 404 -> if found, return JSON -> unexpected error, 500.

### Example 2: Reading a Complex Conditional

```javascript
function calculateShipping(items, destination, customerTier) {
  let totalWeight = 0;
  for (const item of items) totalWeight += item.weight;

  let baseRate;
  if (["US", "CA"].includes(destination.country)) {
    baseRate = 5.00;
  } else if (["UK", "DE", "FR"].includes(destination.country)) {
    baseRate = 15.00;
  } else {
    baseRate = 25.00;
  }

  let weightCharge = totalWeight > 10
    ? baseRate + (totalWeight - 10) * 0.50
    : baseRate;

  const tierDiscounts = { gold: 0.20, silver: 0.10, bronze: 0.05 };
  const discount = tierDiscounts[customerTier] || 0;

  return weightCharge * (1 - discount);
}
```

Understanding: shipping cost depends on destination zone (NA cheapest, EU medium, rest highest), weight (first 10 units included, extra $0.50/unit), and customer tier (gold 20% off, silver 10%, bronze 5%).

### Example 3: Mapping Dependencies from Imports

```javascript
import { db } from "../database/index.js";
import { emailService } from "./emailService.js";
import { paymentGateway } from "../integrations/stripe.js";
import { InventoryClient } from "../grpc/inventoryClient.js";
```

Dependency graph: orderService depends on database, email, payment processing, and inventory system (via gRPC). This is a critical service connected to many other systems.
