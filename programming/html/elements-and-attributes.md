...
**Case 1: The broken self-closing script.**

A developer migrating from XHTML to HTML5 writes:

```html
<script src="app.js" />
```

In XHTML served as `application/xhtml+xml`, this loads the script. In HTML5, the parser sees `<script src="app.js" />` as an opening tag — the `/` is just a stray character. The script does not load, and everything after it is treated as JavaScript source code until a `</script>` tag appears.

If the page has content after the script tag, that content disappears because the browser is still "inside" the script element.

The fix: always use explicit closing tags for non-void elements:

```html
<script src="app.js"></script>
```

**Case 2: The unquoted class attribute.**
...
The browser parses this as `<div class="container">` followed by a stray `fluid>` attribute with no value. The element only has the `container` class, not `fluid`. The `>` character ends the tag, so `fluid>` might be interpreted as a second element or ignored.

The fix: always quote attributes:

```html
<div class="container fluid">
```

**Case 3: Duplicate ID collision.**

A developer dynamically generates page content and inadvertently creates two elements with `id="price"`:
...
A JavaScript function calls `document.getElementById('price')` expecting the first element but the behavior is unreliable. CSS selectors with `#price` only match the first element, but the second one is styled differently (or not at all). The `aria-labelledby` attribute that references `#price` now points to two elements, confusing assistive technology.

The fix: ensure IDs are unique across the entire page. Use a linter or HTML validator to catch duplicates.

**Case 4: The disabled attribute that never enables.**

A developer toggles a button's disabled state:

```html
<button disabled="false">Submit</button>
```

The button remains disabled because `disabled="false"` still counts as the presence of the attribute. In JavaScript:

```javascript
button.disabled = false;  // This works correctly
```

But in the HTML source, any value for a boolean attribute — even "false" — means the attribute is present and active.

The fix: omit the attribute entirely when you want the element enabled, or use JavaScript to toggle the property.

## Examples

**Example 1: Element with multiple attribute types.**
...
<a
    ...
    id="download-btn"
    data-file-size="2.4"
    aria-label="Download user manual (PDF, 2.4 MB)"
    target="_blank"
    rel="noopener"
>
    Download Manual
...
This anchor demonstrates: standard attributes (`href`, `target`, `rel`), a boolean attribute (`download`), CSS classes (`class`), an identifier (`id`), a custom data attribute (`data-file-size`), and an accessibility attribute (`aria-label`).

**Example 2: Invalid nesting detection.**
...
The validator catches this: `<div>` is not allowed as a child of `<ul>`. The `<div>` will be moved outside the `<ul>` by the parser, breaking the intended layout.

**Example 3: Data attributes in a dynamic list.**

```html
<ul id="book-list">
    <li data-id="1" data-title="Clean Code" data-author="Robert Martin" data-year="2008">
        Clean Code
    </li>
    <li data-id="2" data-title="The Pragmatic Programmer" data-author="Andrew Hunt" data-year="1999">
        The Pragmatic Programmer
    </li>
    <li data-id="3" data-title="Design Patterns" data-author="Gang of Four" data-year="1994">
        Design Patterns
    </li>
</ul>
```

JavaScript can use these data attributes to power filtering, sorting, and interaction without additional API calls:
...
**Example 4: ARIA attributes enhancing semantic HTML.**

```html
<nav aria-label="Breadcrumb" aria-labelledby="breadcrumb-heading">
    <h2 id="breadcrumb-heading" class="sr-only">You are here:</h2>
    <ol>
        <li><a href="/">Home</a></li>
        <li><a href="/html">HTML</a></li>
        <li aria-current="page">Elements & Attributes</li>
    </ol>
</nav>
```

The `aria-label` provides a descriptive name for the nav. The `aria-current="page"` tells screen readers that the last item is the current page. The `sr-only` heading is visually hidden but available to assistive technology.
