...
```javascript
// vite.config.js — modern build pipeline
...
export default defineConfig({
    ...
    build: {
        cssCodeSplit: true,            // Code-split CSS per entry
        cssMinify: 'esbuild',          // Fast CSS minifier
    },
});
```
...
- [ ] Full CSS async-loaded via `media="print"` trick or `preload`
- [ ] Fonts use `font-display: swap`
- [ ] Images have explicit `width` and `height` (or `aspect-ratio`)
- [ ] Animations use `transform` and `opacity` only
- [ ] Off-screen sections use `content-visibility: auto`
- [ ] Selectors are flat and simple
- [ ] No `@import` in CSS (inline during build)
- [ ] PurgeCSS or similar removes unused CSS
- [ ] Bundle gzipped (or Brotli) — target < 100KB CSS

## Examples

### Example 1: PostCSS config
...
```json
{
    ...
    "rules": {
        ...
        "selector-class-pattern": "^[a-z][a-zA-Z0-9]*(--[a-z][a-zA-Z0-9]*)?(__[a-z][a-zA-Z0-9]*)?$",
        "declaration-block-no-redundant-longhand-properties": true
    }
}
```

### Example 3: HTML with critical CSS
