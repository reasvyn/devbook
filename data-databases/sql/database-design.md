...
### Case 1: Update Anomaly from Unnormalized Schema

A startup stores all employee data in a single table:
...
Multiple employees share the same department. When the department budget changes, every row for that department must be updated. If one row is missed, the budget is inconsistent:

```sql
-- 3 rows to update if 3 employees in Engineering
UPDATE employees SET department_budget = 500000 WHERE department_name = 'Engineering';
```

This is an **update anomaly** caused by transitive dependency (id -> department_name -> department_budget). The fix is 3NF normalization: extract `departments(id, name, budget)` and reference it from `employees(department_id)`.

### Case 2: Missing Index on a Large Table

A social media app stores 50 million posts. The moderation dashboard runs:

```sql
SELECT id, content, author_id, created_at
FROM posts
WHERE flagged = true AND created_at >= '2025-01-01'
ORDER BY created_at DESC
LIMIT 100;
```

Without an index, this triggers a sequential scan and sort of 50 million rows. Execution time: 12 seconds.

Adding a composite index:

```sql
CREATE INDEX idx_posts_flagged_created ON posts(flagged, created_at DESC);
```

The query now scans only the rows matching `flagged = true` (roughly 100k rows), reads them in pre-sorted order (no sort needed), and stops after 100. Execution time: 3 milliseconds.

The lesson: always profile queries on production-scale data before deploying. A missing index on a small development database (1k rows) is invisible; on 50 million rows it is catastrophic.

### Case 3: Migration Gone Wrong

A team adds a `nickname` column to the `users` table and runs this in production:

```sql
ALTER TABLE users ADD COLUMN nickname VARCHAR(50) NOT NULL;
```

The table has 2 million existing rows. The ALTER TABLE fails with:

```
ERROR: column "nickname" contains null values
```

A `NOT NULL` column without a default cannot be added to a non-empty table. The safe sequence:

```sql
-- Step 1: Add as nullable
ALTER TABLE users ADD COLUMN nickname VARCHAR(50);

-- Step 2: Backfill with a default value
...
For very large tables, the UPDATE should be batched to avoid long transactions. Alternatively, use a DEFAULT in the ALTER (metadata-only in PostgreSQL) and backfill later.

## Examples

### Example 1: Library System ER Model

A library tracks books, authors, members, and loans. Books and authors have a many-to-many relationship resolved by a junction table. Members borrow books via loans.
...
The partial index indexes only unreturned loans -- smaller and faster than indexing every loan.

### Example 2: Before/After Normalization

**Before (1NF violation -- comma-delimited list in a column):**

| order_id | customer | products | total |
|----------|----------|----------|-------|
| 1 | Alice | "Mouse,Keyboard,Monitor" | 469.97 |
| 2 | Bob | "Mouse" | 29.99 |

Querying "how many keyboards sold" requires string parsing. Adding a product requires modifying the list.
...
|----|----------|-------|------------|
| 1 | Alice | 469.97 | 2025-03-01 |
| 2 | Bob | 29.99 | 2025-03-02 |
...
Now querying is straightforward: `SELECT COUNT(*) FROM order_items WHERE product_name = 'Keyboard'`. No string parsing, no update anomalies.

### Example 3: EXPLAIN Output Walkthrough

Consider a `users` table with 1 million rows. Without an index on `last_name`:
...
```text
 Seq Scan on users  (cost=0.00..18342.00 rows=1 width=136)
                    (actual time=0.015..153.234 rows=823 loops=1)
   Filter: ((last_name)::text = 'Smith'::text)
   Rows Removed by Filter: 999177
 Execution Time: 153.312 ms
```

153 ms to scan a million rows. After adding `CREATE INDEX idx_users_last_name ON users(last_name)`:

```text
 Index Scan using idx_users_last_name on users  (cost=0.42..24.70 rows=823 width=136)
                                                  (actual time=0.023..0.312 rows=823 loops=1)
 Execution Time: 0.345 ms
```

0.34 ms vs 153 ms -- a 450x improvement.

With a query requiring sort (`ORDER BY first_name`):

```text
 Sort  (cost=25.05..26.11 rows=823 width=136) (actual time=0.512..0.542 rows=823 loops=1)
   Sort Key: first_name
   ->  Index Scan using idx_users_last_name on users
         (cost=0.42..24.70 rows=823 width=136)
         (actual time=0.023..0.330 rows=823 loops=1)
 Execution Time: 0.578 ms
```

The database uses the index for WHERE but adds a separate sort. A composite index `(last_name, first_name)` would eliminate the sort step.
...
- Explore the [Backend Developer Roadmap](../../roadmaps/senior/backend-developer.md) for database design in production systems
