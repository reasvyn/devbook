...
```mermaid
graph LR
    A[No Exercise] -->|BDNF baseline| B[Low BDNF]
    B -->|20 min walk| C[Acute BDNF Spike]
    C -->|30 min moderate| D[Significant Increase]
    D -->|150 min/week| E[Sustained Elevation]
    E -->|Beyond threshold| F[Diminishing Returns]
...
```mermaid
graph LR
    A[0 min/week] --> B[150 min/week]
    B --> C[300 min/week]
    C --> D[600+ min/week]
    
    A ---|"No exercise: cognitive baseline"| E
    B ---|"WHO minimum: 80% of max benefit"| F
    C ---|"Double: ~90% of max benefit"| G
    D ---|"High volume: ~95% of max benefit"| H
...
```mermaid
graph TD
    A[Focused Work: 60-90 min] --> B[Cortisol Rising]
    B --> C[Attention Declining]
    C --> D[Movement Break: 5-15 min]
    D --> E[Cerebral Blood Flow Restored]
    E --> F[Cortisol Reducing]
    F --> G[Prefrontal Cortex Recovering]
    G --> A
...
Starting from zero requires a specific approach. The approach is not about exercise science. It is about behavior change — the same process described in [The Mechanism of Change](../foundations/the-mechanism-of-change.md): awareness, agency, and action.
