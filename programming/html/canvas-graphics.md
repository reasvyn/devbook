...
**Case 1: The game that drains the battery.**

A developer uses `setInterval(fn, 16)` for a game loop. The game runs at 60fps even when the browser tab is in the background, draining the CPU and battery.

The fix: use `requestAnimationFrame` which automatically pauses when the tab is hidden.

**Case 2: The chart that looks blurry on Retina displays.**

A developer creates a 400x300 canvas that looks crisp on a standard display but blurry on a Retina (2x) screen. The canvas uses device-independent pixels, not physical device pixels.
...
**Case 3: The color picker that cannot export.**

A developer loads a user-uploaded image from a file input and draws it on canvas. When exporting, `toDataURL()` throws a security error.

The fix: load the image via `URL.createObjectURL()` with local file access, or ensure the image server sends CORS headers and set `img.crossOrigin = 'anonymous'`.
...
function drawBarChart(ctx, data, labels, width, height) {
    ...
    data.forEach((value, i) => {
        ...
        const x = margin.left + (chartWidth / data.length) * i + (chartWidth / data.length) * 0.15;
    ...
    labels.forEach((label, i) => {
        const x = margin.left + (chartWidth / data.length) * i + (chartWidth / data.length) * 0.5;
    ...
    ctx.textAlign = 'center';
    ctx.fillText('Quarterly Sales', width / 2, 16);
}
```

**Example 2: Animated bouncing balls.**
...
canvas3.addEventListener('mouseleave', () => { isDrawing = false; });
...
canvas3.addEventListener('touchmove', (e) => {
    ...
    ctx3.beginPath();
    ctx3.moveTo(lastX, lastY);
    ctx3.lineTo(touch.clientX - rect.left, touch.clientY - rect.top);
    ctx3.stroke();
    lastX = touch.clientX - rect.left;
    lastY = touch.clientY - rect.top;
});

canvas3.addEventListener('touchend', () => { isDrawing = false; });
```
