...
```mermaid
graph LR
    A[Paul Jobs] -->|"Craftsmanship"| B[Steve Jobs]
    C[Silicon Valley Ecosystem] -->|"Proximity to innovation"| B
    D[Homestead High School] -->|"Meets Wozniak"| E[Partnership]
    E --> F[Wozniak: Technical genius]
    E --> G[Jobs: Product vision]
    F --> H[Apple Computer founded, 1976]
    G --> H
```
...
```mermaid
graph TD
    A[Apple I, 1976] -->|"Hobbyist kit"| B[Apple II, 1977]
    B -->|"Mass-market design"| C[VisiCalc, 1979]
    C -->|"Business utility"| D[Apple II becomes dominant platform]
    D --> E[Revenue funds Apple's growth]
    E --> F[Macintosh development begins]
```
...
```mermaid
graph TD
    A[Jobs returns to Apple, 1997] --> B[Product line simplification]
    B --> C[iMac, 1998]
    B --> D[iPod, 2001]
    B --> E[iTunes Store, 2003]
    B --> F[iPhone, 2007]
    B --> G[iPad, 2010]
    C -->|"Design as differentiator"| H[Apple's brand identity]
    D -->|"Integration of hardware + software"| H
    E -->|"Digital content distribution"| H
    F -->|"Mobile computing revolution"| H
    G -->|"Post-PC era"| H
    H --> I[Apple becomes most valuable company, 2010]
```
...
```mermaid
graph TD
    A[Jobs's Perfectionism] --> B[Outstanding Products]
    A --> C[Interpersonal Cruelty]
    B --> D[Apple's market dominance]
    C --> E[Employee burnout and resentment]
    D --> F[Celebrated legacy]
    E --> G[Human cost — often unacknowledged]
    F --> H[Tension: can brilliance excuse harm?]
    G --> H
```
...
timeline
    ...
    1967 : Family moves to Los Altos
    1972 : Enrolls at Reed College, drops out after one semester
    1974 : Works at Atari, attends Homebrew Computer Club
    1976 : Co-founds Apple Computer
    1977 : Apple II ships
    1984 : Macintosh launches
    1985 : Ousted from Apple, founds NeXT
    1986 : Purchases Pixar from Lucasfilm
    1995 : Toy Story released
    1996 : Apple acquires NeXT; Jobs returns to Apple
    1997 : Becomes interim CEO; launches "Think Different"
    1998 : iMac launches
    2001 : iPod and iTunes Store
    2003 : Pancreatic cancer diagnosed
    2007 : iPhone launches
    2010 : iPad launches
    2011 : Resigns as CEO; dies 5 October, age 56
```
...
```mermaid
graph LR
    A["Technology"] -->|"Intersection"| D[Products that delight]
    B["Liberal Arts"] -->|"Intersection"| D
    C["Design"] -->|"Intersection"| D
    D --> E[iMac]
    D --> F[iPod]
    D --> G[iPhone]
    D --> H[iPad]
    E --> I[Beauty as function]
    F --> J[Simplicity as power]
    G --> K[Touch as interface]
    H --> L[Post-PC computing]
```
...
```mermaid
graph TD
    A[Jobs's Legacy] --> B[Apple's Dominance]
    A --> C[Design Philosophy]
    A --> D[Consumer Technology Revolution]
    A --> E[Leadership Principles]
    B --> F[Most valuable company, 2010]
    B --> G[Global ecosystem of products]
    C --> H["Intersection of technology and liberal arts"]
    C --> I[Simplicity as design principle]
    D --> J[Smartphone revolution]
    D --> K[Tablet computing]
    D --> L[Digital content distribution]
    E --> M[Focus — saying no]
    E --> N[Taste as discernment]
    E --> O[Anticipating unarticulated needs]
    E --> P[Awareness of mortality]
```
