...
**Case 1: The page that loads fonts too late.**

A developer imports a Google Font via `@import` in CSS. The font is not discovered until the CSS is downloaded and parsed, causing a flash of invisible text (FOIT) or flash of unstyled text (FOUT).

The fix: preload the font and preconnect to Google Fonts:
...
**Case 2: The blog with no canonical URL.**

A developer's blog platform generates URLs like `/post?id=123` and `/post/123/hello-world`. Both show the same content. Search engines index both, splitting the page authority.

The fix:

```html
<link rel="canonical" href="https://blog.example/post/123/hello-world">
```

**Case 3: The PWA that cannot be installed.**

A developer builds a Progressive Web App but forgets to link the manifest file. The browser does not prompt the user to install the app.
...
**Case 4: The image that flashes on every page load.**

A developer does not preload the hero image. The browser discovers it late in the HTML parsing process, causing a visible delay before the image appears.
...
**Example 1: Complete `<head>` with all resource hints.**
...
<head>
    ...
    <link rel="preload" href="critical.css" as="style">
    <link rel="preload" href="hero-product.webp" as="image" media="(min-width: 768px)">
    ...
    <!-- SEO -->
    <link rel="canonical" href="https://store.example/products/product-name">
    <link rel="alternate" href="https://store.example/products/product-name" hreflang="en">
    <link rel="alternate" href="https://store.example/fr/products/product-name" hreflang="fr">
...
**Example 3: Preloading with responsive images.**

```html
<head>
    <!-- Preload different hero images based on device -->
    <link rel="preload" href="hero-desktop.webp" as="image" media="(min-width: 1024px)">
    <link rel="preload" href="hero-tablet.webp" as="image" media="(min-width: 600px) and (max-width: 1023px)">
    <link rel="preload" href="hero-mobile.webp" as="image" media="(max-width: 599px)">
</head>
```

**Example 4: Preloading a module script.**
