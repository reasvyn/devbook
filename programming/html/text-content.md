...
**Case 1: The skipped heading.**

A developer writes a page with this heading structure:
...
A screen reader user navigates by heading level. The user jumps from `<h1>` directly to `<h3>`, which is fine. But there is no `<h2>` — the outline is incomplete. When the user tries to get a page overview using heading navigation, the structure is confusing because there are no intermediate levels.

The fix: maintain a proper hierarchy without skipping levels.
...
**Case 2: The decorative list.**

A developer uses `<ul>` for social media icons that are just unrelated links. Screen readers announce them as a list, which is misleading. The fix: use `<nav>` with plain links instead of a list. Only use `<ul>` when items are meaningfully grouped.

**Case 3: The misused `<blockquote>` for indentation.**

Using `<blockquote>` purely for visual indentation misleads screen readers into thinking the content is a quotation. Use CSS margins (`margin-left`) for indentation instead.

**Case 4: Overusing `<br>` for spacing.**

Multiple `<br>` tags between sections is structurally incorrect. Use CSS margins instead of line breaks for visual spacing.

## Examples

**Example 1: Article with proper text structure.**

```html
<article>
    <h1>Getting Started with HTML</h1>

    <p>HTML is the foundation of every web page. This article covers the essentials.</p>

    <h2>What You Need</h2>
    <ul>
        <li>A text editor (VS Code, Sublime Text, or Notepad)</li>
        <li>A web browser (Chrome, Firefox, or Edge)</li>
        <li>Basic computer literacy</li>
    </ul>

    <h2>Your First Page</h2>
    <p>Create a file called <code>index.html</code> and open it in your browser.</p>
...
&lt;/html&gt;</code></pre>
    ...
    <dl>
        ...
        <dt><code>&lt;a&gt;</code></dt>
        <dd>Hyperlink to another page</dd>
        <dt><code>&lt;img&gt;</code></dt>
...
**Example 3: Mixed text semantics.**

```html
<p>
    The <abbr title="World Wide Web Consortium">W3C</abbr> was founded by
    <cite>Tim Berners-Lee</cite> in 1994. Its mission is to lead the web to
    its full potential. The term <dfn>web standards</dfn> refers to the
    specifications published by the W3C and other standards bodies.
</p>

<p>
    According to the report, <q>over <mark>98% of websites</mark> use HTML</q>
    as their primary markup language. The remaining <mark>2%</mark> uses
    proprietary formats.
</p>

<p>
    In the equation <var>E</var> = <var>mc</var><sup>2</sup>, the variable
    <var>c</var> represents the speed of light in a vacuum
    (approximately 3 &times; 10<sup>8</sup> m/s).
</p>
```
