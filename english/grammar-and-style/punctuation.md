...
### Case 1: Fixing Punctuation in an API Reference

```text
// Before — inconsistent
Gets user data. Parameters id (required) type: string
...
- type (optional) — The response format: "json" or "xml".

Returns: A User object, or null if the user does not exist.
```

### Case 2: Clarifying a Conditional Instruction

```text
// Before — ambiguous
If the build fails check the logs fix the errors and rebuild.

// After — clear punctuation
If the build fails, check the logs, fix the errors, and rebuild.
```

### Case 3: Formatting a Complex List

```text
// Before — hard to parse
Local development uses SQLite, staging uses PostgreSQL, production uses Aurora.

// After — semicolons for clarity
Local development uses SQLite; staging uses PostgreSQL; production uses Aurora.
```

## Examples

### Example 1: Oxford Comma Prevents Ambiguity

```text
// Without — ambiguous: is the editor a single person?
I thank my parents, the editor and the reviewer.

// With — clear: four separate entities
I thank my parents, the editor, and the reviewer.
```

### Example 2: Colon for Introductions

```text
The deployment script does three things: builds the project, runs migrations,
and restarts the server.
```

### Example 3: Em Dash for Emphasis

```text
There is one feature every developer requests — dark mode.
```

### Example 4: Hyphenated Compound

```text
The real-time dashboard updates every five seconds.
```

### Example 5: Apostrophe with "its"

```text
The application stores its configuration in the database.
It's configured by setting environment variables.
```
