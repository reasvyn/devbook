...
### Case 1: Truncation Error Analysis

When approximating functions with finite series, we must understand the tradeoff between accuracy and computation.

```python
import numpy as np

def truncation_error_analysis():
    """Analyze truncation error for sin(x) approximation."""
    x_vals = [0.1, 0.5, 1.0, 2.0, 3.0]

    print(f"{'x':>4} {'terms':>5} {'approx':>15} {'error':>12} {'bound':>12}")
    print("-" * 52)
    ...
    for x in x_vals:
        ...
        for n_terms in [2, 4, 6, 8]:
            ...
            bound = abs(x)**(2*n_terms + 1) / np.math.factorial(2*n_terms + 1)
            print(f"{x:4.1f} {n_terms:5d} {approx:15.8f} {error:12.2e} {bound:12.2e}")

truncation_error_analysis()
```

### Case 2: Convergence of Gradient Descent as a Geometric Series

The error of gradient descent on a strongly convex quadratic function decays as a geometric series.

```python
import numpy as np

def gradient_descent_geometric(A, b, x0, lr, n_steps=50):
    """Gradient descent for quadratic f(x) = 0.5*x^T A x - b^T x."""
    x = x0.copy()
    errors = []
    ...
    for k in range(n_steps):
        ...
        errors.append(error)
        if k < 5 or k >= n_steps - 3:
            print(f"Step {k}: error = {error:.6e}")
...
lr = 2 / (max(eigvals) + min(eigvals))  # optimal step size
errors = gradient_descent_geometric(A, b, x0, lr, n_steps=20)

# Ratio of consecutive errors should approach (kappa-1)/(kappa+1)
ratios = [errors[k+1]/errors[k] for k in range(len(errors)-1)]
predicted = (kappa - 1) / (kappa + 1)
print(f"Convergence ratio: {np.mean(ratios[-5:]):.4f} (predicted {predicted:.4f})")
```

### Case 3: Approximating Pi Using Series

Machin's formula uses the arctangent series to compute pi.
...
def arctan_series(x, n_terms=20):
    """Compute arctan(x) using Maclaurin series."""
    result = 0
    for n in range(n_terms):
...
def compute_pi_machin(n_terms=20):
    return 4 * (4 * arctan_series(1/5, n_terms) - arctan_series(1/239, n_terms))

for n in [1, 2, 5, 10, 15, 20]:
    pi_approx = compute_pi_machin(n)
    error = abs(np.pi - pi_approx)
    print(f"n_terms={n:2d}: pi ≈ {pi_approx:.12f}, error = {error:.2e}")
```

### Case 4: Fourier Series for Audio Compression

```python
import numpy as np

def analyze_audio_signal(frequencies, amplitudes, duration=1.0, sample_rate=44100):
    """Create and analyze a composite signal."""
    ...
    # FFT to recover frequencies
    fft = np.fft.fft(signal)
    freqs = np.fft.fftfreq(len(t), 1/sample_rate)

    # Find peaks
    magnitudes = np.abs(fft) / len(t)
    positive_idx = freqs > 0
    peak_idx = np.argsort(magnitudes[positive_idx])[-5:]

    print("Detected frequencies:")
    for idx in peak_idx:
        if magnitudes[positive_idx][idx] > 0.01:
            print(f"  {freqs[positive_idx][idx]:.1f} Hz (amplitude: {magnitudes[positive_idx][idx]:.3f})")

analyze_audio_signal([440, 880, 1320], [1.0, 0.5, 0.25])
```

## Examples

### Example 1: Ratio Test in Code

```python
import numpy as np

def ratio_test(f, n_max=1000):
    """Apply ratio test numerically to series ∑ f(n)."""
    n = np.arange(1, n_max)
    a_n = f(n)
    a_n1 = f(n + 1)
    ratios = np.abs(a_n1 / a_n)
    L = np.mean(ratios[-100:])
    if L < 1:
        return f"Converges (L = {L:.4f})"
    elif L > 1:
        return f"Diverges (L = {L:.4f})"
    else:
        return f"Inconclusive (L = {L:.4f})"

print("∑ 1/n!:", ratio_test(lambda n: 1/np.array([np.math.factorial(int(k)) for k in n])))
print("∑ n!/n^n:", ratio_test(lambda n: np.array([np.math.factorial(int(k))/k**k for k in n])))
print("∑ 1/n:", ratio_test(lambda n: 1/n))
```

### Example 2: Power Series for exp(x) with Error Bound

```python
import numpy as np

def exp_series(x, n_terms):
    """Compute exp(x) using power series."""
    result = 0.0
    term = 1.0
    for n in range(n_terms):
        result += term
...
def exp_error_bound(x, n_terms):
    """Lagrange remainder bound for exp(x)."""
    xi = max(x, 0)  # max of |f^(n+1)(xi)| is exp(|x|)
    return np.exp(abs(xi)) * abs(x)**n_terms / np.math.factorial(n_terms)

x = 1.0
for n in [1, 3, 5, 10, 15]:
    approx = exp_series(x, n)
    error = abs(np.exp(x) - approx)
    bound = exp_error_bound(x, n)
    print(f"n={n:2d}: approx={approx:.10f}, err={error:.2e}, bound={bound:.2e}")
```

### Example 3: Visualizing Taylor vs Fourier Approximation

```python
import numpy as np

# Taylor approximates locally (near expansion point)
# Fourier approximates globally (over the period)

def compare_approximations():
    """Compare Taylor and Fourier approximations of a smooth function."""
    f = lambda x: np.sin(x) * np.exp(-x**2/4)

    # Taylor around x=0
    # f(0)=0, f'(0)=1, f''(0)=0, f'''(0)=-3/2, f^{(4)}(0)=0
    def taylor_approx(x):
        return x - 1.5 * x**3 / 6  # up to 3rd order

    # Fourier on [-pi, pi]
    from scipy import integrate
    T = 2 * np.pi
    a0 = (1/T) * integrate.quad(f, -np.pi, np.pi)[0]
    an = []
    bn = []
    for n in range(1, 6):
        an.append((2/T) * integrate.quad(lambda x: f(x)*np.cos(2*np.pi*n*x/T), -np.pi, np.pi)[0])
        bn.append((2/T) * integrate.quad(lambda x: f(x)*np.sin(2*np.pi*n*x/T), -np.pi, np.pi)[0])
    ...
    x_test = 0.3
    print(f"f({x_test}) = {f(x_test):.6f}")
    print(f"Taylor:  {taylor_approx(x_test):.6f}")
    print(f"Fourier: {fourier_approx(x_test):.6f}")

compare_approximations()
```

### Example 4: Finding the Sum of a Series Numerically

```python
import numpy as np

def series_sum(f, tol=1e-12, max_terms=1e8):
    """Sum a convergent series until terms are below tolerance."""
    total = 0.0
    n = 1
    while n <= max_terms:
        term = f(n)
        ...
        if abs(term) < tol:
            return total, n
        n += 1
    return total, int(max_terms)

# ζ(2) = ∑ 1/n^2 = π^2/6
zeta2, terms = series_sum(lambda n: 1/n**2)
print(f"zeta(2) ≈ {zeta2:.10f} (expected {np.pi**2/6:.10f}), {terms} terms")

# Alternating harmonic: ∑ (-1)^{n+1}/n = ln(2)
alt_harm, terms = series_sum(lambda n: (-1)**(n+1)/n)
print(f"ln(2) ≈ {alt_harm:.10f} (expected {np.log(2):.10f}), {terms} terms")
```
