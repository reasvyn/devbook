...
### Case 1: Full Workflow for a Research Paper

A developer needs to understand the Raft consensus protocol to evaluate a production etcd incident.

**Monday — Preview (5 min):**

- Title: "In Search of an Understandable Consensus Algorithm"
- Abstract: Proposes Raft as an understandable alternative to Paxos
- Sections: Introduction, Protocol, Evaluation, Related Work, Conclusions
- Figures: Leader election state diagram, log replication timeline
- Triage: A — directly relevant to production issue

**Tuesday — Read (60 min):**

- Three-pass Pass 2: full read with annotations
- 15 marginalia items, 3 code snippet extracts (leader election, log replication, safety)
- Key insight: leader election timeout randomization strategy
- 3 action annotations for etcd source investigation

**Tuesday evening — Process (30 min):**

- Write permanent note on Raft consensus
- Create atomic notes for: leader election, log replication, safety properties
- Link to notes on CAP theorem and etcd
- Action item: trace etcd source for timeout implementation

**Wednesday — Archive (10 min):**

- Save PDF to Zotero with full metadata
- Create archive entry: tags #paper #distributed-systems #consensus #raft
- Link Zotero entry to Obsidian permanent note

**Thursday — Creation (45 min):**

- Read etcd source code for election timeout implementation
- Write findings: incident caused by misconfigured election timeout
- Create ADR with timeout configuration recommendation
- Share with team

Outcome: 5 hours invested, 5 outputs (permanent note, 3 atomic notes, ADR). Incident resolved, team has documented knowledge.

### Case 2: Handling a Blog Post Reading Spree

A developer opens 30 browser tabs after reading a popular blog.

**Step 1 — Preview all 30 tabs (30 min):**

- 15 tabs: Discard (outdated, low authority, not relevant)
- 10 tabs: Triage C — save with one-line summary
- 4 tabs: Triage B — relevant to current project
- 1 tab: Triage A — directly applicable to today's work

**Step 2 — Read triage A (20 min):** Full read with annotations, extract code snippet.

**Step 3 — Process triage A (15 min):** Write atomic note, create action item.

**Step 4 — Schedule triage B (10 min):** Add to queue, close all other tabs.

Outcome: 75 minutes to process 30 tabs. One actionable insight extracted, 10 deferred, 15 discarded.

### Case 3: Converting from Passive to Active Reading

A developer reads extensively but cannot recall details — 10-15 articles per week, no notes, 80% forgotten within a week.

After adopting the four-phase workflow:

- Reduced reading to 3-5 high-quality sources per week
- Discards 50% at preview, processes within 24 hours
- Archives with metadata — retrieves anything from the past 6 months

Results after 3 months: 150+ interconnected notes, one blog post, two production improvements. Reading time unchanged, retention up 5x.

## Examples

### Example 1: Annotated Paper on Distributed Caching

Paper proposes a tiered cache with L1 (local memory) and L2 (remote Redis):

```text
Paper text:
"We propose a tiered cache architecture with L1 (local memory)
and L2 (remote Redis cluster). Consistency is maintained through
invalidating L1 entries when L2 updates occur."

Annotations:
! L1/L2 separation is common. Applied to cache consistency model.
? What happens to L1 during network partition? Stale reads?
* We use write-through. Trade-off: write latency vs. read freshness.
C L1 invalidation listener: L2 events -> broadcast -> clear local
A Compare invalidation vs. write-through for payment service
R Check reference [14]: "Consistency in Tiered Caches" (2023)
```

### Example 2: Quick Preview Triage

Five sources discovered in one morning:

```text
1. "Kubernetes: 10 Lessons Learned" — Blog, 2023, relevant to migration.
   Triage: B

2. "Why Rust is the Future" — Opinion, no author/date, hype, no code.
   Triage: X (Discard)

3. "YCSB: MongoDB vs. PostgreSQL" — Conference paper, 2024, reproducible.
   Triage: A

4. "CSS Grid Tips" — Tutorial, 2022, not my domain.
   Triage: D (Archive reference)

5. "RFC 9000: QUIC" — Standards track, 131 pages, Q2 deep dive.
   Triage: C
```

### Example 3: Permanent Note from Documentation

From PostgreSQL EXPLAIN ANALYZE docs:
