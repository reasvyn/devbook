...
### Case 1: Man-in-the-Middle Attack on DH

```python
def dh_mitm_demo():
    """Demonstrate DH without authentication is vulnerable to MITM."""
    p, g, _ = dh_generate_params(64)
    ...
    mallory_priv_a, mallory_pub_a = dh_keygen(p, g)  # for Alice
    mallory_priv_b, mallory_pub_b = dh_keygen(p, g)  # for Bob
    
    # Alice computes shared secret with Mallory's key
    alice_shared = dh_shared_secret(mallory_pub_b, alice_priv, p)
    # Bob computes shared secret with Mallory's key
    bob_shared = dh_shared_secret(mallory_pub_a, bob_priv, p)
    # Mallory computes both
    mallory_shared_a = dh_shared_secret(alice_pub, mallory_priv_b, p)
    mallory_shared_b = dh_shared_secret(bob_pub, mallory_priv_a, p)
    
    print(f"Alice-Mallory shared match: {alice_shared == mallory_shared_a}")
    print(f"Bob-Mallory shared match: {bob_shared == mallory_shared_b}")
    print("Mallory can intercept, decrypt, re-encrypt all messages!")

dh_mitm_demo()
```

**Solution:** Use authenticated key exchange (signed DH, STS protocol).

### Case 3: RSA-CRT Performance Comparison

```python
def rsa_performance_comparison():
    """Compare RSA decryption with and without CRT."""
    keys = rsa_keygen(2048)
    p, q = keys["p"], keys["q"]
    d = keys["private_key"]["d"]
    n = keys["public_key"]["n"]
    
    m = random.randint(2, n - 2)
    c = pow(m, keys["public_key"]["e"], n)
    ...
    print(f"Standard RSA: {t1:.4f}s")
    print(f"RSA-CRT: {t2:.4f}s")
    print(f"Speedup: {t1/t2:.2f}x")
    print(f"Correct: {m == m2}")

rsa_performance_comparison()
```

## Examples

### Example 1: RSA with Chinese Remainder

```python
def rsa_decrypt_crt(c: int, d: int, p: int, q: int) -> int:
    """RSA decryption accelerated with CRT."""
    dp = d % (p - 1)
    dq = d % (q - 1)
    qinv = pow(q, -1, p)
...
print(f"RSA-CRT decryption: {m2} (expected {m})")
```

### Example 2: ElGamal Homomorphic Property

```python
# ElGamal is homomorphic: E(m1) * E(m2) = E(m1 * m2)
p, g, _ = dh_generate_params(64)
pub, priv = elgamal_keygen(p, g)

m1 = 7
m2 = 11
c1 = elgamal_encrypt(m1, pub)
c2 = elgamal_encrypt(m2, pub)

# Multiply ciphertexts component-wise
c_prod = ((c1[0] * c2[0]) % p, (c1[1] * c2[1]) % p)
m_prod = elgamal_decrypt(c_prod, priv, p)
print(f"Product: {m_prod} (expected {m1 * m2 % p})")
```

### Example 3: ECDH Key Exchange

```python
def ecdh_full(curve: ECCurve, G: tuple):
    """Full ECDH key exchange demonstration."""
    alice_priv = random.randint(1, curve.p - 1)
    bob_priv = random.randint(1, curve.p - 1)
    ...
    # Use x-coordinate of shared point as key material
    key_alice = shared_alice[0].to_bytes(32, 'big')
    key_bob = shared_bob[0].to_bytes(32, 'big')
    
    print(f"Shared key (Alice): {key_alice.hex()[:20]}...")
    print(f"Shared key (Bob): {key_bob.hex()[:20]}...")
