...
### Case 1: Stripe's Documentation-First Approach

Stripe's developer documentation is widely considered the gold standard. Their approach:

- **Documentation before implementation:** API endpoints are documented before they are built
- **Dedicated documentation team:** Writers embedded with engineering teams
- **User testing:** Regular documentation usability studies
- **Code samples in 6+ languages:** All tested and verified
- **Interactive API console:** Try endpoints directly in the browser

Result: Stripe's developer experience (DX) is a competitive advantage. Developers choose Stripe partly because of the quality of its documentation.

### Case 2: A Documentation Rescue

A SaaS startup had 200 pages of API documentation written by engineers over three years. Key problems:

- No consistent structure (some pages used tables, some paragraphs)
- Code examples in only one language (curl)
- No getting-started guide
- Outdated screenshots from a previous UI

**The fix:**
1. Conducted user interviews with customers — identified the top 5 tasks
2. Wrote getting-started guides for each task
3. Established a documentation template (description, prerequisites, steps, examples)
4. Added code samples in Python, JavaScript, and Ruby
5. Set up CI checks for broken links and outdated references

Result: Support tickets about integration decreased by 40%. Customer onboarding time dropped from 2 weeks to 3 days.

### Case 3: Open Source Documentation Success

A popular JavaScript library had excellent code but minimal documentation. The README showed basic usage but nothing about error handling, configuration, or advanced patterns.

**Approach:**
1. Added a "Common Recipes" section — 10 practical examples answering the most common support questions
2. Created a dedicated documentation site using Docusaurus
3. Added search and keyboard navigation
4. Wrote migration guides for each major version
5. Added a contributing guide that included documentation standards

Result: Pull requests increased 3x. The time maintainers spent answering questions on GitHub Issues dropped by 60%.
...
```
In order to use this feature, you will need to ensure that you have installed the
prerequisite software packages. The installation can be accomplished by running
the setup script which will automatically detect your operating system and
install the appropriate dependencies.
```

After (technical writer edits):

```
1. Run `./setup.sh` to install dependencies.
   The script detects your operating system automatically.
```

Changes: Removed hedging, removed passive voice, removed redundancy, added code block, used numbered steps.

### Example 2: Writing for different audiences

Same feature, different documentation:

**For novices (Getting Started):**
```
Deploy your first application
1. Click "New Application" in the dashboard
2. Select your repository from the list
3. Click "Deploy"
Your application is now live at https://your-app.example.com
```

**For intermediates (How-to Guide):**
```
...
1. Create a .env file in your project root:
   ...
   API_KEY=sk-abc123
   ```
2. Commit the .env.example file (not .env)
3. Set the variables in your deployment dashboard
```
...
Good:
```
Error: Failed to connect to database "production"
Cause: Connection timeout after 30 seconds
Solution: Verify that the database host is reachable:
  ping db.example.com
Check firewall rules and network security groups.
```

The good message tells the reader what happened, why, and what to do about it.

### Example 4: README before and after
