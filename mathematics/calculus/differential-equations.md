...
### Case 1: Physics Engine — Projectile with Air Resistance

Realistic projectile motion requires solving:

$$
m\ddot{x} = -k v_x, \quad m\ddot{y} = -mg - k v_y
$$
...
def projectile_with_drag(t, state, m=1.0, g=9.81, k=0.1):
    x, y, vx, vy = state
...
v0, angle = 50, np.pi/4
vx0, vy0 = v0*np.cos(angle), v0*np.sin(angle)
sol = solve_ivp(projectile_with_drag, [0, 10], [0, 0, vx0, vy0],
                args=(1.0, 9.81, 0.1), max_step=0.01, events=lambda t, y: y[1])

x_final = sol.y[0, -1]
y_final = sol.y[1, -1]
print(f"Range with drag: {x_final:.2f}m")
print(f"Time of flight: {sol.t[-1]:.2f}s")

# Without drag: range = v0^2 * sin(2*theta) / g
range_no_drag = v0**2 * np.sin(2*angle) / 9.81
print(f"Range without drag: {range_no_drag:.2f}m")
```

### Case 2: Solving ODEs with scipy.integrate.odeint
...
x, y, z = trajectory.T
print(f"Lorenz attractor: x range=[{x.min():.1f}, {x.max():.1f}]")
print(f"                 y range=[{y.min():.1f}, {y.max():.1f}]")
print(f"                 z range=[{z.min():.1f}, {z.max():.1f}]")

# Sensitive dependence on initial conditions
eps = 1e-10
initial_state2 = [1.0 + eps, 1.0, 1.0]
trajectory2 = odeint(lorenz_system, initial_state2, t)

diff = np.linalg.norm(trajectory - trajectory2, axis=1)
print(f"Divergence after 40s with eps={eps}: {diff[-1]:.4f}")
```

### Case 3: Chemical Reaction Kinetics

The Brusselator is a model of an autocatalytic chemical reaction:
...
sol = solve_ivp(brusselator, [0, 50], [1.0, 1.0], args=(1.0, 3.0), max_step=0.01)
x, y = sol.y
print(f"Brusselator: x range=[{x.min():.2f}, {x.max():.2f}]")
print(f"             y range=[{y.min():.2f}, {y.max():.2f}]")
```

### Case 4: Predator-Prey with Harvesting

Adding harvesting to the Lotka-Volterra model:

$$
\frac{dx}{dt} = \alpha x - \beta xy - h_x x, \quad
\frac{dy}{dt} = \delta xy - \gamma y - h_y y
$$
...
def lotka_volterra_harvest(t, state, alpha=1.0, beta=0.1, delta=0.075,
                            gamma=1.5, hx=0.05, hy=0.05):
    x, y = state
    dx = alpha * x - beta * x * y - hx * x
    dy = delta * x * y - gamma * y - hy * y
    return [dx, dy]

# Without harvesting
sol_no = solve_ivp(lambda t, s: lotka_volterra_harvest(t, s, hx=0, hy=0),
                    [0, 100], [10, 5], max_step=0.1)

# With harvesting
sol_yes = solve_ivp(lotka_volterra_harvest, [0, 100], [10, 5], max_step=0.1)

print(f"No harvest: x={sol_no.y[0,-1]:.2f}, y={sol_no.y[1,-1]:.2f}")
print(f"Harvest:    x={sol_yes.y[0,-1]:.2f}, y={sol_yes.y[1,-1]:.2f}")
```

## Examples

### Example 1: Euler vs RK4 Convergence

```python
import numpy as np

def convergence_study(f, exact, y0, t_span, dt_values):
    """Compare Euler and RK4 convergence."""
    print(f"{'dt':<10} {'Euler error':<20} {'RK4 error':<20} {'Euler order':<15} {'RK4 order':<15}")
    print("-" * 80)
    ...
    for dt in dt_values:
        t_e, y_e = euler(f, y0, t_span, dt)
        t_r, y_r = rk4(f, y0, t_span, dt)
        exact_vals = exact(t_e)

        euler_err = np.max(np.abs(y_e - exact_vals[:len(y_e)]))
        rk4_err = np.max(np.abs(y_r - exact_vals[:len(y_r)]))

        euler_order = np.log2(prev_euler_err/euler_err) if prev_euler_err else 0
        rk4_order = np.log2(prev_rk4_err/rk4_err) if prev_rk4_err else 0

        print(f"{dt:<10.1e} {euler_err:<20.2e} {rk4_err:<20.2e} {euler_order:<15.2f} {rk4_order:<15.2f}")

        prev_euler_err = euler_err
        prev_rk4_err = rk4_err

f = lambda t, y: -2*y
exact = lambda t: np.exp(-2*t)
convergence_study(f, exact, 1.0, (0, 5), [0.1, 0.05, 0.025, 0.0125])
```

### Example 2: Solving an ODE Symbolically with SymPy
...
sol = sp.dsolve(ode, ics={y(0): 1, sp.diff(y(t), t).subs(t, 0): 0})
print(f"y'' - y = 0, y(0)=1, y'(0)=0: {sol}")

# y' = y - y^2 (logistic, y(0)=0.5)
ode2 = sp.Eq(sp.diff(y(t), t), y(t) - y(t)**2)
sol2 = sp.dsolve(ode2, ics={y(0): 0.5})
print(f"y' = y(1-y), y(0)=0.5: {sol2}")
```

### Example 3: Stiff ODEs and Special Solvers

Stiff ODEs require special methods (implicit, like BDF) because explicit methods like Euler or RK4 would need extremely small step sizes.
...
def stiff_system(t, y):
    """dy/dt = -1000y + 1000*cos(t) - sin(t).
    The -1000y term makes it stiff."""
    return -1000 * y + 1000 * np.cos(t) - np.sin(t)

# RK45 fails for stiff systems
sol_rk = solve_ivp(stiff_system, [0, 5], [0.0], method='RK45', max_step=0.1)
print(f"RK45: {sol_rk.t.shape[0]} steps, end value={sol_rk.y[0,-1]:.6f}")

# BDF (backward differentiation formula) handles stiffness
sol_bdf = solve_ivp(stiff_system, [0, 5], [0.0], method='BDF', max_step=0.1)
print(f"BDF:  {sol_bdf.t.shape[0]} steps, end value={sol_bdf.y[0,-1]:.6f}")

exact = np.cos(5)
print(f"Exact: {exact:.6f}")
```

### Example 4: Phase Portrait of a 2D System

```python
import numpy as np

def phase_portrait():
    """Analyze the phase portrait of dx/dt = y, dy/dt = -x - 0.2y (damped oscillator)."""
    from scipy.integrate import solve_ivp
    ...
    print("Phase portrait trajectories (final state):")
    for ic in initial_conditions:
        sol = solve_ivp(system, [0, 20], ic, max_step=0.1)
        xf, yf = sol.y[0, -1], sol.y[1, -1]
        print(f"  IC={ic}: final=({xf:.4f}, {yf:.4f})")
