...
### Case 1: Performance Feedback on a Hot Path

An engineer submitted a PR adding a new endpoint. The implementation used nested loops that the reviewer identified as a performance risk.

```text
// Original reviewer feedback (poor)
This is going to be slow. Redo it.

// Author's reaction
Frustrated — the comment gives no guidance and assumes bad intent.

// SBI-framed revision
SITUATION: In the new `/search` endpoint PR,
BEHAVIOR: the current implementation uses nested loops that perform a database query per result,
IMPACT: which will cause response times to grow linearly with result count. At 10,000 results, this could exceed our 500ms SLA.

Suggested approach: batch-load the related data with a single query using a WHERE IN clause.

// Author's reaction
Understood the concern, knew exactly what to fix, and learned about N+1 detection.
```

### Case 2: Receiving Difficult Feedback

An engineer received feedback that their documentation was unclear. Their initial reaction was defensive.

```text
// Feedback received
The API documentation for the `/users` endpoint does not explain error responses.
A developer integrating with this API would not know how to handle failures.

// Initial defensive response
The errors are standard HTTP codes. Any developer should know those.

// After applying Acknowledge-Reflect-Act
Acknowledge: "I hear you. The error responses are not documented explicitly."
Reflect: "Is the issue that the error format is undocumented? Or that specific error codes are missing?"
Act: "I will add an error response table to the docs. I will also include example error bodies for each code."
```

### Case 3: Cross-Team Escalation

Two teams disagreed on whether to share a database or use separate databases with an API layer.

```text
// Positions
Team A: "Shared database is simpler and faster to implement."
Team B: "Shared database creates tight coupling and a single point of failure."

// Escalation to architecture review
1. Both teams wrote one-paragraph position statements.
2. The architecture review meeting had a 45-minute timebox.
3. A compromise was reached: separate databases with a shared API layer, but Team A would own the API.
4. Decision was documented with explicit trade-offs and a six-month review date.

// Outcome
Both teams felt heard. Team A got their speed. Team B got their decoupling.
```

## Examples

### Example 1: Strong SBI Feedback

```text
SITUATION: In the incident review for the payment service outage,
BEHAVIOR: the runbook did not include steps for rolling back a partial deployment,
IMPACT: which added 45 minutes to the recovery time while the on-call engineer figured out the rollback process.

Suggestion: Add a rollback section to the payment service runbook and schedule a tabletop exercise to validate it.
```

### Example 2: Weak SBI Feedback (Fixed)

```text
// Weak
Your runbook was bad.

// Fixed with SBI
SITUATION: During the payment service outage,
BEHAVIOR: the runbook lacked rollback instructions,
IMPACT: which delayed recovery by 45 minutes.

Please add rollback steps.
```

### Example 3: Question-Framed Suggestion

```text
I noticed the config values are hardcoded in the service class. Have you considered using environment variables or a config file instead? That would let us change values without redeploying.
```

### Example 4: Direct Statement for Security Issue

```text
The API endpoint `/admin/users` does not check the caller's role before returning all user data. This is an authorization bypass.

Fix: Add a role check middleware that rejects requests without the `admin` role. I can pair on implementing this if needed.
```

### Example 5: Receiving Feedback Well

```text
// Feedback
The test coverage on this PR is low. There are no tests for the error paths.

// Good response
You are right. I focused on the happy path and skipped error handling tests. I will add tests for network timeouts, invalid input, and rate limiting. I should have included them from the start.
```

### Example 6: Handling Disagreement Professionally

```text
// Person A
I think we should rewrite the frontend in React.

// Person B
I disagree. The current Angular app works well, and a rewrite would take three months with high risk.

// Resolution
Person A: "My concern is developer velocity — the team is much stronger in React."
Person B: "My concern is delivery risk — we have commitments for the next quarter."
Compromise: "What if we migrate incrementally? We can build new features in React and gradually replace Angular components."
Both agreed because the compromise addressed both concerns.
```

### Example 7: Async Feedback Done Right

```text
Subject: Feedback on deployment script PR

Hi Maria,

I reviewed the deployment script PR. The automation is a great improvement over the manual process.

One concern: the script does not check for disk space before pulling the Docker image. If the disk is full, the pull fails silently and the deployment proceeds with the old image. We had a similar issue last quarter.

Could we add a disk space check at the start of the script? Something like:
  if [ $(df / | tail -1 | awk '{print $4}') -lt 5242880 ]; then
    echo "Error: insufficient disk space" && exit 1
  fi
...
### Example 8: Sync Feedback Agenda

```text
Agenda for 15-minute feedback sync:

1. Context: Your PR #342 for the caching layer
2. Concern: The cache invalidation strategy uses TTL but does not handle the "stale data" case
3. Suggestion: Consider a write-through cache with versioning
4. Outcome: Agree on approach or schedule follow-up with architect

Goal: Get to a clear decision, not to debate indefinitely.
```
