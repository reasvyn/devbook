...
### Study Case 1: Neural Network Layer as Matrix Multiplication

A fully connected neural network layer computes $\mathbf{h} = W\mathbf{x} + \mathbf{b}$, where $W$ is a weight matrix and $\mathbf{b}$ is a bias vector.
...
h = W @ x + b
print(f"Single sample output: {h}")

# Batched: stack inputs as rows
X = np.random.randn(32, 4)
H = X @ W.T + b  # 32x3 in one multiply
print(f"Batch output shape: {H.shape}")
```

### Study Case 2: Covariance Matrix in Statistics

The covariance matrix captures pairwise feature correlations. It is symmetric and positive semidefinite.
...
### Example 2: Solving Linear Systems with Matrix Inverse
...
# Using inverse (not recommended for large systems)
x_inv = np.linalg.inv(A) @ b
print(f"Via inverse: {x_inv}")
```

### Example 3: Determinant and Rank in Practice

```python
matrices = {
    "Full rank": np.array([[2, 0], [0, 3]]),
    "Singular": np.array([[1, 2], [2, 4]]),
    "Near-singular": np.array([[1, 2], [2, 4.0001]])
}
...
for name, M in matrices.items():
    ...
    print(f"{name}: det={det:.4f}, rank={rank}, cond={cond:.2f}")
```

### Example 4: Matrix Properties Exploration
...
def analyze_matrix(A):
    ...
    if A.shape[0] == A.shape[1]:
        ...
        print(f"Invertible: {np.linalg.det(A) != 0}")
...
### Example 5: Hadamard Product and Broadcasting
...
factors = np.array([10, 0.1]).reshape(-1, 1)
scaled = A * factors  # broadcasting
print("Scaled rows:\n", scaled)

# Add bias to each column
bias = np.array([100, 200, 300])
biased = A + bias  # broadcasting
