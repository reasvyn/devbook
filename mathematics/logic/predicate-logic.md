...
### Case 1: Translating Business Rules to Logic

A company has these rules:
1. Every manager is an employee: $\forall x (\text{Manager}(x) \to \text{Employee}(x))$
2. Some managers are remote: $\exists x (\text{Manager}(x) \land \text{Remote}(x))$
3. No remote employee is a contractor: $\neg \exists x (\text{Remote}(x) \land \text{Contractor}(x))$
4. Every employee has a manager: $\forall y (\text{Employee}(y) \to \exists x (\text{Manager}(x) \land \text{Manages}(x, y)))$

These can be used to verify a database schema or generate test cases.

```sql
-- Rule 3 as SQL
SELECT * FROM employees
WHERE is_remote = 1 AND is_contractor = 1;
-- Should return 0 rows
```

### Case 2: Checking an Invariant

A banking system has the invariant:

$$\forall a \in \text{Accounts} (\text{balance}(a) \ge 0)$$

After each transaction, the system must verify the invariant:

```python
def check_invariant(accounts):
    return all(a.balance >= 0 for a in accounts)

# The negation (what would make the invariant false):
def violates_invariant(accounts):
    return any(a.balance < 0 for a in accounts)
```

### Case 3: Nested Quantifier in a Search Engine

Search for "documents where all authors are senior":

$$\{d \in \text{Docs} \mid \forall a \in \text{Authors} (\text{AuthorOf}(a, d) \to \text{Senior}(a))\}$$

```python
def documents_with_only_senior_authors(docs):
    result = []
    for doc in docs:
        if all(is_senior(a) for a in doc.authors):
            result.append(doc)
    return result
```
...
### Example 1: Translating Natural Language to Predicate Logic

| Natural Language | Predicate Logic |
|-----------------|-----------------|
| All cats are mammals | $\forall x (\text{Cat}(x) \to \text{Mammal}(x))$ |
| Some birds can fly | $\exists x (\text{Bird}(x) \land \text{Flies}(x))$ |
| No fish are mammals | $\neg \exists x (\text{Fish}(x) \land \text{Mammal}(x))$ |
| Only senior devs review | $\forall x (\text{Reviews}(x) \to \text{Senior}(x))$ |
| Every user has a unique email | $\forall u \in U, \forall v \in U (u \neq v \to \text{email}(u) \neq \text{email}(v))$ |

### Example 2: Failure Mismatch, see Failure Mismatch

$$\forall x \exists y \text{ ParentOf}(y, x) \quad \text{(Everyone has a parent)}$$
$$\exists y \forall x \text{ ParentOf}(y, x) \quad \text{(Someone is parent of everyone)}$$

The first is true in any populated domain. The second is false (no single person is everyone's parent).

### Example 3: Negating a Complex Formula

Original: $\forall x (\text{Student}(x) \to \exists y (\text{Course}(y) \land \text{Enrolled}(x, y)))$

Negation step by step:

$$\neg \forall x (\text{Student}(x) \to \exists y (\text{Course}(y) \land \text{Enrolled}(x, y)))$$
$$\equiv \exists x \neg (\text{Student}(x) \to \exists y (\text{Course}(y) \land \text{Enrolled}(x, y)))$$
$$\equiv \exists x (\text{Student}(x) \land \neg \exists y (\text{Course}(y) \land \text{Enrolled}(x, y)))$$
$$\equiv \exists x (\text{Student}(x) \land \forall y \neg (\text{Course}(y) \land \text{Enrolled}(x, y)))$$
$$\equiv \exists x (\text{Student}(x) \land \forall y (\neg \text{Course}(y) \lor \neg \text{Enrolled}(x, y)))$$

Final: "There is a student who is not enrolled in any course."

### Example 4: TypeScript Type Predicates

```typescript
// Predicate logic in TypeScript's type system
function isString(value: unknown): value is string {
    return typeof value === 'string';
}

// The return type `value is string` means:
// ∀value (isString(value) → typeof value === 'string')
```

### Example 5: Prolog Implementation

Prolog is essentially a predicate logic theorem prover:

```prolog
% Facts (atomic predicates)
parent(alice, bob).
parent(bob, carol).
parent(carol, dave).

% Rules (quantified implications)
ancestor(X, Y) :- parent(X, Y).                    % ∀x∀y parent(x,y) → ancestor(x,y)
ancestor(X, Y) :- parent(X, Z), ancestor(Z, Y).    % ∀x∀y ∃z parent(x,z) ∧ ancestor(z,y) → ancestor(x,y)

% Queries (existential)
% ?- ancestor(alice, dave).   → true
% ?- ancestor(X, dave).       → X = carol ; X = bob ; X = alice
```
