...
### Style Form States Without JavaScript

Using pseudo-classes for form validation styling:
...
/* While typing: subtle blue */
input:focus {
    border-color: #0066cc;
    outline: none;
}

/* Invalid after user interaction: red */
input:user-invalid {
    border-color: #dc3545;
    background: #fff5f5;
}

/* Valid after interaction: green */
input:user-valid {
    border-color: #28a745;
    background: #f0fff4;
}
...
/* Current page — works with SSR or static sites */
...
### Alternating Row Colors Without Extra Classes
...
/* First and last rows with rounded corners */
...
### Example 1: Zebra-striped table

```css
/* Even rows get a background */
tbody tr:nth-child(even) {
    background: #f8f9fa;
}
...
/* Cards that are "featured" get a border */
.card:has(.badge--featured) {
    border-color: gold;
}
```

### Example 3: Attribute selector for data-driven styling

```css
/* Data attribute for priority */
[data-priority="high"] { background: #dc3545; color: white; }
[data-priority="medium"] { background: #ffc107; }
[data-priority="low"] { background: #28a745; color: white; }

/* Data attribute for status */
[data-status="active"] { border-left: 4px solid #28a745; }
[data-status="inactive"] { border-left: 4px solid #dc3545; opacity: 0.6; }
```
