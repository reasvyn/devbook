...
### Case 1: Implementing a JSON Parser

JSON has a simple grammar, completable in under 100 lines.
...
A recursive descent parser mirrors this grammar directly: `parse_value()` dispatches to `parse_object()`, `parse_array()`, `parse_string()`, etc.

### Case 2: Natural Language Parsing with spaCy
...
for token in doc:
    print(f"{token.text:12} {token.pos_:6} {token.dep_:12} {token.head.text}")
```

Output: Each token shows its POS tag, dependency relation, and head word. The dependency tree reveals that "jumps" is the root, "fox" is its subject, and "dog" is the object of "over".

### Case 3: Error Recovery with Tree-sitter

Tree-sitter maintains AST even with syntax errors using ERROR nodes:
...
This allows IDEs to provide autocomplete and highlighting despite errors.
...
### Example 2: Shift-Reduce Parse of `id + id * id`
...
### Example 3: Lexer for a Mini Language
...
### Example 4: PEG for a Calculator
...
### Example 5: POS Tagging with HMM

Viterbi algorithm finds the most likely tag sequence: `P(T|W) proportional to P(W|T) * P(T)` where emission probabilities come from word-tag co-occurrence and transition probabilities from tag-sequence patterns.
