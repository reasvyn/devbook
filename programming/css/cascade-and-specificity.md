...
```css
/* Problem: button is blue, not red */
<button class="btn btn-primary">Submit</button>

.btn { background: gray; }
.btn-primary { background: blue; }
/* Later in the stylesheet: */
.btn { background: red; }       /* Should this make it red? NO! */
```

Why: `.btn-primary` has same specificity as `.btn` (both 0,0,1,0). But `.btn-primary` appears in the stylesheet first, so the cascade tries order next. Wait — `.btn { background: red; }` is later. But `.btn-primary { background: blue; }` has the same specificity. In this case, since both target the same element, the browser evaluates which declarations match. The element has both classes. `.btn { background: red; }` matches and `.btn-primary { background: blue; }` matches. They have equal specificity. `.btn { background: red; }` appears later, so `background: red` should win.

Actually wait — let me re-check. If `.btn-primary` comes first and `.btn` comes second, both apply to the same element. Same specificity. Order of appearance — the later `.btn` block wins. So `background: red` applies.

But if `.btn-primary { background: blue !important; }` then `!important` changes the origin and beats the normal `.btn` rule. This is the most common source of cascade confusion.

### Cascade Layers for Third-Party CSS

```css
/* Put third-party CSS in a low-priority layer */
@layer reset, vendor, theme;

@layer vendor {
    /* Import Bootstrap or other framework here */
...
Without layers, every third-party selector with higher specificity wins. With layers, your theme layer overrides vendor regardless of specificity.

## Examples

### Example 1: Forcing inheritance

```css
/* Reset button to look like plain text */
.text-button {
    all: unset;
    display: inline;
    cursor: pointer;
...
```css
/* Sidebar should not inherit the dark background */
.sidebar {
    background: #333;
    color: #fff;
}
...
.sidebar a {
    color: inherit;             /* Inherit white from .sidebar */
}

.sidebar a:not([class]) {
    color: #66b3ff;             /* Links without class get a distinct color */
}
```

### Example 3: Revert for component isolation

```css
/* In a web component, revert to browser default for h1 */
:host h1 {
    font-size: revert;
    margin: revert;
    font-weight: revert;
