...
### Case 1: Blog Database Schema with Relationships

A blog with posts, authors, comments, and tags. This demonstrates one-to-many and many-to-many relationships with foreign keys and referential actions.
...
- **One-to-many:** author -> posts, post -> comments
- **Many-to-many:** posts <-> tags (via `post_tags` junction table)
- **Cascading deletes:** removing a post deletes its comments and junction rows

### Case 2: Normalizing a Denormalized Spreadsheet

A product manager exports sales data as a single spreadsheet with repeated groups. Transform it into a normalized 3NF schema.

**Raw spreadsheet (violates 1NF, 2NF, 3NF):**

| OrderID | Customer | Email | Product | Quantity | Price | Category |
|---------|----------|-------|---------|----------|-------|----------|
| 1 | Alice | alice@... | Mouse,Keyboard | 2,1 | 25,90 | Electronics |

**Step 1 -- 1NF:** Split the multi-valued columns into separate rows.

**Step 2 -- 2NF:** Separate product details from order details (product_name, price depend on product_id, not on order_id).

**Step 3 -- 3NF:** Extract category into its own table (category_name depends on category_id).

Result: the `customers`, `orders`, `order_items`, `products`, `categories` schema used throughout this module. Each fact is stored once and referenced by key.

### Case 3: Foreign Key Design for a Multi-Tenant SaaS

A SaaS application needs to isolate tenant data while sharing the same schema. Every table includes a `tenant_id` as part of the composite key.
...
Composite foreign keys enforce that a task belongs to the right project within the right tenant. The cascading delete ensures that removing a project removes all its tasks without affecting other tenants' data.

## Examples

### E-Commerce Schema Overview

Below is a text representation of a typical e-commerce database schema demonstrating all three relationship types. The `*` marks primary keys, and `FK` marks foreign keys.
...
inventory
  inventory_id *     SERIAL
  product_id         INTEGER  FK -> products(product_id)  [one-to-one]
  quantity           INTEGER
  warehouse          TEXT
...
| products -> inventory | One-to-One (product has one inventory record) |
| products -> reviews | One-to-Many |
| customers -> reviews | One-to-Many |
| products -> categories | Many-to-One (category has many products) |
| order_items links orders & products | Many-to-Many (via junction table) |

### Relationship Decision Tree

```
Does each A have at most one B?
  ├── Yes → Does each B have at most one A?
  │   ├── Yes → One-to-One (store FK in either table, or use shared PK)
  │   └── No  → One-to-Many (store FK in the "many" table)
  └── No  → Many-to-Many (create a junction table)
```
...
```
Unnormalized:
  orders(id, customer, email, products, quantities)

1NF (atomic columns):
  orders(id, customer, email)
