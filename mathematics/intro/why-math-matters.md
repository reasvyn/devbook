...
### Case 1: Debugging a Performance Regression

A team notices that a search endpoint slowed down after a code change. The original implementation used a hash set for lookups ($O(1)$ average), but the new implementation accidentally used a list ($O(n)$):

```python
# Before — fast
def find_active_users(all_users, active_ids):
    active_set = set(active_ids)  # O(n) to build, O(1) lookups
    return [u for u in all_users if u.id in active_set]

# After — slow
def find_active_users(all_users, active_ids):
    # active_ids is now a list — O(n) per lookup
    return [u for u in all_users if u.id in active_ids]
```

Mathematical reasoning identifies the problem: the list version has $O(n \cdot m)$ complexity where $n$ is the number of users and $m$ is the number of active IDs. The set version has $O(n + m)$. For 10,000 users and 5,000 active IDs, the list version does 50 million comparisons versus 15,000 operations for the set version.

### Case 2: Diagnosing NaN Values in Gradient Descent

A machine learning engineer finds that the loss becomes NaN after a few training steps. Mathematical reasoning identifies the cause: the learning rate is too high, causing parameter updates to overshoot and diverge.
...
If $\eta$ is too large, $\theta_{t+1}$ may be farther from the minimum than $\theta_t$, causing divergence. The fix is to reduce $\eta$ or add gradient clipping.

```python
# Fix 1: Reduce learning rate
optimizer = torch.optim.SGD(model.parameters(), lr=0.001)  # was 0.1

# Fix 2: Gradient clipping
torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
```

### Case 3: Designing a Rate Limiter

An API needs to limit requests to 100 per minute per user. The team considers sliding window vs. fixed window approaches.

Fixed window uses modular arithmetic:

```python
def is_rate_limited_fixed(user_id):
...
Sliding window uses a more precise mathematical model:

```python
def is_rate_limited_sliding(user_id):
    now = time.time()
    key = f"{user_id}:sliding"
...
The sliding window approach corresponds to maintaining a moving window over a time series — a concept from signal processing and statistics. Both are mathematical models applied to infrastructure design.

## Examples

### Example 1: Converting a Loop Invariant to Code

The invariant: at the start of each iteration of a linear search, the target has not been found in the already-searched portion. This invariant justifies the search algorithm:

```python
def linear_search(arr, target):
    # Invariant: target not in arr[:i]
...
### Example 2: Counting Subsets for Test Coverage

A function takes three parameters where each can be null or non-null. How many test cases do you need?

Mathematically, this is the set of all subsets of three nullable parameters:

$$
\text{test cases} = 2^3 = 8
$$
...
### Example 3: Parse Tree and Boolean Algebra

A complex boolean expression in code:

```python
if (user.is_active or user.is_admin) and not user.is_banned:
```

This maps to the boolean algebra expression $(A \lor B) \land \lnot C$ where $A$ = is_active, $B$ = is_admin, $C$ = is_banned. The truth table covers all eight combinations:
...
Applying De Morgan's law: $\lnot (A \lor B) \lor C$ is equivalent to $(\lnot A \land \lnot B) \lor C$. This transformation can simplify conditionals:
