...
### Case 1: A/B Testing a New Landing Page

An e-commerce site tests a new landing page design. Control: 5.2% conversion (104/2000). Variant: 6.1% (122/2000).
...
ci_diff = (p_b - p_a) - 1.96 * se_diff, (p_b - p_a) + 1.96 * se_diff

print(f"Control: {p_a:.4f}, Variant: {p_b:.4f}")
print(f"Difference: {p_b-p_a:.4f} ({((p_b-p_a)/p_a*100):.1f}% lift)")
...
print(f"Required n per group for 80% power: {sample_size_two_prop(p_a, p_b)}")
```

Output:
```
Control: 0.0520, Variant: 0.0610
Difference: 0.0090 (17.3% lift)
z-stat: 1.236, p-value: 0.2165
95% CI for difference: [-0.0052, 0.0232]
Required n per group for 80% power: 4343
```

The difference is not statistically significant ($p = 0.217$). The confidence interval includes zero. We need approximately 4,343 users per group to detect this effect with 80% power.

### Case 2: Comparing Two Code Optimizations

A developer compares three implementations of a sorting function: baseline, optimization A, and optimization B.
...
baseline = np.random.normal(250, 20, 50)   # ms
opt_a = np.random.normal(230, 20, 50)
opt_b = np.random.normal(210, 20, 50)

# ANOVA
f_stat, p_value = stats.f_oneway(baseline, opt_a, opt_b)
print(f"ANOVA: F={f_stat:.2f}, p={p_value:.6f}")

# Pairwise t-tests with Bonferroni correction
groups = [('Baseline', baseline), ('Opt A', opt_a), ('Opt B', opt_b)]
n_tests = 3
alpha = 0.05
...
for i in range(len(groups)):
    for j in range(i+1, len(groups)):
        ...
        t_stat, p = stats.ttest_ind(data_i, data_j)
        significant = p < bonf_alpha
        print(f"{name_i} vs {name_j}: t={t_stat:.2f}, p={p:.4f}, " +
              f"significant (Bonf): {significant}")
```

### Case 3: Model Evaluation — Is Model A Better Than Model B?

Two ML models are evaluated on the same test set using k-fold cross-validation. The paired t-test accounts for the paired nature of the comparison.
...
model_a = np.array([0.92, 0.88, 0.91, 0.89, 0.90])
model_b = np.array([0.91, 0.87, 0.90, 0.88, 0.91])

# Paired t-test
t_stat, p_value = stats.ttest_rel(model_a, model_b)
mean_diff = np.mean(model_a - model_b)
ci = stats.t.interval(0.95, df=len(model_a)-1,
                      loc=mean_diff,
                      scale=stats.sem(model_a - model_b))

print(f"Model A mean: {np.mean(model_a):.4f}")
print(f"Model B mean: {np.mean(model_b):.4f}")
...
### Example 1: One-Tailed vs Two-Tailed Tests

Use a one-tailed test only when you have a strong directional hypothesis. If you test a new drug, you might use a one-tailed test (new drug is better). But convention favors two-tailed tests because they are more conservative and don't assume direction.

```python
from scipy import stats

# One-tailed p-value is half of two-tailed
t_stat = 2.5
df = 30
p_two_tailed = 2 * stats.t.sf(abs(t_stat), df)
p_one_tailed = stats.t.sf(t_stat, df)  # for positive t (right-tailed)
...
When the sampling distribution is unknown (non-normal data, complex statistics), use the bootstrap — resample with replacement many times.
...
### Example 3: The Difference Between "Not Significant" and "No Effect"

A study with 20 subjects finds $p = 0.09$. This does NOT mean there is no effect — the study may be underpowered. Report the effect size and confidence interval, not just the p-value.
...
n_small = 10
n_large = 200
effect = 0.5  # true effect size

# Small sample
small_a = np.random.normal(0, 1, n_small)
small_b = np.random.normal(effect, 1, n_small)
_, p_small = stats.ttest_ind(small_a, small_b)

# Large sample
large_a = np.random.normal(0, 1, n_large)
large_b = np.random.normal(effect, 1, n_large)
_, p_large = stats.ttest_ind(large_a, large_b)

print(f"Small sample (n={n_small}): p={p_small:.4f} {'significant' if p_small<0.05 else 'not significant'}")
print(f"Large sample (n={n_large}): p={p_large:.4f} {'significant' if p_large<0.05 else 'not significant'}")
```

### Example 4: Mann-Whitney U Test (Non-parametric Alternative)

When normality assumptions are violated, use a rank-based test.
...
### Example 5: Interpreting Confidence Intervals

A 95% CI of [0.02, 0.18] for a difference in conversion rates means: all values between 2% and 18% are plausible for the true difference.

- [0.02, 0.18]: Effect is positive and practically significant (smallest plausible effect is 2%)
- [-0.01, 0.12]: Effect may be zero or negative (not statistically significant)
- [0.001, 0.003]: Effect is statistically significant but may not be practically significant

```python
import numpy as np

results = [
    ("Significant & meaningful", 0.10, 0.04),
    ("Not significant", 0.05, 0.03),
    ("Significant but tiny", 0.002, 0.0005),
]

for label, effect, se in results:
    ci = (effect - 1.96*se, effect + 1.96*se)
    print(f"{label:25s}: effect={effect:.4f}, 95% CI=[{ci[0]:.4f}, {ci[1]:.4f}]")
```
