...
### Case 1: Building a Dynamic To-Do List

```html
<ul id="todo-list">
    <li>Learn JS <button class="delete">✕</button></li>
</ul>
...
### Case 2: Generic Table Filter

Filter table rows by user input:
...
filterTable("search-input", "data-table");
```

### Case 3: Lazy-Loading Images
...
### Example 1: Get All Checked Checkboxes
...
### Example 2: Create a Navigation Menu from Data

```javascript
function buildNav(items, parentSelector) {
    ...
    document.querySelector(parentSelector).append(ul);
}
...
searchInput.addEventListener("input", debounce((e) => {
    ...
    results.innerHTML = `Searching for: ${e.target.value}`;
}, 300));
```
