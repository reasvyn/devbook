...
### Case 1: Python's Indentation as Syntax

Most programming languages use braces or keywords to delimit blocks. Python uses indentation. This is a rare case of a typographic feature (whitespace) being elevated to syntactic status — analogous to how tone is phonemic in Mandarin but not in English.

```python
# These two programs are structurally different
if True:
    print("A")
    print("B")
...
In the first, both prints are inside the if. In the second, only print("A") is inside. This is impossible in C/Java where braces make block structure explicit.

### Case 2: Grammar Error Messages as Pragmatic Communication

Compiler error messages have evolved from terse, unhelpful statements to detailed, context-aware explanations. This is a shift in pragmatics — the compiler is learning to communicate more effectively with its human interlocutor:
...
42 |     let x: i32 = "hello";
  ...
  |
help: try using `.parse()` method
  |
42 |     let x: i32 = "hello".parse();
  |                   +++++++++++++
```

The modern message follows Grice's maxims: it gives enough information, it's truthful, it's relevant, and it's clear.

### Case 3: The Evolution of Code Search

Early programmers searched code with `grep`. Modern code search uses NLP techniques:

- **Semantic search:** Find functions by what they do, not just by name
- **Type-aware search:** Match patterns at the type level
- **AST search:** Search for code structure, not text

Tools like Sourcegraph, Codota, and GitHub Code Search apply NLP techniques (embeddings, language models) to code. They treat code as a kind of natural language.

### Case 4: Bilingual Programmers

Programmers who speak multiple natural languages show interesting patterns:

- **Variable names:** Often in the programmer's native language, not English
- **Comments:** Code-switching between English and the native language
- **Technical discussions:** Most discussion happens in English, but problem-solving often switches to the native language
- **Learning resources:** Programmers prefer resources in their native language but accept English when better quality exists

A study of Chinese and Japanese developers found that reading comprehension improves significantly when documentation and code comments are in the programmer's native language, even for English-fluent developers.

## Examples

### Example 1: Tree structures in language and code
...
Parse tree (simplified constituency):
```
        S
       / \
      NP        VP
...
Both reveal hierarchical structure beneath linear sequences. The tree is the same kind of object — a constituency structure — whether for natural or programming language.

### Example 2: Ambiguity in English vs. Code

English sentence (ambiguous):
"Visiting relatives can be boring."

Interpretation 1: The act of visiting relatives is boring.
Interpretation 2: Relatives who are visiting are boring.

Two different parse trees, same surface string.
...
This is unambiguous to the compiler (`*(p++)` per C operator precedence), but human readers must know the precedence rules. "I before E except after C" is to English spelling what operator precedence is to programming — a rule that reduces ambiguity.

### Example 3: Morphology in naming conventions

Natural language derivation:
- compute (verb) → computer (noun) → computational (adjective) → computationally (adverb)

Programming naming:
- `parseJSON` (verb phrase) → `JSONParser` (noun) → `parsedJSON` (adjective) → `parseJSONAsync` (modified verb)

The morphological patterns differ by programming language:
- Java: `parseJSON`, `JSONParser`, `parsedJSON`
- Python: `parse_json`, `JsonParser`, `parsed_json`
- C#: `ParseJson`, `JsonParser`, `ParsedJson`

The same grammatical transformations, different orthographic conventions.

### Example 4: Code as speech act
...
// Illocution: programmer's intent
// "I want to compute the sum of item prices"

// Perlocution: effect on reader
// The reader understands: this function aggregates prices
```

A well-named function's illocutionary force is immediately clear. A poorly named function (`doStuff`) creates a pragmatic gap between intent and expression.

### Example 5: Language change over time

Old English: "Hwæt! We Gar-Dena in gear-dagum þeod-cyninga þrym gefrunon..."

Modern translation: "Listen! We have heard of the glory of the Spear-Danes' kings in days of yore..."
...
dict.iteritems()     # iterator over items

# Python 3
print("Hello")       # function call
print("Hello", end="")  # keyword argument
input()              # read string (raw_input removed)
dict.items()         # returns view (iteritems removed)
```

Both examples show language change: sounds and words were lost in English; statements became functions and functions were renamed in Python.
