...
### Case 1: Building a Knowledge Base for a New Domain

A frontend developer decides to learn distributed systems from scratch.

Week 1: Read introductory content, create atomic notes for CAP theorem, consensus, replication. Link notes to show relationships.

Week 2: Read the Raft paper, create detailed note linked to CAP theorem and consensus. Create code snippet note for Raft in Go.

Week 3: Read about ZooKeeper and etcd. Create comparison table: Raft vs. Paxos vs. Zab.

Month 2: 40+ interconnected notes. Can explain distributed consensus concepts to peers.

### Case 2: Recovering from Information Overload

A developer accumulated 2000+ bookmarks, 500+ highlights, and 30 open browser tabs. They feel overwhelmed and cannot find anything.

Recovery plan:

1. **Prune aggressively:** Delete bookmarks older than 2 years. Archive 80% of highlights without processing.
2. **Start fresh with one topic:** Pick the most relevant domain (currently learning Kubernetes).
3. **Process top-down:** Read official Kubernetes documentation. Create atomic notes for core concepts (pods, deployments, services, configmaps).
4. **Limit inputs:** Unsubscribe from 5 newsletters. Reduce Twitter/LinkedIn follows by 50%.
5. **Set a weekly note limit:** Maximum 10 new notes per week. Quality over quantity.

Outcome: After 1 month, the developer has 40 well-connected Kubernetes notes, no open tabs, and a manageable information diet.

### Case 3: Using Notes to Debug a Production Issue

A production incident occurs: database connections are being exhausted. The on-call developer needs to diagnose quickly.

The developer searches their notes:

- "database-connection-pool.md" — explains connection pooling patterns
- "postgresql-max-connections.md" — notes on PostgreSQL connection limits
- "connection-leak.md" — notes from a previous incident

The notes remind the developer:

1. Check `SELECT * FROM pg_stat_activity` for idle connections
2. Look for connection pool configuration (min/max idle, max lifetime)
3. The previous incident was caused by a connection leak in the ORM
4. The fix was to add connection validation

Using the notes, the developer diagnoses the issue in 15 minutes (connection leak from a recent code change that bypassed the pool) and rolls back the change. Without notes, this would have taken hours of investigation.

### Case 4: Converting from Passive to Active Reading

A developer reads extensively but cannot recall details. After adopting active reading:

- Read 3-5 high-quality sources per week instead of 10-15
- Take atomic notes and write a one-paragraph summary for each source
- Link new notes to existing knowledge

After 3 months: 200+ interconnected notes, recall of details from months ago, one blog post per month from note material.
...
A distributed transaction protocol that ensures all participants
agree to commit or abort a transaction.
