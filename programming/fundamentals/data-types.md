...
**Case 1: Type coercion in JavaScript.**

A developer calculates a total price:
...
console.log(calculateTotal([10, 20, "30"]));  // "102030"!
```

The bug: `"30"` is a string. The `+=` operator performs string concatenation when one operand is a string. The fix: ensure all values are numbers before addition:

```javascript
function calculateTotal(prices) {
    let total = 0;
    for (const price of prices) {
        total += Number(price);  // explicit conversion
    }
    return total;
}
```

This demonstrates why explicit type conversion is safer than relying on coercion.

**Case 2: Floating-point in financial calculations.**

A banking application calculates interest:
...
After 10 years, the result may differ from the mathematically correct value by a few cents due to accumulated rounding errors. For financial applications, use the `Decimal` type or integer arithmetic:
...
**Case 3: String input validation and type safety.**

A web form collects user age as a string. The developer needs to validate and convert it:

```python
def process_age(user_input):
    ...
    if not age_str.isdigit():
        return None, "Age must be a positive number"
...
This case combines string operations (strip, isdigit), explicit type conversion (int), and null handling (None for error cases). Each step validates before proceeding, avoiding runtime type errors.

## Examples

**Example 1: Type conversion chain.**

JavaScript coercion in `"5" + 3 * "2"`:

Step 1: `3 * "2"` → `3 * 2` (string `"2"` coerced to number) → `6`
Step 2: `"5" + 6` → `"5" + "6"` (number `6` coerced to string) → `"56"`

Result: `"56"`

**Example 2: String method chaining.**

```python
# Method chaining — each method returns a new string
text = "  Hello, World!  "
...
Each method returns a new string (strings are immutable), so you can chain them.

**Example 3: Boolean short-circuit for default values.**

```javascript
function greetUser(config) {
    const name = config.name || "Guest";
    const greeting = config.greeting || "Hello";
    const prefix = config.prefix ?? "";  // nullish coalescing — only replaces null/undefined
    return `${greeting}, ${prefix}${name}!`;
}

console.log(greetUser({ name: "Alice", greeting: "Hi" }));  // "Hi, Alice!"
console.log(greetUser({}));                                  // "Hello, Guest!"

// The difference between || and ??
console.log(0 || 100);    // 100 — 0 is falsy, so || falls through
console.log(0 ?? 100);    // 0 — 0 is not null/undefined, so ?? keeps it
```

The `||` operator returns the first truthy value, which works for most default-value patterns but fails when the valid value is falsy (e.g., `0`, `""`). The nullish coalescing operator `??` only falls through for `null` and `undefined`, which is safer for numeric defaults.
