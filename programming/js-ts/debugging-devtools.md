...
### Case 1: Debugging Memory Leak from Detached Nodes

```javascript
// Problem: elements keep accumulating in memory
function createNotifications() {
    const container = document.getElementById("notifications");
    for (let i = 0; i < 100; i++) {
        const div = document.createElement("div");
        div.textContent = `Notification ${i}`;
        container.appendChild(div);

        // Bug: listener holds reference to div even after removal
        div.addEventListener("click", () => {
            console.log(div.textContent);     // closure keeps div alive
        });
    }
    // No cleanup — removed elements stay in memory
}
```
...
2. Filter for "detached" — see `HTMLDivElement` instances
3. Click one to see the retaining path in the Retainers pane
4. Identify the closure holding the reference
5. Fix: use event delegation or properly clean up

### Case 2: Debugging Network Request Timing

```javascript
// Problem: slow page load
async function loadPage() {
    const user = await fetch("/api/user");
    const posts = await fetch("/api/posts");    // sequential — waits for user
    const comments = await fetch("/api/comments"); // waits for both
}
```

**Debugging steps:**

1. Open Network → check "Preserve log"
2. Reload the page
3. Look at the Waterfall column — requests are sequential (not overlapped)
4. Each request starts after the previous finishes
5. Fix: use `Promise.all()` for independent requests

### Case 3: Debugging Event Storm

```javascript
// Problem: resize handler fires too often
window.addEventListener("resize", () => {
    const elements = document.querySelectorAll(".responsive");
    for (const el of elements) {
        el.style.width = `${window.innerWidth / 2}px`; // layout thrash
    }
});
```
...
2. Resize the window
3. Stop recording — look for red FPS bars
4. Find repeated "Layout" events in the flame chart
5. Fix: debounce or throttle, batch writes and reads separately

## Examples

### Example 1: Debugging with Conditional Breakpoints

```javascript
function processItems(items) {
    for (let i = 0; i < items.length; i++) {
        const result = transform(items[i]);   // add conditional breakpoint: items[i].id === 42
        save(result);
    }
}
```

Set a conditional breakpoint on the first line of the loop: `items[i].id === 42`. The debugger pauses only for that specific item.

### Example 2: Console Group for Structured Debugging
...
### Example 3: Monitoring Property Changes
...
### Example 4: Network Request Logger

```javascript
(function debugNetwork() {
    const originalFetch = window.fetch;
    window.fetch = async (...args) => {
        console.groupCollapsed(`fetch ${args[0]}`);
        console.time(args[0]);
        try {
...
### Example 5: Quick Element Selector

```javascript
// In the console — utility function
const $x = (xpath) => {
    const results = [];
