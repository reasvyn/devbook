...
**Case 1: The form that submits despite invalid data.**

A developer relies entirely on client-side validation. An attacker uses curl to POST directly to the server endpoint with invalid data. The server accepts it because there is no server-side validation.

The fix: implement server-side validation for every field.

**Case 2: The validation message that screen readers miss.**

A developer shows error messages by adding text to a `<div>` near the input but does not associate it with `aria-describedby`. Screen reader users see no indication that the field has an error.
...
**Case 3: The real-time validation that fires too early.**

A developer validates on every keystroke from the start. Users see error messages before they have finished typing, creating a frustrating experience:

```javascript
email.addEventListener('input', function() {
    showError(email, email.validationMessage); // Shows error on first keystroke
});
```

The fix: validate on blur first, then on input after first interaction:
...
email.addEventListener('blur', function() {
    ...
    if (!email.checkValidity()) {
        showError(email, email.validationMessage);
    }
});
...
**Case 4: The pattern that rejects valid input.**

A developer uses a strict pattern that does not account for reasonable variations:

```html
<input type="text" name="phone" pattern="[0-9]{3}-[0-9]{3}-[0-9]{4}">
```

Users with international phone numbers (e.g., +44 20 7946 0958) cannot submit the form.

The fix: use a more permissive pattern or `type="tel"` without validation:
...
**Example 1: Inline validation for individual fields.**
...
<div>
    ...
    <span id="age-error" role="alert" class="error"></span>
</div>
```
...
**Example 2: Async username availability check.**
...
<div>
    ...
    <span id="username-status" role="alert"></span>
</div>
```
...
newUsername.addEventListener('input', function() {
    ...
    debounceTimer = setTimeout(function() {
        // Simulate API call
        const taken = ['admin', 'root', 'test'].includes(val.toLowerCase());

        if (taken) {
            status.textContent = 'Username is taken';
            status.className = 'error';
            newUsername.setCustomValidity('Username is taken');
        } else {
