...
### Case 1: Topological Data Analysis of Point Clouds

Given a point cloud from a 3D scanner, TDA computes persistent homology to find the shape:

```python
def vietoris_rips_complex(points, epsilon):
    """Build a Vietoris-Rips simplicial complex at scale epsilon."""
    n = len(points)
    ...
    for i in range(n):
        for j in range(i + 1, n):
            for k in range(j + 1, n):
                if ((i, j) in edges and (i, k) in edges and
                    (j, k) in edges):
...
By computing homology at varying $\varepsilon$ scales, we identify which features (connected components, loops, voids) persist — these are likely real topological features rather than noise.

### Case 2: Robot Configuration Space Obstacles

A 2-link planar robot arm must avoid an obstacle. The joint space is $S^1 \times S^1$ (a torus). Mapping obstacles to the configuration space reveals the forbidden region:

```python
def configuration_space_obstacle(theta1, theta2, link_lengths, obstacles):
    """Check if a robot configuration collides with obstacles."""
    ...
    arm_points = [(0, 0), (x1, y1), (x2, y2)]
    for obs in obstacles:
        for p in arm_points:
            if euclidean_dist(p, obs.center) < obs.radius:
                return True  # Collision
    return False
```

The free space (set of collision-free configurations) is an open subset of the torus. Path planning finds a continuous path in this open set from start to goal.

### Case 3: Network Topology Robustness

A computer network arranged in a star topology is connected but vulnerable: a single point of failure (the hub) disconnects the network. In contrast, a mesh topology is $k$-connected (multiple disjoint paths exist between any two nodes).

```python
def network_connectivity(graph):
    """Compute edge connectivity of a network graph (minimum cuts)."""
    # Edge connectivity is the minimum number of edges to remove
    # to disconnect the graph. This is a max-flow / min-cut problem.
    pass

def min_cut(graph, source, target):
    """Find minimum cut between source and target (Ford-Fulkerson)."""
    # The size of the min cut equals the max flow
    # High connectivity means the network is robust to failures
    pass
```

## Examples

### Example 1: Continuous vs. Discontinuous Function

```python
def continuous_f(x):
    return x * x  # Continuous everywhere

def discontinuous_f(x):
    if x >= 0:
        return 1
    return 0  # Discontinuous at x = 0
```

### Example 2: Homeomorphism Between (0,1) and R

```python
def homeomorphism_to_R(x):
    """Homeomorphism from (0,1) to R."""
    return math.tan(math.pi * x - math.pi / 2)

def homeomorphism_from_R(y):
    """Inverse homeomorphism from R to (0,1)."""
    return math.atan(y) / math.pi + 0.5
```

### Example 3: Euler Characteristic of Platonic Solids

```python
def euler_characteristic(name, V, E, F):
    chi = V - E + F
    print(f"{name}: V={V}, E={E}, F={F}, chi={chi}")
    return chi
...
### Example 4: Connected Components of a Mesh

```python
def mesh_connected_components(vertices, triangles):
    ...
    for tri in triangles:
        for i in range(3):
            for j in range(3):
                if i != j:
                    adj[tri[i]].add(tri[j])
    return connected_components_labeling_from_adj(adj)
```
...
def contraction_mapping(x):
    return 0.5 * x + 0.5  # Contraction with factor 0.5

fixed_point = fixed_point_iteration(contraction_mapping, 10.0)
# Converges to 1.0
```

### Example 6: Stereographic Projection

```python
def stereographic_projection(x, y, z):
    """Project S^2 minus north pole (0,0,1) onto R^2."""
    if abs(z - 1) < 1e-10:
        return None  # North pole projects to infinity
    return (x / (1 - z), y / (1 - z))

def stereographic_inverse(u, v):
    """Inverse stereographic projection from R^2 to S^2."""
    denom = 1 + u*u + v*v
    return (2*u / denom, 2*v / denom, (u*u + v*v - 1) / denom)
```

### Example 7: Topological Sort is Not Topology

Topological sorting in graph theory is unrelated to mathematical topology. The name is coincidental.

### Example 8: Quotient Space of Glued Edges

```python
def torus_from_square(u, v):
    """Map (u,v) in [0,1]x[0,1] to torus with opposite edges identified."""
    theta = 2 * math.pi * u
    phi = 2 * math.pi * v
    R = 2  # Major radius
    r = 1  # Minor radius
