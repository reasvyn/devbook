...
A developer writes code to filter out bad data:
...
The bug: modifying a list while iterating over it causes items to be skipped. When item at index 2 is removed, the next item shifts to index 2, but the loop moves to index 3.

Fix: create a new list instead of modifying in place:
...
**Case 2: Refactoring into composed functions.**

A junior developer writes a script that processes a log file:
...
This is hard to test and reuse. Refactored into functions:
...
def parse_error(line):
    if "ERROR" not in line:
        return None
    parts = line.split()
    return {"time": parts[0], "message": " ".join(parts[1:])}

def format_error(e):
    return f"{e['time']}: {e['message']}"

def process_log(path):
    lines = read_log(path)
    errors = [parse_error(line) for line in lines if parse_error(line)]
    errors.sort(key=lambda e: e["time"])
    for e in errors:
        print(format_error(e))
```

Now each piece can be tested individually and reused elsewhere.

**Case 3: Debugging step by step.**

A program that computes Fibonacci numbers produces wrong results:
...
Output: `fib(0) = 0, fib(1) = 1, fib(2) = 1, fib(3) = 2` — correct for the sequence starting at fib(0)=0, fib(1)=1.

But the user expected fib(1)=1, fib(2)=1, fib(3)=2, fib(4)=3 (1-indexed). The fix: `range(n)` should be `range(n - 1)`.

This demonstrates the importance of understanding the contract: does the function take 0-indexed or 1-indexed input?

## Examples

**Example 1: Expression evaluation tree.**

The expression `(3 + 4) * (5 - 2)` evaluates as:
...
Tracking state like this helps you understand exactly what happens at each step.

**Example 3: Accumulator loop vs reduce.**
