...
### Case 1: Accidentally Updating All Rows

You need to update the price of one book but forget the WHERE clause:

```sql
-- Intent: update only the book with id = 1
UPDATE books SET price = 0;
-- Effect: every book now has price = 0
```

**Prevention:**

- Always write the SELECT first to verify your WHERE condition
- Run UPDATE inside a transaction
- Enable safe-update mode: `SET SQL_SAFE_UPDATES = 1` (MySQL)

```sql
BEGIN;
SELECT * FROM books WHERE id = 1; -- verify
UPDATE books SET price = 0 WHERE id = 1;
SELECT * FROM books WHERE id = 1; -- verify again
COMMIT; -- or ROLLBACK if wrong
```

### Case 2: NULL Comparison Bug

You want to find books with no ISBN assigned:
...
`NULL = NULL` evaluates to UNKNOWN, and WHERE filters out UNKNOWN results. This is the most common NULL-related bug in SQL.

### Case 3: Choosing Wrong Data Type

Storing phone numbers as INTEGER:
...
INSERT INTO contacts (id, phone) VALUES (1, 0800123456);
-- INTEGER: stores as 800123456 (leading zero lost!)
```

**Correct approach**: store phone numbers as VARCHAR.
...
Phone numbers are identifiers with formatting, not arithmetic values.

## Examples

### Example 1: Complete CRUD Cycle
...
### Example 3: Python Parameterized Query with Error Handling
...
def get_books_by_author(author_name):
    with closing(sqlite3.connect("devbook.db")) as conn:
