...
### Case 1: Paginated Product List with Category Filter

Build a product listing endpoint that shows products and their category names, paginated. The naive approach fetches all products then looks up categories in application code.

```sql
-- Single query with JOIN -- one round trip, filtered at the source
...
Without the JOIN, a `N+1` problem emerges: one query for the product list plus one query per product to fetch its category. With JOIN, the database returns all needed data in a single result set. The `LIMIT`/`OFFSET` keeps the payload small.

### Case 2: Detecting Data Drift with FULL OUTER JOIN

A data migration copies orders from a legacy system to a new schema. Use `FULL OUTER JOIN` to compare records and find rows present in only one system.

```sql
-- Compare legacy.orders vs new.orders
SELECT COALESCE(l.order_id, n.order_id) AS order_id,
       CASE
           WHEN l.order_id IS NULL THEN 'only_in_new'
           WHEN n.order_id IS NULL THEN 'only_in_legacy'
           ELSE 'in_both'
       END AS status
FROM legacy.orders l
FULL OUTER JOIN new.orders n ON l.order_id = n.order_id
WHERE l.order_id IS NULL OR n.order_id IS NULL;
```

Rows where one side is `NULL` indicate mismatches. This pattern is invaluable for ETL validation, schema migrations, and reconciliation audits.

### Case 3: Self-Join for a Product Recommendation Engine

Given a table of product purchases, find products frequently bought together using a self-join on the `order_items` table.
...
The self-join pairs each product in an order with every other product in the same order. The `a.product_id < b.product_id` condition avoids duplicate pairs (A-B vs B-A). The result is a co-occurrence matrix used for "customers who bought X also bought Y" recommendations.

## Examples

### JOIN Type Decision Matrix

| Requirement | JOIN Type | Preserves left? | Preserves right? | Needs ON? |
|---|---|---|---|---|
| Only matching rows | `INNER JOIN` | No | No | Yes |
| All from primary table, matches from secondary | `LEFT JOIN` | Yes | No | Yes |
| All from secondary, matches from primary | `RIGHT JOIN` | No | Yes | Yes |
| All rows from both | `FULL OUTER JOIN` | Yes | Yes | Yes |
| Every combination | `CROSS JOIN` | N/A | N/A | No |

### Sample Result Sets by JOIN Type

Using `customers` (5 rows) and `orders` (6 rows, customer 4 has none):

| Customer | INNER JOIN | LEFT JOIN | RIGHT JOIN | FULL OUTER |
|---|---|---|---|---|
| Alice Wang | 2 orders | 2 orders | 2 orders | 2 orders |
| Bob Chen | 1 order | 1 order | 1 order | 1 order |
| Carol Davis | 2 orders | 2 orders | 2 orders | 2 orders |
| Danilo Souza | excluded | NULL row | excluded | NULL row |
| Eve Park | 1 order | 1 order | 1 order | 1 order |

`INNER JOIN` drops Danilo Souza entirely. `LEFT JOIN` keeps him with `NULL`s. `FULL OUTER JOIN` would also show orphan orders from the right side.

### Table Aliasing Quick Reference
...
-- Inline alias for calculated column
