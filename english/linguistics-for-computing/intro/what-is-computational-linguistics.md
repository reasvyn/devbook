...
### Case 1: The Stanford Sentiment Treebank

Socher et al. (2013) created a dataset of movie reviews with fine-grained sentiment annotations at every node in the parse tree.

**The insight:** Sentiment is compositional. "Not good" has different sentiment than "not bad." A model that only sees bag-of-words misses this.

**The approach:**
1. Parse all sentences into constituency trees
2. Annotate every node with sentiment (very negative to very positive)
3. Train a recursive neural network on the tree structure

**Results:** The recursive model outperformed bag-of-words and sequence models because it captured compositional sentiment.

**Relevance:** This case shows how linguistic structure (syntax trees) improves NLP performance — a core CL insight.

### Case 2: Word Embeddings and Analogy

Mikolov et al. (2013) showed that word2vec embeddings capture analogy relationships:
...
**The insight:** Distributional similarity (words in similar contexts have similar meanings) can capture relational structure.

**The math:**
- vec("king") - vec("man") + vec("woman") ≈ vec("queen")
- The offset represents a relation (male → female, capital → country)

**Relevance:** This showed that simple vector arithmetic on word embeddings captures rich semantic relationships — a foundational result for neural CL.

### Case 3: BERT and Contextual Representations

Devlin et al. (2018) introduced BERT (Bidirectional Encoder Representations from Transformers).

**The insight:** Previous word embeddings (word2vec, GloVe) gave the same representation for "bank" regardless of context ("river bank" vs "savings bank"). Context matters.

**The approach:**
1. Mask 15% of words in a sentence
2. Train the model to predict the masked words (bidirectional context)
3. Fine-tune on specific tasks (classification, QA, NER)

**Results:** BERT achieved state-of-the-art on 11 NLP tasks simultaneously.

**Relevance:** BERT showed that pre-training on large text corpora with a linguistically motivated objective (context prediction) produces representations that work for many downstream tasks.

### Case 4: Machine Translation and the Attention Mechanism

Bahdanau et al. (2014) introduced attention for neural machine translation.

**The problem:** Encoder-decoder models compressed the entire source sentence into a fixed-length vector. Long sentences suffered.

**The insight:** Different target words depend on different source words. The model should focus on relevant source positions when generating each target word.

**The approach:**
1. Encode the source sentence as a sequence of vectors (one per position)
2. For each target position, compute attention weights over source positions
3. The weighted sum of source vectors informs target word generation

**Results:** Attention significantly improved translation of long sentences.

**Relevance:** Attention became the foundation for Transformer architectures (Vaswani et al., 2017), which are now the dominant architecture in CL and beyond.

## Examples

### Example 1: From regex to FSA
...
```
States: {start, local, at, domain, dot, tld, accept}
start --[a-zA-Z0-9._%+-]--> local
local --[a-zA-Z0-9._%+-]--> local
local --[@]--> at
at --[a-zA-Z0-9.-]--> domain
...
The FSA perspective reveals: this regex accepts infinitely many valid emails (the loops) but cannot validate email deliverability (requires world knowledge).
...
### Example 4: Coreference resolution

Input: "Alice gave Bob the book. She said he had been wanting it."
...
text = "The new update is amazing! The UI is much cleaner."
doc = nlp(text)

print(doc._.blob.polarity)   # 0.5 (positive)
print(doc._.blob.subjectivity)  # 0.6 (fairly subjective)
print(doc._.blob.sentiment_assessments)
# [('amazing', 0.7, 0.5, None),
#  ('cleaner', 0.3, 0.5, None)]
```
