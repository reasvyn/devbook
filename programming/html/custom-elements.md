...
A custom `<custom-select>` that mimics `<select>` with form participation:
...
class CustomSelect extends HTMLElement {
    ...
    selectOption(value, label) {
        ...
        this.shadowRoot.querySelector('.trigger').textContent = label;
        this.shadowRoot.querySelector('.listbox').hidden = true;
    ...
    render() {
        const options = Array.from(this.children)
            .map(child =>
                `<li role="option" data-value="${child.value}">${child.textContent}</li>`
            ).join('');

        this.shadowRoot.innerHTML = `
            <style>
                :host { display: inline-block; position: relative; }
                .trigger { padding: 8px; border: 1px solid #ccc; cursor: pointer; }
                .listbox {
            ...
            <ul class="listbox" role="listbox" hidden>${options}</ul>
        `;

        this.shadowRoot.querySelector('.listbox')
...
class CountdownTimer extends HTMLElement {
    ...
    get seconds() { return Number(this.getAttribute('seconds')) || 0; }
    set seconds(val) { this.setAttribute('seconds', val); }

    start() {
        clearInterval(this._interval);
        this._interval = setInterval(() => {
            const remaining = this.seconds;
            if (remaining <= 0) {
                clearInterval(this._interval);
    ...
    attributeChangedCallback() {
        if (this.shadowRoot) {
            this.shadowRoot.querySelector('.display').textContent = this.seconds;
        }
    }

    connectedCallback() {
        this.shadowRoot.innerHTML = `
            <style>.display { font-size: 3em; text-align: center; }</style>
            <div class="display">${this.seconds}</div>
        `;
        this.start();
    }
}
