...
### Case 1: Proving a Tautology in Natural Deduction

Prove: $\vdash (p \to (q \to r)) \to ((p \to q) \to (p \to r))$
...
### Case 2: SAT Solver Encoding

A scheduling problem: tasks A, B, C must be scheduled in time slots 1, 2, 3.

Constraints:
- Each task gets exactly one slot.
- A and B cannot share a slot.
- C must be after A.

Let $A_i$ mean "A is in slot i" (for i = 1,2,3).

CNF encoding:
```
(A1 ∨ A2 ∨ A3)         // A in at least one slot
(¬A1 ∨ ¬A2)            // A not in multiple slots
(¬A1 ∨ ¬A3)
(¬A2 ∨ ¬A3)
(Same for B and C)
(¬A1 ∨ ¬B1)            // A and B not together
(¬A2 ∨ ¬B2)
(¬A3 ∨ ¬B3)
(¬A1 ∨ ¬C2 ∨ ¬C3)     // C must be after A (if A1, then not C1; etc.)
```

A SAT solver would find a satisfying assignment (e.g., A=1, B=2, C=3).

### Case 3: Resolution Proof
...
5. $\neg p \lor r$ (clause 2)
6. $\neg r$ (clause 3)
7. $q \lor r$ (resolve 4, 5 on p)
8. $q$ (resolve 7, 6 on r)
9. $\neg q$ (negated goal for refutation)
10. $\bot$ (resolve 8, 9)

Since the empty clause is derived from clauses + negated goal, the original goal follows.

## Examples

### Example 1: Natural Deduction for $p \land (q \land r) \vdash (p \land q) \land r$
...
### Example 2: Sequent Proof of $A \to B, B \to C \vdash A \to C$
...
\frac{
  ...
  \frac{A \to B, B \to C, A \vdash C}{A \to B, B \to C \vdash A \to C} (\to R)
} (Cut)
  \quad
$$

### Example 3: Hilbert System Proof of $p \to p$

1. $K$ instance: $p \to (q \to p)$
2. $S$ instance: $(p \to (q \to r)) \to ((p \to q) \to (p \to r))$
3. MP on 2,1 with $q = (q \to p)$: $(p \to ((q \to p) \to p)) \to ((p \to (q \to p)) \to (p \to p))$
4. $K$ instance: $p \to ((q \to p) \to p)$
5. MP on 3,4: $(p \to (q \to p)) \to (p \to p)$
6. $K$ instance: $p \to (q \to p)$
7. MP on 5,6: $p \to p$

The numbering follows the standard Hilbert proof of $p \to p$.

### Example 4: Prolog as Resolution
...
% :- parent(john, Z), parent(Z, ann).
% Z = mary (unification)
% :- parent(mary, ann).
% :- fail (mary is not parent of ann)
% backtrack...
% Z = ... 
% In this case, john is not grandparent of ann because the chain is
% john -> mary -> tom -> ann, which requires 3 steps.
```

### Example 5: SMT Solving with Z3

```python
from z3 import *

# Solve: x + y > 10 AND x - y < 3 AND x > 0 AND y > 0
x, y = Ints('x y')
solver = Solver()
solver.add(x + y > 10)
solver.add(x - y < 3)
