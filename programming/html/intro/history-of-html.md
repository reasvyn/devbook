...
**Case 1: The `<marquee>` element.**

Internet Explorer introduced the `<marquee>` element in IE 3.0 (1996). It scrolled text horizontally across the screen:

```html
<marquee behavior="scroll" direction="left">Welcome to my page!</marquee>
```

The element was universally mocked by designers and never part of any HTML standard. Yet it worked in IE, so developers used it. Other browsers were forced to implement it for compatibility. The `<marquee>` element is still supported in every major browser today — nearly 30 years later — despite never being standardized.

This case illustrates the power of backward compatibility: once something ships in a browser and developers depend on it, removing it becomes nearly impossible.

**Case 2: The `<blink>` element and the revenge of standards.**

Netscape introduced the `<blink>` element in Navigator 1.0 (1994). Text inside `<blink>` tags would flash on and off:

```html
<blink>This text flashes. Do not use this.</blink>
```

Users hated it. Designers overused it. The web development community eventually rejected `<blink>` as an anti-pattern. It was never standardized. Unlike `<marquee>`, the `<blink>` element was eventually removed from browsers (Firefox dropped support in 2013).

The difference? `<blink>` was universally despised, including by browser vendors themselves. When there is consensus that a feature should die, even backward compatibility can be overcome.

**Case 3: The `<font>` element and the lesson of separation.**

The `<font>` element was introduced by Netscape and later included in HTML 3.2 and 4.01:

```html
<font size="4" color="red" face="Arial">Styled text</font>
```

It mixed presentation with structure. When CSS emerged, the web community realized that styling should be separate from markup. The `<font>` element was deprecated in HTML 4.01 and removed from the specification.

But it still works in browsers. You can use `<font>` today and it will render. It is not valid HTML, but browsers will not reject it. This is the lenient parsing model in action: browsers prioritize rendering over validation.

## Examples

**Example 1: The first web page recreated.**

The first web page, published August 6, 1991, was a plain document with headings, paragraphs, and links. A reconstruction:
...
<body>
    ...
    <a href="#">subjects</a>, <a href="#">W3 servers</a>, etc.</p>
...
This tiny vocabulary — headings, paragraphs, links — was the entire web in 1991.

**Example 2: HTML 4.01 strict vs HTML5.**

An HTML 4.01 Strict document:
...
The differences: shorter doctype, simpler charset declaration, no `type` attributes on `<link>` and `<script>` (not needed in HTML5), and semantic elements replacing generic divs.

**Example 3: HTML5 feature detection.**

Developers can check whether a browser supports an HTML5 feature before using it:
...
if (document.createElement('canvas').getContext) {
    ...
    // Fall back to a static image or Flash
}
...
if (input.type === 'date') {
    ...
    // Browser fell back to text input
}
```

This pattern — feature detection rather than browser sniffing — became standard practice during the HTML5 era.
