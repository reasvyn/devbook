...
### Case 1: Ray Picking in 3D

A user clicks on the screen at pixel $(x, y)$. To determine which 3D object they clicked:
...
```python
def screen_to_ray(screen_x, screen_y, viewport, view_matrix, proj_matrix):
    # Convert to normalized device coordinates
    ndc_x = (2.0 * screen_x) / viewport[2] - 1.0
    ndc_y = 1.0 - (2.0 * screen_y) / viewport[3]
    ...
    near_world = inv_proj_view @ near
    far_world = inv_proj_view @ far
...
### Case 2: Path Planning with Splines

A robot must navigate through waypoints $\mathbf{w}_0, \mathbf{w}_1, \ldots, \mathbf{w}_n$ with smooth turns. Use a Catmull-Rom spline to generate a smooth path:

```python
def generate_path(waypoints, segments_per_segment=10):
    path = []
...
### Case 3: Plane-Fitting to 3D Point Cloud

Given a set of 3D points (from LiDAR or depth camera), find the best-fit plane:
...
def fit_plane(points):
    ...
    # Center the points
    centered = [(p[0] - centroid[0], p[1] - centroid[1], p[2] - centroid[2])
                for p in points]
    # Build 3x3 covariance matrix
    cov = np.zeros((3, 3))
    for p in centered:
        cov += np.outer(p, p)
    # Eigen decomposition: smallest eigenvector = plane normal
...
### Example 1: Line-Line Intersection in 2D

```python
def line_intersection_2d(p1, d1, p2, d2):
    """Intersection of two lines given point and direction."""
...
def circle_from_three_points(p1, p2, p3):
    """Compute center and radius of circle through three non-collinear points."""
    ax, ay = p1
    bx, by = p2
    cx, cy = p3
...
### Example 3: Quadratic Bezier for Vector Graphics
...
### Example 4: Parabolic Arc for Projectile

```python
def projectile_arc(start, velocity, gravity, time_steps=30):
    points = []
    for i in range(time_steps + 1):
        t = i * 0.1
        x = start[0] + velocity[0] * t
        y = start[1] + velocity[1] * t + 0.5 * gravity * t * t
        points.append((x, y))
    return points
```

### Example 5: Circle Parametrization

```python
def circle_points(center, radius, num_points=32):
    cx, cy = center
    points = []
...
```python
def helix(radius, height, turns, num_points=100):
    points = []
    for i in range(num_points + 1):
        t = i / num_points
