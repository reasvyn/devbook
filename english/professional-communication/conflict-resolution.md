...
### Case 1: Microservices vs. Monolith

Two senior engineers on a team of twelve disagreed about the architecture for a new billing system.

```text
// Position A (microservices)
"We should build separate services for invoicing, payments, and
reconciliation. This lets us scale independently and deploy faster."

// Position B (monolith)
"We should start with a monolith. Our team is small, the domain is tightly
coupled, and splitting prematurely will slow us down."

// SCRIP resolution
Share: Both wrote position statements.
Clarify: A agreed that the domain is tightly coupled. B agreed that if
scalability becomes a real need, a split would be expensive.

Resolve: Build as a modular monolith with clear module boundaries. If any
module needs independent scaling, extract it as a service.
Implement: Defined module interfaces. Added a "service extraction" section to
the architecture docs. Scheduled a review in 6 months.

Prevent: Added "modular monolith first" to the team's architecture principles.
```

### Case 2: The Linter War

A team of five disagreed about whether to adopt Prettier with default settings or customize the configuration.

```text
// Positions
Engineer 1: "Defaults are fine. Less configuration to maintain."
Engineer 2: "I want 120-character lines, not 80. It reduces nesting."
Engineer 3: "I want single quotes, not double quotes."
Engineer 4: "I don't care, but I don't want to argue about formatting."

// Resolution
The team agreed to use Prettier defaults, since research shows formatting
preferences are not correlated with code quality. The disagreement was about
taste, not correctness. Anyone who wanted to change defaults could present
data, but personal preference was not sufficient.

// Prevention
The decision was documented. A ticket was created to add Prettier to the CI
pipeline with default settings. Formatting debates are now closed by the
linter.
```

### Case 3: Ownership Boundary Conflict

Two teams shared a codebase. Team A owned the frontend. Team B owned the API. They disagreed about who should own the GraphQL layer that sat between them.

```text
// Team A position
"We should own GraphQL because it defines what data the frontend needs."

// Team B position
"We should own GraphQL because it sits closest to our API and we know the
data model best."

// Resolution
Shared ownership with clear boundaries:
- Team A defines the GraphQL schema (what data the frontend needs)
- Team B implements the resolvers (how data is fetched)
- Breaking changes to the schema require approval from both teams

// Prevention
Added a "shared ownership" section to the team's operating agreement.
Documented the boundary in the repository's CONTRIBUTING.md.
```

## Examples

### Example 1: Position Statement for Tooling Conflict

```text
Decision: Should we adopt Bazel as our build system?
Position: No, stick with Make + Docker.

Rationale:
1. Our 20 microservices do not need Bazel's monorepo scale.
2. Bazel has a 2-3 week learning curve for the team.
3. Our CI integrates well with Make.

Acknowledged: Bazel offers incremental builds and caching.
Would change my mind: Benchmarks showing 50%+ CI time reduction.
Path forward: Benchmark Make; revisit Bazel if build exceeds 10 minutes.
```
### Example 2: Facilitating a Mediation Session

```text
Mediator: "Thank you both. We have 30 minutes. Alex, share your position on
API versioning for 5 minutes. Maria, please listen without interruption.

Alex: [shares]

Mediator: "Maria, your turn for clarifying questions."

Maria: "You said URL versioning is simpler for clients. Can you give an example?"

Alex: [answers]

Mediator: "Maria, please share your position for 5 minutes."

Maria: [shares]

Mediator: "Alex, your clarifying questions."

Alex: "You mentioned header-based versioning keeps URLs clean. Is that a
frontend team concern or a documentation concern?"

Maria: [answers]

Mediator: "You both agree versioning is needed and that APIs should be easy
to use. You disagree on the mechanism. Let's spend the remaining 10 minutes
finding options that address both concerns."
```

### Example 3: Decision Record After Conflict

```text
Decision: Use PostgreSQL for the billing service.
Date: 2025-03-15
Participants: Alex Chen (Tech Lead), Maria Santos (Senior Engineer),
              Rajat Patel (Engineering Manager)

Context: Billing service stores invoices, line items, payments, refunds.
50K invoices/month. Highly relational data.

Options: PostgreSQL (relational, team has experience), DynamoDB (horizontal
scaling, no team experience, requires denormalization).

Rationale: Data model is relational. Team has strong PostgreSQL expertise.
Volume is well within PostgreSQL range. Migration later is possible.

Trade-offs accepted: Vertical scaling limit at very high volume.
Dissenting opinion: Maria preferred DynamoDB. Revisit if volume exceeds
5M invoices/month.

Action: Maria to set up query monitoring by 2025-03-22.
Review date: 2025-09-15 (6 months).
```

### Example 4: Preventing Conflict with Process

```text
Before process: Each new service had a 2-hour debate about testing frameworks.
The decision differed each time because different people were involved.

After process: The team documented a testing standard (pytest for Python,
testify for Go) in /docs/standards/testing.md. Anyone proposing a different
framework must propose changing the standard with team-wide discussion.

Result: The debate was closed by documented process.
```

### Example 5: Recognizing Personal Conflict

```text
// The signal
Engineer A and Engineer B disagree on every PR, even trivial ones.
Engineer A's comments on Engineer B's work are sharper than on others'.
Engineer B avoids working with Engineer A on shared modules.

// The root cause (revealed in 1:1 with manager)
Six months ago, Engineer A felt Engineer B dismissed their architectural
proposal in a team meeting without giving it a fair hearing. Engineer A has
not felt respected since.

// The resolution
The manager facilitated a conversation where Engineer A shared how the
dismissal felt. Engineer B acknowledged the impact and apologized. They
agreed to give each other's proposals a fair hearing going forward. The
technical disagreements that followed were productive, not personal.
```
