...
.article p + p {
    text-indent: 0;         /* Or 1.5em for traditional print style */
}
...
```css
.price {
    font-variant-numeric: tabular-nums;   /* Same width for all digits */
...
### Example 1: Truncation with ellipsis
...
### Example 2: Link styling with text-decoration
...
a {
    ...
    text-decoration-color: color-mix(in oklch, #0066cc 30%, transparent);
