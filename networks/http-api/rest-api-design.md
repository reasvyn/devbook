...
### Case 1: Designing a REST API for a Blog Platform
...
{
  "data": {
    "id":1,"title":"REST API Design","slug":"rest-api-design",
    "content":"...","status":"published",
    "author":{"id":42,"name":"Alice","avatar_url":"https://cdn.example.com/avatars/42.jpg"},
    "tags":[{"id":3,"name":"API Design"}],"comment_count":12,
...
```bash
GET /api/v1/posts?status=published&q=rest+api&tag=api-design&sort=created_at:desc
```

**Key decisions.** Comments nested under posts (parent-child). Tags as separate resource with many-to-many linking. Pagination on all collections. Filtering via query params.

### Case 2: API without Proper Error Responses

An API returns inconsistent error formats:

```
POST /signup -> 400, {"message":"Validation failed"}
POST /login  -> 401, {"error":"invalid_grant"}
GET  /profile -> 500, "Internal Server Error"
```

**Problem.** The mobile app cannot tell users what went wrong. The signup error does not say which field is invalid. The login error needs custom mapping. The 500 returns a raw string. The app shows a generic error. Support tickets flood in.

**Fix.** Consistent error format everywhere:

```json
{"error":{"code":"VALIDATION_ERROR","message":"3 validation errors",
  "details":[{"field":"email","issue":"must be a valid email"},{"field":"password","issue":"must be at least 8 chars"}]}}
```

**Result.** The mobile app shows field-level errors inline. Support tickets drop by 80%.

### Case 3: Breaking Change Without Versioning

An API with no versioning renames `email` to `email_address` in user responses. Deployed on a Friday to an app with 1 million users.

**Result.** The mobile app reads `response["email"]` and gets `undefined`. User screens break. Login fails. The team reverts over the weekend.

**Fix.** Version from day one:

```
/api/v1/users  -> {"email":"alice@example.com"}
/api/v2/users  -> {"email_address":"alice@example.com"}
```

**Deprecation workflow.** Add v2, set deprecation headers on v1, announce, 6-month migration window, then 410 Gone.

**Before any breaking change.** Can I add instead of rename? Keep old field alongside the new? Created a new version? Communicated the timeline?

## Examples

### Example 1: Full CRUD for Products Resource
...
class PCreate(BaseModel): name:str; price:float; description:Optional[str]=None; category:str; in_stock:bool=True

@app.get("/api/v1/products")
def list_products(page:int=1,per_page:int=20,category:Optional[str]=None,fields:Optional[str]=None):
    items=[p for p in db.values() if not category or p["category"]==category]
    t=len(items); tp=(t+per_page-1)//per_page; d=items[(page-1)*per_page:page*per_page]
    if fields: allowed={"id","name","price","category","in_stock"}; sel=set(fields.split(","))&allowed
    if sel: d=[{k:p[k] for k in sel} for p in d]
    b="/api/v1/products"
    return {"data":d,"pagination":{"page":page,"per_page":per_page,"total":t,"total_pages":tp,
        "next":f"{b}?page={page+1}&per_page={per_page}" if page<tp else None,
        "prev":f"{b}?page={page-1}&per_page={per_page}" if page>1 else None}}

@app.post("/api/v1/products",status_code=201)
def create(p:PCreate):
    global nid; now=datetime.utcnow().isoformat()+"Z"
    item={"id":nid,**p.dict(),"description":p.description or "","created_at":now,"updated_at":now}
    db[nid]=item; nid+=1; return {"data":item}
...
def replace(pid:int,p:PCreate):
    if pid not in db: raise HTTPException(404)
    now=datetime.utcnow().isoformat()+"Z"
    db[pid]={"id":pid,**p.dict(),"description":p.description or "","created_at":db[pid]["created_at"],"updated_at":now}
    return {"data":db[pid]}

@app.patch("/api/v1/products/{pid}")
def patch(pid:int,u:dict):
    if pid not in db: raise HTTPException(404)
    db[pid]={**db[pid],**u,"updated_at":datetime.utcnow().isoformat()+"Z"}
    return {"data":db[pid]}

@app.delete("/api/v1/products/{pid}",status_code=204)
def delete(pid:int):
    if pid not in db: raise HTTPException(404)
    del db[pid]
```
...
```json
{"data":[{"id":1,"name":"Mouse","price":29.99,"category":"electronics"},{"id":2,"name":"Keyboard","price":89.99,"category":"electronics"}],
 "pagination":{"page":1,"per_page":2,"total":147,"total_pages":74,"next":"/api/v1/products?page=2&per_page=2","prev":null}}
```

### Example 3: Error Response

```json
{"error":{"code":"VALIDATION_ERROR","message":"2 validation errors",
  "details":[{"field":"email","issue":"must be valid","value":"invalid"},{"field":"password","issue":"must be 8+ chars","value":"abc"}]}}
```
...
curl -X POST https://api.example.com/v1/orders \
  ...
  -d '{"items":[{"product_id":101,"quantity":2}],"shipping_address":{"street":"123 Main St","city":"Springfield","zip":"12345"}}'
```

```json
{"data":{"id":789,"status":"confirmed",
  "items":[{"product_id":101,"name":"Mouse","quantity":2,"unit_price":29.99,"subtotal":59.98}],
  "total":59.98,"shipping_address":{"street":"123 Main St","city":"Springfield","zip":"12345"},"created_at":"2024-06-01T12:00:00Z"}}
```
