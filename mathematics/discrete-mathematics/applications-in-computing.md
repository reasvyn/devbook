...
### Case 1: Designing a Rate Limiter

**Problem:** Design a rate limiter that allows at most N requests per minute from each user. The rate limiter must use minimal memory and be efficient.

**Analysis using multiple discrete math concepts:**

1. **Set theory:** Track active users as a set. A new user not in the set gets a new bucket.
2. **Functions:** Map user_id -> request count using a hash function.
3. **Combinatorics:** The maximum number of tracked users determines memory requirements.
4. **Boolean algebra:** Check if count < limit and increment atomically.

```python
class SlidingWindowRateLimiter:
    def __init__(self, max_requests, window_seconds, max_users=100000):
        self.max_requests = max_requests
        self.window = window_seconds
        self.buckets = {}  # user_id -> [(timestamp, count), ...]
        self.max_users = max_users

    def allow_request(self, user_id):
        now = time.time()
        if user_id not in self.buckets:
            # New user: may need to evict if at capacity
            if len(self.buckets) >= self.max_users:
                # Evict oldest — set difference
                oldest = min(self.buckets.keys(),
                           key=lambda u: self.buckets[u][-1][0])
                del self.buckets[oldest]
            self.buckets[user_id] = [(now, 1)]
            return True

        # Remove expired entries (window-based)
        cutoff = now - self.window
        self.buckets[user_id] = [
            (t, c) for t, c in self.buckets[user_id] if t > cutoff
        ]

        # Count requests in window
        total = sum(c for _, c in self.buckets[user_id])
        if total >= self.max_requests:
            return False

        self.buckets[user_id].append((now, 1))
        return True
```

The rate limiter combines:
- Set theory (track users, eviction policy)
- Functions (user_id -> request history)
- Combinatorics (determine capacity)
- Boolean logic (allow/deny decision)

### Case 2: Web Crawler Architecture

**Problem:** Design a web crawler that downloads pages without overwhelming servers and avoids infinite cycles.

**Analysis:**

1. **Graph theory:** The web is a directed graph. Crawling is BFS from seed URLs, bounded by depth.
2. **Set theory:** Visited URLs are stored in a set (usually a Bloom filter for memory efficiency).
3. **Partial orders:** URLs from the same domain should be visited with delays defined by robots.txt.
4. **Combinatorics:** The number of URLs to crawl grows exponentially with depth.
...
class WebCrawler:
    def __init__(self, politeness_delay=1):
        ...
        self.last_visit = {}  # Per-domain last visit time
    ...
    def crawl(self, max_pages=1000):
        ...
        while self.queue and pages_crawled < max_pages:
            ...
            if now - last < self.politeness:
                self.queue.append(url)  # re-queue
                continue

            if url in self.visited:  # Set membership test
                continue

            # Download (in real code: fetch page)
            new_urls = extract_urls(url)

            # Add new URLs (set union)
