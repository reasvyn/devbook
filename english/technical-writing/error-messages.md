...
### Case 1: Generic Error Page

```text
Before:
500 Internal Server Error. Something went wrong. The administrators have been notified.

After:
We cannot process your request right now.
Our servers are experiencing a temporary issue. Your data is safe.
Please wait a few minutes and try again.
If the problem persists, contact support with reference ID: ERR-7F3A2D.
```

### Case 2: Inline Validation for Signup

```text
Before:
Name: Invalid entry. Email: Invalid entry. Password: Invalid entry.
...
After:
{
  "error": {
    ...
    "details": [{ "field": "email", "reason": "format", "value": "not-an-email" }],
    "docs_url": "https://example.com/docs/errors#INVALID_INPUT"
  }
}
```

### Case 4: Toast Notifications

```text
Before: "Activity detected."

After:
Success: "Synced 15 new contacts." (auto-dismiss 4s)
Error:   "Sync failed. Check your connection." (persistent with retry)
Warning: "2 sync conflicts. Review in Sync Manager." (persistent with link)
Info:    "Last synced 3 hours ago." (auto-dismiss 5s)
```

### Case 5: Multi-Step Form Errors

```text
Before: "Please correct the errors below."

After:
"3 errors found:
- Step 1: Credit card number is incomplete.
- Step 2: Billing ZIP code is required.
- Step 3: You did not agree to the terms.
Review each step and submit again."
```

## Examples

### Example 1: Three-Part Error

```text
Bad (one part):       "Error: Access denied."
Good (three parts):   "Access denied. Your account does not have permission. Contact your administrator."
```

### Example 2: User-Friendly vs. Technical

```text
Technical:   "ORA-00001: unique constraint (USERS.EMAIL_UNIQUE) violated"
User:        "This email address is already registered. Try signing in instead."
```

### Example 3: Actionable vs. Dead End

```text
Dead end:    "Upload failed."
Actionable:  "Upload failed. The file is 25 MB. Max is 10 MB. Compress and try again."
```

### Example 4: Specific vs. Vague

```text
Vague:       "Operation could not be completed."
Specific:    "Could not delete 'Q4 Report' because it has shared links. Remove them first."
```

### Example 5: Log vs. User-Facing

```text
Log:         "ERROR [auth] JWT expired at 2026-06-29T10:00:00Z."
User:        "Your session has expired. Sign in again to continue."
```
...
```text
Confusing:   "Are you sure?" [OK] [Cancel]
Clear:       "Delete 3 selected files?" [Cancel] [Delete files]
```

### Example 8: Validation Timing

```text
After submit:  "Error: Invalid form submission."
During typing: "Password must include at least one number."
```

### Example 9: Internationalization Template

```text
Template: "{count, plural, one {# item} other {# items}} deleted successfully."
English: "1 item deleted successfully." / "3 items deleted successfully."
```

### Example 10: Button Label Pairs

```text
Generic:    [OK] [Cancel]
Specific:   [Keep editing] [Discard changes]
Generic:    [Yes] [No]
Specific:   [Pause subscription] [Cancel]
```
