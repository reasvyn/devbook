...
### Case 1: Application Entry Point with Route-Based Splitting
...
### Case 2: Shared Component Library via Barrel
...
### Case 3: Utility Library with Tree-Shakeable Exports
...
```javascript
// app.js — only debounce and memoize are included in the bundle
import { debounce, memoize } from "./utils/index.js";
```

## Examples

### Example 1: Conditional Polyfill Loading
...
### Example 2: Lazy I18n Loading

```javascript
async function loadLocale(locale) {
    try {
        const messages = await import(`./locales/${locale}.js`);
        i18n.init(messages.default);
    } catch {
        const fallback = await import("./locales/en.js");
        i18n.init(fallback.default);
    }
}
```

### Example 3: Dynamic Import with Error Boundary
...
### Example 4: Barrel with Selective Re-exports

```javascript
// components/index.js
export { Button } from "./Button.js";

// Only export the types we want consumers to use
export { Input, InputGroup } from "./Input.js";

// Internal only — not exported
// export { InputValidator } from "./InputValidator.js";
```

### Example 5: Development/Production Import Switch

```javascript
let logger;

if (import.meta.env.DEV) {
    const { createDevLogger } = await import("./dev-logger.js");
    logger = createDevLogger();
} else {
    const { createProdLogger } = await import("./prod-logger.js");
