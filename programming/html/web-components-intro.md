...
An accessible accordion with expand/collapse, keyboard handling, and slot-based content:
...
class AccordionSection extends HTMLElement {
    ...
    attributeChangedCallback(name) {
        if (name === 'open') {
            ...
            this.shadowRoot.querySelector('button')
                .setAttribute('aria-expanded', this.open);
        }
    }
...
### Form-Associated Date Input

A custom date input that participates in the parent form via `ElementInternals`:
...
```html
<form>
    <date-input name="birthdate" min="1900-01-01"
                max="2026-12-31" value="2000-01-01"></date-input>
</form>
```
    ...
    connectedCallback() {
        this.shadowRoot.innerHTML = `
            <style>
                ...
                :host(:focus) .tooltip-text { visibility: visible; }
            </style>
            <span class="tooltip">
                <slot></slot>
...
```html
<simple-tooltip text="Click to save">
    <button>Save</button>
</simple-tooltip>
```
