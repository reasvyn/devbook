...
### Case 1: Proving a Program Correct with Dependent Types

Consider a safe array access function. In most languages, out-of-bounds access causes a runtime error. With dependent types, we can prove safety at compile time:

```lean
def safe_nth {α : Type} {n : Nat} (xs : Vec α n) (i : Fin n) : α :=
  match xs with
  | Vec.nil => 
    -- Impossible: Fin 0 has no elements
    match i with
  | Vec.cons x xs' =>
    match i with
    | Fin.mk 0 _ => x
    | Fin.mk (j+1) h => safe_nth xs' ⟨j, by
      -- j < n-1 because j+1 < n
      have : j+1 < n := h
      omega
    ⟩
...
The type `Fin n` is the type of natural numbers less than $n$. The function can never fail because the type of `i` guarantees it is a valid index.

### Case 2: Curry-Howard in Practice

We implement a propositional logic prover using the Curry-Howard correspondence:
...
def Not (A : Prop) : Prop := ¬ A

-- Theorem: A → B → A
theorem k {A B : Prop} (a : A) (b : B) : A := a
-- The proof is literally a function: (a, b) ↦ a

-- Theorem: (A → B → C) → (A → B) → A → C
theorem s {A B C : Prop} (f : A → B → C) (g : A → B) (a : A) : C :=
  f a (g a)
-- The proof is function application
```

### Case 3: Verifying a Sorting Algorithm

```lean
-- Proving that insertion sort returns a sorted permutation
...
def is_permutation (xs ys : List Nat) : Prop :=
  Multiset.ofList xs = Multiset.ofList ys

theorem insertion_sort_sorts (xs : List Nat) : sorted (insertion_sort xs) := ...
theorem insertion_sort_permutes (xs : List Nat) : is_permutation xs (insertion_sort xs) := ...
```

## Examples

### Example 1: System F vs. Simply Typed Lambda Calculus

Simply typed lambda calculus can express:

$$\lambda x : \text{Int}. x : \text{Int} \to \text{Int}$$

But cannot express:

$$\lambda x. x : \forall \alpha. \alpha \to \alpha$$

System F (polymorphic lambda calculus) adds universal quantification over types:
...
### Example 2: Excluded Middle in Classical vs. Intuitionistic

```lean
-- Classical logic: excluded middle is an axiom
axiom em {p : Prop} : p ∨ ¬p

-- In intuitionistic logic (Lean's default), this is not provable
-- But we can prove it for decidable propositions
theorem em_for_nat (n : Nat) : n = 0 ∨ n ≠ 0 := by
  cases n
  · left; rfl
  · right; intro h; injection h
```

### Example 3: Dependent Pattern Matching in Agda

```agda
-- Proving that n + 0 = n by dependent pattern matching
...
### Example 4: Inductive Families (Vector)

```lean
-- Vec is an inductive family: the type depends on the length
inductive Vec (α : Type) : Nat → Type where
  | nil  : Vec α 0
  | cons : α → Vec α n → Vec α (n+1)

-- Map preserves length
def map {α β : Type} {n : Nat} (f : α → β) (xs : Vec α n) : Vec β n :=
  match xs with
  | Vec.nil => Vec.nil
  | Vec.cons x xs' => Vec.cons (f x) (map f xs')

-- Type checker verifies: map preserves n
```

### Example 5: Proving Equality of Types

In proof assistants, we can prove non-trivial type equalities:

```lean
-- The type of booleans is isomorphic to the type of 2-element fin sets
theorem bool_equiv_fin2 : Bool ≃ Fin 2 :=
  { toFun := λ
    | true  => ⟨0, by decide⟩
    | false => ⟨1, by decide⟩
    invFun := λ
