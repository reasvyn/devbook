...
### Case 1: Designing a Quantizer for Sensor Data

A temperature sensor outputs values in $[0, 50]^\circ C$. Design a 3-bit quantizer:

```python
a = 50
M = 8
R = np.log2(M)
D_uniform = uniform_quantization_distortion(a / 2, M)  # centered at 25

# Data is not uniform -- concentrated around 22 (room temperature)
np.random.seed(42)
sensor_data = np.random.normal(25, 8, 10000)
sensor_data = np.clip(sensor_data, 0, 50)

# Lloyd-Max quantizer
levels = lloyd_max(sensor_data, M)
distances = np.abs(sensor_data[:, np.newaxis] - levels[np.newaxis, :])
indices = np.argmin(distances, axis=1)
reconstructed = levels[indices]
D_optimal = np.mean((sensor_data - reconstructed)**2)

print(f"Uniform quantizer: D={D_uniform:.4f}")
print(f"Optimal quantizer: D={D_optimal:.4f}")
print(f"Improvement: {10*np.log10(D_uniform/D_optimal):.2f} dB")
```

### Case 2: Rate-Distortion Curve for a Gaussian Source

```python
import matplotlib.pyplot as plt

sigma2 = 1.0
rates = np.linspace(0, 5, 100)
distortions = sigma2 * 2**(-2 * rates)

# Find R needed to achieve D = 0.1 * sigma^2
D_target = 0.1 * sigma2
R_needed = 0.5 * np.log2(sigma2 / D_target)
print(f"Target D={D_target:.2f}: R = {R_needed:.2f} bits/sample")
```

### Case 3: JPEG Compression Analysis

```python
# Simulate: effect of JPEG quality factor on rate and distortion
def jpeg_quality_analysis(Q):
    block = np.random.randn(8, 8) * 50 + 128
    rec, mse, nz = jpeg_block_encode(block, Q)
    return nz / 64, mse

for Q in [10, 30, 50, 70, 90, 100]:
    sparsity, mse = jpeg_quality_analysis(Q)
    print(f"Q={Q}: sparsity={1-sparsity:.2%}, MSE={mse:.2f}")
```

## Examples

### Example 1: Rate-Distortion for a Binary Source

```python
def binary_rd(p, D):
    h_p = -p * np.log2(p) - (1-p) * np.log2(1-p)
    h_D = -D * np.log2(D) - (1-D) * np.log2(1-D) if D > 0 else 0
    if D >= min(p, 1-p):
        return 0
    return h_p - h_D
...
for p in [0.1, 0.25, 0.5]:
    ...
    for D in [0, 0.05, 0.1, 0.15, 0.2]:
        print(f"  D={D:.2f}: R={binary_rd(p, D):.4f}")
```

### Example 2: Scalar vs. Vector Quantization Comparison
...
# Scalar quantization (1D, 4 bits per dimension)
def sq_encode(data, bits_per_dim):
    levels_per_dim = 2**bits_per_dim
    encoded = np.zeros_like(data)
    for dim in range(data.shape[1]):
        sorted_uniq = np.sort(data[:, dim])
        quantiles = np.linspace(0, len(data), levels_per_dim + 1).astype(int)[1:-1]
        edges = sorted_uniq[quantiles]
        encoded[:, dim] = np.digitize(data[:, dim], edges)
    return encoded

# 4-bit VQ (16 codewords)
kmeans = KMeans(n_clusters=16, random_state=42, n_init=10)
vq_indices = kmeans.fit_predict(data)
vq_rec = kmeans.cluster_centers_[vq_indices]
vq_mse = np.mean((data - vq_rec)**2)

# 4+4 = 8 bits total for SQ, 4 bits total for VQ
print(f"SQ (8 bits total): MSE comparison not directly fair")
print(f"VQ (4 bits total, 2D): MSE = {vq_mse:.4f}")
```

### Example 3: Distortion-Rate Function Plot

```python
rates = np.linspace(0, 5, 50)
gauss_D = sigma2 * 2**(-2 * rates)
laplace_variance = 1.0
laplace_D = laplace_variance * 2**(-2 * rates) * 2  # Approximate

print("Rate | Gauss MSE | Laplace MSE (approx)")
print("-----|-----------|-------------------")
for i in range(0, 50, 10):
    print(f"{rates[i]:.1f}  | {gauss_D[i]:.6f} | {laplace_D[i]:.6f}")
```

### Example 4: Transform Coding Gain

```python
def coding_gain(eigenvalues):
    n = len(eigenvalues)
    arithmetic = np.mean(eigenvalues)
    geometric = np.exp(np.mean(np.log(eigenvalues)))
    return arithmetic / geometric

eigvals = np.array([4.0, 1.0, 0.5, 0.1])
gain = coding_gain(eigvals)
print(f"Coding gain: {gain:.2f} ({10*np.log10(gain):.2f} dB)")

# A gain of 6 dB means the transform saves 1 bit per sample
print("This transform saves approximately", np.log2(gain), "bits per sample")
```
