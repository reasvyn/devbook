...
### Case 1: Implementing a TLS 1.3 Client

A developer needs to implement a minimal TLS 1.3 client for an embedded system. They start with RFC 8446.

Pass 1:

- RFC 8446 status: Proposed Standard
- Updated by: RFC 8446 has errata — check them
- Abstract: TLS 1.3 provides a secure transport connection
- Introduction: Key changes from TLS 1.2 include reduced handshake latency and removed insecure features

Pass 2:

- Section 2: Terminology — understand "endpoint", "client", "server", "handshake", "record"
- Section 4: Handshake protocol — the core protocol flow
  - Client sends ClientHello
  - Server responds with ServerHello, EncryptedExtensions, Certificate, CertificateVerify, Finished
  - Client sends Finished
- Section 5: Record protocol — framing and encryption
- Section 6: Key schedule — how keys are derived
- Security considerations (Section 9): downgrade protection, replay, etc.

Implementation:

The developer builds a minimal handshake flow, focusing on the required ciphersuite (TLS_AES_128_GCM_SHA256) and the basic record layer. They defer optional features like 0-RTT and PSK.

Errata check reveals a clarification about the usage of certain extensions in Section 4.2. The developer incorporates this clarification.

Result: A compliant TLS 1.3 client for the embedded target.

### Case 2: Debugging an HTTP/2 Interoperability Issue

A server implementation sends HTTP/2 frames that work with some clients but not others.

The developer traces the issue to a client that sends RST_STREAM frames for certain requests. Reading RFC 9113:

- Section 2.2: Stream states and transitions
- Section 5.1: How RST_STREAM is processed
- The developer discovers that their server is not properly handling RST_STREAM in all stream states

The fix: Implement correct state transitions on RST_STREAM as specified in Section 2.2 (Figure 2 — stream state machine).

Without reading the RFC, the developer might have added a workaround for the specific client instead of fixing the root cause.

### Case 3: Confusing MUST and SHOULD in URI Parsing

A developer reads a blog post about URI parsing that says "you SHOULD decode percent-encoded characters."

Reading RFC 3986 directly:

- Section 2.1: Percent-Encoding — defines percent-encoding
- Section 3.3: Path component — specifies that percent-encoded characters in the path SHOULD be decoded for comparison
- However, Section 7.5: Scheme-specific processing — says that the scheme MAY define different rules

The developer realizes:

- Decoding percent-encoded characters is a SHOULD, not a MUST
- The scheme may override this (e.g., some schemes require preserving encoded characters)
- The blog post incorrectly stated it as a MUST

By reading the RFC directly, the developer avoids a bug where percent-decoding would break URIs for schemes that require preservation.

## Examples

### Example 1: Normative Language in RFC 7230

```text
RFC 7230 Section 5.3:

"A user agent MUST NOT send a Connection header field that contains
the header-field-name 'close' [...] A server MUST NOT send a
Connection header field that contains the header-field-name 'close'
in a response that has a 1xx (Informational) status code."

Interpretation:
- MUST NOT is an absolute prohibition
- Both clients and servers are prohibited from sending certain Connection headers
- Violation means non-compliance
- The specific case (close in 1xx responses) is explicitly prohibited
```

### Example 2: RFC Structure Navigation

Finding the WebSocket close frame format in RFC 6455:

1. Table of Contents -> Section 5: Data Framing
2. Section 5.2: Base Framing Protocol -> Figure 2 shows the frame format
3. The close frame is opcode 8 (0x8)
4. Section 5.5.1: Close -> details the close frame format and semantics
5. Section 7.4: Status Codes -> lists defined close status codes

Navigation pattern: use the table of contents and section numbering to drill down to the specific detail.

### Example 3: Checking RFC Status

A project depends on a protocol defined in RFC 5000 (fictional). Before implementing:

1. Check status on the RFC Editor site
2. Is it Proposed Standard, Internet Standard, or Historic?
3. Is it obsoleted by a newer RFC?
4. Are there errata?

If RFC 5000 is Historic and obsoleted by RFC 6000, implement RFC 6000. Check errata for RFC 6000 as well.

### Example 4: The RFC 2119 Key Words in Practice

```text
RFC 9110 Section 3.1:

"An origin server MUST NOT send a Transfer-Encoding header field in
any response with a status code of 1xx (Informational) or 204 (No
Content)."

"If a client receives a response that contains a Content-Length
header field whose value is a decimal number that is not equal to the
decimal number in the Content-Length header field received from the
server, the client SHOULD treat the response as invalid."

"A user agent MAY choose to automatically change the request method
from POST to GET for the purposes of link traversal."
```

An implementation must:
- Enforce the MUST NOT (reject Transfer-Encoding in 1xx/204 responses)
- Ideally enforce the SHOULD (validate matching Content-Length)
- May optionally implement method transformation (MAY)
