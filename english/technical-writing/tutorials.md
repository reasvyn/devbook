...
### Case 1: Rewriting a Tutorial That Did Not Work

A tutorial for a web framework had 47 steps, did not specify versions, and never worked on first try.

```text
Problems identified:
- 47 steps was too long (readers abandoned before finishing)
- No version pinning (code broke when dependencies updated)
- No verification points (reader did not know if they were on track)
- No error recovery (reader was stuck with no help)
- Mixed tutorial modes (tried to teach the framework, the language,
  and the deployment platform all at once)

Rewrite:
- Split into 3 tutorials (beginner, intermediate, advanced)
- Each tutorial: 8-12 steps, 20-30 minutes
- Added version pinning and a starter template
- Verification points after every 2-3 steps
- Common error recovery section
- Clear prerequisites with verification commands

Result:
- Tutorial completion rate: 12% -> 68%
- Support requests for tutorial issues: 45/month -> 6/month
- Positive feedback mentions: 2% -> 45%
```

### Case 2: A Tutorial That Teaches Independence

```text
Design: The tutorial teaches how to build a CLI tool in Python.

Structure:
1. Scaffolding (5 steps, high guidance)
   - Set up the project structure
   - Implement the main command
   - Run the tool

2. Extension (3 steps, medium guidance)
   - "Now add a --verbose flag. Hint: use argparse."
   - "Implement a --output flag that writes to a file."
   - "Add error handling for missing files."

3. Challenge (1 step, no guidance)
   - "Add a --format flag that supports JSON and YAML output.
     The implementation is up to you."

Result: Readers report feeling confident enough to build their own CLI tools
after completing the tutorial. The challenge step is the most commonly cited
reason for this confidence gain.
```

## Examples

### Example 1: Tutorial Step with Verification

```text
Step 3: Create your first API endpoint

Add the following code to server.js, right after the app setup:
...
This code creates a new endpoint at /hello that returns a JSON response.

Verify it works:

1. Start the server: `node server.js`
2. In a new terminal, run: `curl http://localhost:3000/hello`
3. You should see: `{"message":"Hello, World!"}`
```

### Example 2: Prerequisites with Verification
