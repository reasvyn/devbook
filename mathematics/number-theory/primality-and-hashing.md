...
### Case 1: Generating Primes for RSA Key Generation

```python
def generate_rsa_prime(bits: int) -> int:
    """Generate a prime suitable for RSA key generation."""
    while True:
        # Generate random odd number
        n = random.getrandbits(bits)
        n |= (1 << bits - 1) | 1  # Ensure bit length and odd
        
        # Quick trial division filter
        if not trial_division_with_primes(n, primes_up_to_100k):
            continue
        
        # Miller-Rabin with 20 rounds (error < 2^-40)
...
bits = 512
start = time.time()
p = generate_rsa_prime(bits)
elapsed = time.time() - start
print(f"Generated {bits}-bit prime in {elapsed:.2f}s")
print(f"Prime: {hex(p)[:30]}...")
print(f"Bit length: {p.bit_length()}")
```

### Case 2: Password Storage with Salting and Hashing
...
def hash_password(password: str, salt: bytes = None) -> tuple[bytes, bytes]:
    ...
    # In production, use bcrypt/scrypt/argon2 with work factor
...
stored_key, salt = hash_password(password)

# Login attempt
attempt = "correct-horse-battery-staple"
print(f"Correct password: {verify_password(attempt, stored_key, salt)}")

# Wrong password
attempt = "wrong-password"
print(f"Wrong password: {verify_password(attempt, stored_key, salt)}")

# Same password produces different hash (different salt)
stored_key2, _ = hash_password(password)
print(f"Same password, different hash: {stored_key != stored_key2}")
```

## Examples

### Example 1: Comparison of Prime Generation Times

```python
import time

def benchmark_prime_generation():
    """Compare time to generate primes of different bit lengths."""
    for bits in [32, 64, 128, 256, 512]:
        start = time.time()
        for _ in range(10):
            p = generate_rsa_prime(bits)
        avg = (time.time() - start) / 10
        print(f"{bits:>4}-bit prime: avg {avg:.3f}s to generate")

benchmark_prime_generation()
```

### Example 2: Finding Carmichael Numbers

```python
def count_carmichael(limit: int) -> int:
    """Count Carmichael numbers up to limit."""
    count = 0
    for n in range(3, limit + 1, 2):
        if is_prime_trial(n):
            continue
        # Korselt criterion check
        factors = {}
        temp = n
        p = 2
        while p * p <= temp:
...
print(f"Carmichael numbers < 10^4: {count_carmichael(10000)}")
print(f"Carmichael numbers < 10^5: {count_carmichael(100000)}")
```

### Example 3: Hash Collision Search

```python
def search_collision_8bit(max_samples: int = 10000):
    """Search for collision in truncated 8-bit hash."""
    seen = {}
    for i in range(max_samples):
        data = f"message_{i}".encode()
        h = hashlib.sha256(data).digest()[0]  # first byte (8 bits)
        if h in seen:
            print(f"Collision: '{seen[h]}' and '{data.decode()}' both hash to {h:02x}")
            return
        seen[h] = data
    print("No collision found (unlikely for 8-bit hash)")

search_collision_8bit()
```

### Example 4: HMAC-Based Message Authentication

```python
key = bytes.fromhex("deadbeef0123456789abcdef01234567")
message = b"Transfer $1000 to account 12345"
mac = hmac.new(key, message, hashlib.sha256).hexdigest()
print(f"HMAC: {mac}")

tampered = b"Transfer $10000 to account 12345"
tampered_mac = hmac.new(key, tampered, hashlib.sha256).hexdigest()
print(f"Tampered verify: {hmac.compare_digest(mac, tampered_mac)}")
```
