...
**Case 1: The breadcrumb with no list.**

A developer writes breadcrumbs as plain text links:
...
Screen readers announce the links but have no way to know they form a sequence. Using an ordered list:
...
Screen readers announce "breadcrumb" navigation, list with 3 items, and the current page.

**Case 2: The tabnapping vulnerability.**

A developer adds external links with `target="_blank"`:

```html
<a href="https://external-site.com" target="_blank">Visit Partner Site</a>
```

The external site opens in a new tab. Its JavaScript can access `window.opener.location` and redirect the original page to a phishing site. The fix:

```html
<a href="https://external-site.com" target="_blank" rel="noopener noreferrer">Visit Partner Site</a>
```

**Case 3: The inaccessible icon link.**

A social media footer uses icon-only links:

```html
<a href="https://twitter.com/company"><img src="twitter.svg"></a>
<a href="https://facebook.com/company"><img src="facebook.svg"></a>
```

Screen readers cannot determine where these links go. The fix uses `aria-label`:

```html
<a href="https://twitter.com/company" aria-label="Follow us on Twitter"><img src="twitter.svg" alt=""></a>
<a href="https://facebook.com/company" aria-label="Follow us on Facebook"><img src="facebook.svg" alt=""></a>
```

The `alt=""` on the image marks it as decorative, and `aria-label` provides the accessible name on the link.

## Examples

**Example 1: Complete site navigation.**
...
<p>
    ...
    <a href="https://github.com/example" target="_blank" rel="noopener">
        View our GitHub
        <span class="sr-only">(opens in new tab)</span>
    </a>
</p>
...
<p>
    <!-- Download link -->
    <a href="/files/brochure.pdf" download>Download our brochure (PDF)</a>
</p>
...
**Example 3: Email link with parameters.**

```html
<a href="mailto:feedback@example.com?subject=Website%20Feedback&body=Hello%2C%0A%0AI%20have%20a%20suggestion%3A">
    Send Feedback
</a>
```

The body parameter uses `%0A%0A` for line breaks and `%3A` for colon — standard URL encoding.

**Example 4: Pagination with prev/next.**

```html
<nav aria-label="Search results pages">
    <ul>
        <li><a href="/search?page=1" rel="prev" aria-label="Previous page">&laquo; Previous</a></li>
        <li><a href="/search?page=1" aria-label="Page 1">1</a></li>
        <li aria-current="page"><span aria-label="Page 2, current">2</span></li>
        <li><a href="/search?page=3" aria-label="Page 3">3</a></li>
        <li><a href="/search?page=3" rel="next" aria-label="Next page">Next &raquo;</a></li>
    </ul>
</nav>
```
