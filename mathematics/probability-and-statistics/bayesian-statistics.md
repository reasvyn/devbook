...
### Case 1: Bayesian Spam Filter

A spam filter uses Naive Bayes with Bayesian parameter estimation. The prior for each word's probability in spam/ham incorporates Laplace smoothing (Beta(1, 1) prior).

```python
import numpy as np

# Training data: word counts
# Prior: Beta(1, 1) — Laplace smoothing
alpha_prior, beta_prior = 1, 1

# Counts from training
spam_emails, ham_emails = 300, 700
spam_word_count = 2000
ham_word_count = 5000

# Word "free" appears in 150 spam and 20 ham
free_in_spam = 150
free_in_ham = 20

# Posterior for P(free | spam)
p_free_spam = stats.beta(alpha_prior + free_in_spam,
                          beta_prior + spam_word_count - free_in_spam)
p_free_ham = stats.beta(alpha_prior + free_in_ham,
                         beta_prior + ham_word_count - free_in_ham)

# Estimate (posterior mean)
print(f"P('free' | spam) = {p_free_spam.mean():.4f}")
print(f"P('free' | ham)  = {p_free_ham.mean():.4f}")

# Prediction for new email
p_spam = 0.30  # prior probability of spam
# A new email contains "free"
likelihood_spam = p_free_spam.mean()
likelihood_ham = p_free_ham.mean()
post_spam = (likelihood_spam * p_spam) / (likelihood_spam * p_spam + likelihood_ham * (1-p_spam))
print(f"P(spam | 'free') = {post_spam:.4f}")
```

### Case 2: Recommendation System with Thompson Sampling

Thompson sampling balances exploration and exploitation using Bayesian posteriors. For each user, maintain a Beta posterior over the click-through rate of each item.
...
# Simulate Thompson Sampling for 5 items
n_items = 5
true_ctr = np.array([0.05, 0.10, 0.15, 0.08, 0.12])  # true rates
n_trials = 10000

# Initialize Beta(1, 1) priors
alpha = np.ones(n_items)
beta = np.ones(n_items)
chosen_counts = np.zeros(n_items)
reward_counts = np.zeros(n_items)

for t in range(n_trials):
    # Sample from posteriors
    sampled_rates = np.random.beta(alpha, beta)
    # Choose item with highest sampled rate
    chosen = np.argmax(sampled_rates)
    chosen_counts[chosen] += 1
    # Simulate Bernoulli outcome
    reward = np.random.binomial(1, true_ctr[chosen])
    reward_counts[chosen] += reward
    # Update posterior
    alpha[chosen] += reward
    beta[chosen] += 1 - reward

print(f"Item | True CTR | Chosen | Rewards | Posterior mean")
for i in range(n_items):
    post = stats.beta(alpha[i], beta[i])
    print(f"  {i}  |  {true_ctr[i]:.2f}   | {int(chosen_counts[i]):5d} | {int(reward_counts[i]):4d} |    {post.mean():.3f}")
```

### Case 3: Bayesian Hierarchical Model for School Test Scores

Hierarchical models share information across groups, improving estimates for groups with few observations (partial pooling).
...
# Simulate 10 schools
n_schools = 10
true_theta = np.random.normal(50, 10, n_schools)
n_students = np.random.poisson(30, n_schools)

# Generate student scores
scores = [np.random.normal(true_theta[j], 15, n_students[j])
          for j in range(n_schools)]

# No-pooling estimate (each school independently)
no_pool = np.array([np.mean(s) for s in scores])
no_pool_se = np.array([np.std(s, ddof=1) / np.sqrt(len(s)) for s in scores])

# Complete pooling (all schools combined)
complete_pool = np.mean(np.concatenate(scores))

# Approximate Bayesian partial pooling using James-Stein
# (a simple empirical Bayes approach)
grand_mean = np.mean(no_pool)
variance_between = np.var(no_pool) - np.mean(no_pool_se**2)
variance_between = max(variance_between, 0)  # avoid negative
shrinkage = 1 - (no_pool_se**2) / (no_pool_se**2 + variance_between)
partial_pool = grand_mean + shrinkage * (no_pool - grand_mean)

print("School | True | No-pool | Partial-pool | n")
for j in range(n_schools):
    print(f"  {j:2d}    | {true_theta[j]:5.1f} | {no_pool[j]:7.1f} | "
          f"{partial_pool[j]:12.1f} | {n_students[j]:3d}")
```

The partial-pooling estimates for schools with few students are "shrunk" toward the grand mean, reducing their variance at the cost of slight bias — improving overall MSE.

### Case 4: Bayesian Decision Theory for Model Selection

Choosing between deploying model A or model B. The loss function: deploying an inferior model costs 1 unit; doing nothing costs 0.2 units.

```python
import numpy as np
from scipy import stats

# Posterior difference from A/B test
np.random.seed(42)
diff_samples = np.random.normal(0.02, 0.015, 100000)

# Probability B is better
prob_b_better = np.mean(diff_samples > 0)
print(f"P(B better) = {prob_b_better:.4f}")

# Expected loss of deploying B
loss_deploy_B = -np.mean(diff_samples[diff_samples < 0]) if np.any(diff_samples < 0) else 0
# Expected loss of deploying A
loss_deploy_A = np.mean(diff_samples[diff_samples > 0]) if np.any(diff_samples > 0) else 0

# Decision rule: choose the action with minimum expected loss
print(f"Expected loss of deploying B: {loss_deploy_B:.6f}")
print(f"Expected loss of deploying A: {loss_deploy_A:.6f}")
```

## Examples

### Example 1: Beta-Binomial Shrinkage

With a Beta(10, 10) prior (mean 0.5, variance 0.012), observing 5 successes in 10 trials gives a posterior that shrinks the observed proportion toward 0.5.
...
obs_prop = 5/10
prior_mean = 10/(10+10)
post = stats.beta(10+5, 10+5)
post_mean = post.mean()

print(f"Observed proportion: {obs_prop:.3f}")
print(f"Prior mean:          {prior_mean:.3f}")
print(f"Posterior mean:      {post_mean:.3f}")
print(f"Shrinkage:           {(1 - (post_mean-0.5)/(obs_prop-0.5)):.1%}")
```

Shrinkage is stronger when the prior is more informative (larger $\alpha + \beta$) and the sample size is smaller.

### Example 2: Credible Interval vs Confidence Interval

For a binomial proportion with 8 successes in 20 trials:

**Frequentist 95% CI (Wilson):** [0.212, 0.579] — "If we repeat the experiment many times, 95% of intervals will contain the true value."

**Bayesian 95% CI (Beta(1,1) prior):** [0.226, 0.592] — "There is a 95% probability the true value is in this interval."
...
freq_ci = (center - half_width, center + half_width)

# Bayesian equal-tailed interval
post = stats.beta(1+k, 1+n-k)
bayes_ci = post.ppf([0.025, 0.975])

print(f"Frequentist 95% CI (Wilson): [{freq_ci[0]:.3f}, {freq_ci[1]:.3f}]")
print(f"Bayesian 95% CI:             [{bayes_ci[0]:.3f}, {bayes_ci[1]:.3f}]")
```

### Example 3: Prior Influence Diminishes with Data

As sample size grows, the prior becomes less influential. With 1,000 observations, a Beta(1,1) and Beta(100,100) prior give nearly identical posteriors.
...
for a, b in priors:
    ...
    print(f"Beta({a:3d}, {b:3d}) prior: posterior mean={post.mean():.4f}, " +
          f"95% CI=[{post.ppf(0.025):.4f}, {post.ppf(0.975):.4f}]")
```

### Example 4: Bayesian Linear Regression

In Bayesian linear regression, the parameters $\beta$ have a posterior distribution that captures both the best fit and the uncertainty.

$$y \sim \text{Normal}(X\beta, \sigma^2 I)$$
$$\beta \sim \text{Normal}(0, \tau^2 I)$$

The posterior mean is ridge regression (L2 regularization):

$$\beta_{\text{post}} = (X^T X + \frac{\sigma^2}{\tau^2} I)^{-1} X^T y.$$
...
beta_true = np.array([2, -1, 0, 0.5, 0])
y = X @ beta_true + np.random.normal(0, 1, n)

# Bayesian linear regression (simplified: known sigma)
sigma2 = 1.0
tau2 = 10.0  # prior variance

# Posterior mean (ridge)
XtX = X.T @ X
beta_post = np.linalg.inv(XtX + sigma2/tau2 * np.eye(p)) @ X.T @ y
beta_var = sigma2 * np.linalg.inv(XtX + sigma2/tau2 * np.eye(p))

print("True | Estimated | 95% CI")
for j in range(p):
    sd = np.sqrt(beta_var[j, j])
    ci = (beta_post[j] - 1.96*sd, beta_post[j] + 1.96*sd)
    print(f"{beta_true[j]:4.1f} | {beta_post[j]:8.3f} | [{ci[0]:.2f}, {ci[1]:.2f}]")
```

### Example 5: Bayesian Model Averaging

Instead of choosing one model, Bayesian model averaging weights predictions by posterior model probabilities:

$$P(y \mid \text{data}) = \sum_{k=1}^K P(y \mid \text{model}_k, \text{data}) P(\text{model}_k \mid \text{data}).$$

This accounts for model uncertainty and often improves predictive performance.

```python
# Two models with different posterior probabilities
models = [
    {"weight": 0.7, "mean": 0.10, "se": 0.02},  # simpler model
    {"weight": 0.3, "mean": 0.12, "se": 0.03},  # complex model
]

# Bayesian model averaging for prediction
ensemble_mean = sum(m["weight"] * m["mean"] for m in models)
# Variance: law of total variance
ensemble_var = sum(m["weight"] * (m["se"]**2 + (m["mean"] - ensemble_mean)**2) for m in models)
ensemble_se = np.sqrt(ensemble_var)
print(f"Ensemble mean: {ensemble_mean:.4f}")
print(f"Ensemble SE:   {ensemble_se:.4f}")
```
