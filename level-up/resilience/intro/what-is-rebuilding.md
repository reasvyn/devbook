...
- [When Everything Stops](../../foundations/when-everything-stops.md) — surviving the acute phase of collapse
- [Navigating Setbacks](../../foundations/navigating-setbacks.md) — frameworks for understanding why you fall back
...
```mermaid
graph TD
    A[Rebuilding Practice] --> B[Emotional Processing]
    A --> C[Support Building]
    A --> D[Pattern Interruption]
    A --> E[Pain as Teacher]
    B -->|creates honesty for| C
    C -->|provides safety for| B
    D -->|creates space for| B
    B -->|reveals values through| E
    E -->|motivates| D
    C -->|holds accountable for| D
...
sequenceDiagram
    ...
    participant G as Grief
    participant F as Fear
    participant A as Anger
    participant S as Shame
    participant R as Rebuilding

    D->>G: Awakening unlocks suppressed loss
    G->>F: Loss reveals uncertainty
    F->>A: Uncertainty provokes frustration
    A->>S: Frustration turns inward
    S->>R: Honesty dissolves isolation
    R->>R: Emotional clarity emerges
```
...
```mermaid
graph LR
    A[Clean Break Impulse] --> B{What changes?}
    B -->|Scenery| C[New city, new job, new partner]
    B -->|Patterns| D[Same avoidance, same coping]
    C --> E[Temporary relief]
    D --> F[Same void, new context]
    E --> F
    F --> G[Another clean break impulse]
