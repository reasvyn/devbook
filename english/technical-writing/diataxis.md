...
### Case Studies of Docs Restructured with Diataxis

**Case Study 1: A web framework's documentation**

```text
Before: 200+ pages alphabetically, tutorial mixed with reference.
After: /tutorials (3 paths), /guides (40 guides), /concepts, /reference.

Results: Tutorial completion 8% -> 52%. Support tickets down 35%.
Satisfaction 3.1/5 -> 4.6/5. Time to find reference reduced 60%.
```

**Case Study 2: A cloud platform's documentation**

```text
Before: Organized by product team, each section different style.
After: Each product section reorganized into Diataxis sub-sections.
Cross-product tutorials added. Reference standardized across products.

Results: Cross-product feature adoption increased 25%. Time-to-first-deploy
reduced from 2 days to 4 hours.
```

## Study Cases

### Case 1: Identifying Mixed Modes

A documentation page titled "Using the API" contains:

```text
Section 1: "What is an API?" — Explanation (belongs in concepts)
Section 2: "Setting up your first API call" — Tutorial (belongs in getting started)
Section 3: "Authenticating requests" — How-to guide (belongs in guides)
Section 4: "Endpoint reference" — Reference (belongs in reference)
```

The page serves four different reader needs poorly instead of one reader need well.

### Case 2: Applying Diataxis to a Changelog

A changelog can be structured using Diataxis principles:

```text
Changelog entry for v2.0.0:

Breaking changes (reference mode — list facts):
- Removed: /api/v1/users (use /api/v2/users)
- Changed: Response format for GET /users (see migration guide)

Migration guide (how-to mode — task oriented):
"To migrate from v1 to v2, follow these steps..."

Why we changed the API (explanation mode — understanding):
"The v1 API had inconsistent naming conventions. v2 standardizes..."

Tutorial: "Building with the v2 API" (tutorial mode — if applicable)
```

## Examples

### Example 1: Pure Tutorial vs. Pure How-To Guide

```text
Tutorial: "Build your first Express server"
"Welcome! In this tutorial, you will create a web server from scratch..."

Step 1: Install Node.js (explains what Node.js is)
Step 2: Create a project (explains what npm init does)
Step 3: Write the server (explains what each line does)
...

How-to guide: "Add authentication to your Express server"
"This guide assumes you already have an Express server running..."

Step 1: Install the auth middleware
Step 2: Configure the auth strategy
Step 3: Add the middleware to your routes
...
```
...
callback: function — The handler function(req, res, next)."

Explanation content (separate page):
"Express uses a routing system that maps HTTP methods and URL paths
to handler functions. When a request arrives, Express matches the URL
against registered routes in the order they were defined, and calls
the first matching handler. This design was chosen because it is simple,
flexible, and easy to reason about."
```

### Example 3: Categorized Navigation

```text
Documentation
├── Getting Started (tutorials)
│   ├── Your first project (15 min)
│   ├── Building a REST API (30 min)
│   └── Adding a frontend (45 min)
├── How-to Guides
...
### Example 4: Before and After Diataxis

```text
Before (mixed modes — one page):
"Building a Web App with Framework X"
1. Introduction to Framework X (explanation)
2. Installation (how-to)
3. Your first app (tutorial)
4. API Reference (reference)
5. Deployment guide (how-to)
(One page, 85 sections, took 3 hours to read)

After (Diataxis — multiple focused pages):
/tutorials/build-first-app.md (tutorial, 30 min)
/guides/installation.md (how-to, 5 min)
/guides/deployment.md (how-to, 10 min)
/concepts/architecture.md (explanation, 15 min)
/reference/api.md (reference, lookup only)
```
