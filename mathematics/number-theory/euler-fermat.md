...
### Case 1: RSA Key Generation from Scratch

Generate RSA keys with 1024-bit primes (simulated with small numbers):

```python
def rsa_full_keygen(bits: int = 32):
    """Generate RSA key pair with approximate bit length."""
    import random
    
    def generate_prime_candidate(bits: int) -> int:
        """Generate an odd integer of specified bit length."""
        n = random.getrandbits(bits)
        n |= (1 << bits - 1) | 1  # Ensure top bit and odd
        return n
    ...
    while not is_probable_prime(q) or q == p:
        q = generate_prime_candidate(bits // 2)
...
keys = rsa_full_keygen(32)
print(f"p={keys['p']}, q={keys['q']}, n={keys['n']}")
print(f"e={keys['e']}, d={keys['d']}")

# Test
m = 12345
c = pow(m, keys['e'], keys['n'])
m2 = pow(c, keys['d'], keys['n'])
print(f"Encrypt/Decrypt: {m} -> {c} -> {m2}")
```

### Case 2: Computing phi(n) for Large Numbers

```python
# phi of a product of two primes is (p-1)(q-1)
def phi_rsa(p: int, q: int) -> int:
    """phi for RSA modulus."""
    return (p - 1) * (q - 1)

# phi of a power of a prime
def phi_prime_power(p: int, k: int) -> int:
    """phi(p^k)."""
    return pow(p, k - 1) * (p - 1)

print(phi_rsa(61, 53))        # 3120
print(phi_prime_power(2, 5))  # 16 (numbers 1..32 coprime to 32)
```

### Case 3: Finding All Numbers with Given phi

```python
def numbers_with_phi(target: int, limit: int = 1000) -> list[int]:
    """Find all n <= limit such that phi(n) = target."""
    result = []
...
print(numbers_with_phi(4, 50))   # [5, 8, 10, 12]
print(numbers_with_phi(6, 50))   # [7, 9, 14, 18]
```

## Examples

### Example 1: Reducing Large Exponents

Compute 7^1000000 mod 13.

Since 13 is prime and gcd(7, 13) = 1:

phi(13) = 12, so 7^12 = 1 (mod 13).

1000000 mod 12 = 1000000 - 83333*12 = 1000000 - 999996 = 4

So 7^1000000 = 7^4 = 2401 = 2401 - 13*184 = 2401 - 2392 = 9 (mod 13).

```python
print(pow(7, 1000000, 13))  # 9
```

### Example 2: Fermat Primality Test in Action

```python
def fermat_witnesses(n: int) -> list[int]:
    """Find Fermat witnesses for compositeness of n."""
    witnesses = []
...
print(fermat_witnesses(15))    # Multiple witnesses
print(fermat_witnesses(561))   # No witnesses (Carmichael)
print(fermat_witnesses(97))    # No witnesses (actual prime)
```

### Example 3: Computing phi(n) for Large n

```python
# phi(1000000) using prime factorization
# 10^6 = 2^6 * 5^6
n = 10**6
result = n
for p in [2, 5]:
    result -= result // p
print(f"phi(10^6) = {result}")  # 400000

# Verification through Euler theorem
print(pow(3, result, n))  # Should be 1
```

### Example 4: Using Euler for Multiplicative Inverses

```python
# Find x such that 5x = 1 (mod 21)
# phi(21) = 12, so 5^{-1} = 5^{11} mod 21
a, n = 5, 21
inv = pow(a, phi(n) - 1, n)
print(f"{a} * {inv} mod {n} = {(a * inv) % n}")  # 5*17 = 85 = 1 mod 21
print(f"Verification: pow({a}, -1, {n}) = {pow(a, -1, n)}")
```

### Example 5: Solving a Congruence Using Euler

Solve 3x = 7 (mod 10):

x = 7 * 3^{-1} (mod 10)
phi(10) = 4, so 3^{-1} = 3^{3} = 27 = 7 (mod 10)
x = 7 * 7 = 49 = 9 (mod 10)
...
x = (b * inv) % n
print(f"x = {x}")  # 9
print(f"Verification: {a}*{x} mod {n} = {(a*x) % n} (expected {b})")
```
