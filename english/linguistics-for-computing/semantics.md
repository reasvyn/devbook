...
### Case 1: Proving Program Correctness with Hoare Logic

Program computing factorial:
...
**Invariant:** `fact = i! and i <= n`

Base case: i=0, fact=1, 0! = 1. Holds. Inductive step: assume invariant, execute body, fact becomes (i-1)! * i = i!. After loop: i=n, fact=n!.

### Case 2: API Renaming Based on Lexical Semantics

Before: `POST /documents/submit`, `GET /documents/query`, `PUT /documents/update/{id}`

Problems: Redundant synonyms, wrong HTTP verbs, workflow-specific names.

After: `POST /documents`, `GET /documents/{id}`, `PUT /documents/{id}`, `DELETE /documents/{id}`

### Case 3: Semantic Versioning Conflict Resolution

Project depends on lib-a >= 2.0 and lib-b (requires lib-a ^1.5). Resolution: lib-b is incompatible with lib-a 2.x. Wait for lib-b update or downgrade lib-a.

## Examples

### Example 1: Lexical Semantics in Programming

The `map` function differs semantically:
...
```javascript
// JavaScript — eager array
result = ["a", "b", "c"].map(x => x.toUpperCase());
```

```rust
// Rust — lazy, zero-cost iterator
let result: Vec<String> = vec!["a", "b", "c"].into_iter()
    .map(|x| x.to_uppercase()).collect();
```

### Example 2: Operational Semantics of a Counter

```
<x := 0; while x < 3 do x := x + 1, [x->0]>
  -> <while x < 3 do..., [x->0]>
  -> <x := x + 1; while..., [x->0]>
  -> <while..., [x->1]>
  ...
...
### Example 3: Type Derivation for `(3 + 4.0) * 2`
...
### Example 4: Compositional Semantics in NL

"Fido chases the cat":

```
Fido: fido
chases: lambda x. lambda y. chases(y, x)
cat: lambda x. cat(x)
the cat: (exists x. unique(x, cat) and cat(x))
Fido chases the cat: chases(fido, the_cat)
```
