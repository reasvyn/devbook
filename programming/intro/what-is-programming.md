...
**Case 1: A program that does the wrong thing.**

A junior developer writes a program to calculate the average of a list of numbers:
...
This works for the happy path. But what happens with an empty list?

```python
average([])  # ZeroDivisionError: division by zero
```

The program crashes because it tries to divide by zero. A robust version checks for edge cases:
...
This illustrates a fundamental truth of programming: you must think about what can go wrong, not just what should happen.

**Case 2: The fast solution is not always the best.**

A programmer needs to find duplicate values in a list of one million numbers. They write:

```python
def find_duplicates_slow(values):
    duplicates = []
...
This works but uses nested loops, resulting in O(n^2) complexity. For one million items, this would take hours.

A better approach uses a set for O(n) time:

```python
def find_duplicates_fast(values):
    seen = set()
    duplicates = set()
    for value in values:
        if value in seen:
...
This finishes in seconds instead of hours. Knowing how to choose the right algorithm is more important than knowing syntax.

## Examples

**Example 1: Binary representation.**

Everything in a computer is binary. The number 42 in decimal is 101010 in binary. A character like `A` is stored as the number 65 (ASCII), which is 1000001 in binary. Understanding binary helps you understand why integers overflow, why floating-point arithmetic is imprecise, and why bitwise operations are fast.

**Example 2: The first program.**

```python
print("Hello, World!")
```

This small program demonstrates several concepts: a function call (`print`), a string literal (`"Hello, World!"`), and program output. It is traditionally the first program written when learning a new language because it verifies that the development environment works.

**Example 3: Programming language comparison.**

The same program in different languages shows how syntax varies:
...
The logic is the same. Only the syntax differs. This is why learning programming concepts is more valuable than learning a single language.
