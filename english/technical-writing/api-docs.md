...
### Case 1: Rewriting a Sparse API Reference
...
Retrieves a single user's profile by their unique identifier.

Path Parameters:
| Parameter | Type   | Required | Description                        |
|-----------|--------|----------|------------------------------------|
| userId    | string | yes      | User ID (prefix: usr_)             |
...
| 404    | RESOURCE_NOT_FOUND   | No user exists with the given ID    |
...
### Case 2: Documenting a Complex Nested Resource
...
| Field              | Type   | Required | Description                     |
|--------------------|--------|----------|----------------------------------|
| customerId         | string | yes      | Existing customer user ID       |
| items              | array  | yes      | Array of line items (min: 1)    |
...
Response (201):
{
  ...
  "customerId": "usr_abc123",
  "items": [
    { "productId": "prod_xyz789", "quantity": 2, "unitPrice": 29.99 }
  ],
  "total": 59.98,
  "status": "pending",
...
### Case 3: Migrating from Code-First to Design-First

A team maintains an API where documentation is generated from code annotations. The spec has drifted from the implementation.

```text
Problem:
- Generated docs are 6 months behind the API
- Deprecated endpoints still appear in docs
- Error responses are not documented

Solution:
- Adopt OpenAPI 3.1 as the single source of truth
- Write the spec before implementing new endpoints
- Add contract tests that validate the spec against the implementation
- Serve the OpenAPI spec via Swagger UI

Results:
- Documentation accuracy improved from 60% to 98%
- Support tickets about API behavior dropped by 40%
```

## Examples

### Example 1: Minimal Endpoint Documentation Template
...
### Example 2: Error Response Documentation
...
### Example 3: Pagination Pattern Documentation
...
### Example 4: SDK Method Documentation Template
...
| Parameter      | Type   | Required | Description           |
|----------------|--------|----------|-----------------------|
| options.role   | string | no       | Filter by role        |
| options.page   | number | no       | Page number           |
| options.limit  | number | no       | Results per page      |
...
### Example 5: OpenAPI Component Reuse
