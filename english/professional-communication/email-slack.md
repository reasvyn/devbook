...
### Case 1: Avoiding a Channel Thread Disaster

A team had a 200-message thread in #general about a deployment issue. The thread contained debugging steps, partial logs, and side conversations. No one could find the final resolution.

The team adopted a policy: any thread longer than 10 messages must be summarized with the resolution. Now, the last message in every support thread is a clear summary of the problem, cause, and solution.

### Case 2: Cross-Time-Zone Coordination

A team had members in San Francisco, London, and Bangalore. The San Francisco team would ask questions at 5 PM PT, expecting an answer by the next morning. The Bangalore team would see the questions at 9 AM IST and respond, but the San Francisco team would not see the response until 2 AM PT.

The fix was to establish an async handoff protocol:

- SF team posts questions with full context before end of day PT
- Bangalore team answers during their morning, adding any follow-up questions
- London team picks up any remaining items during their afternoon
- SF team reviews all responses at the start of their day

### Case 3: The Cost of Vague Questions

An engineer posted in #backend: "Anyone know about the auth issue?" This resulted in 30 messages of back-and-forth, with people asking clarifying questions before anyone could actually help.

The team implemented a "questions that get answers" format in their onboarding documentation. After that, the same engineer posted: "I'm getting a 401 error from the auth service when calling POST /v1/orders with a valid token. Token is a JWT issued 5 minutes ago. Here is the token header and payload. I have tried refreshing the token and checking the auth service logs. Any ideas?" The first response came within 5 minutes and correctly identified the issue (the audience claim did not include the orders service).

## Examples

### Example 1: Good Slack Question

```text
Question about the deployment pipeline.

TICKET-1234: The deploy script is failing at the "migrate" step with:
  Error: migration V42 is already applied but hash does not match

I am deploying the `fix/timeout` branch to staging. The migration V42 was
applied last week. The error suggests the migration file has changed since
then, but I did not modify it.

Environment: staging, deployment tool: Flyway, DB: PostgreSQL 16

Any ideas? Happy to share the full log in a thread.
```

### Example 2: Good Email Subject and Body

```text
Subject: [TICKET-5678] Database Migration Window: Wednesday, July 5, 14:00 UTC

Hi team,

We will run the schema migration for the payments service on Wednesday,
July 5, starting at 14:00 UTC.

Impact:
- Payments API will be in read-only mode for approximately 15 minutes
- In-flight transactions will complete, new payment requests will be queued

No action required from most teams. The migration has been tested in staging
with production-sized data.

Rollback plan:
1. Re-run the migration script with the `--rollback` flag
2. Estimated rollback time: 5 minutes
3. Verified in staging on Monday

Please reply by Tuesday if you have concerns.

Thanks,
Alice
```

### Example 3: Good Follow-Up After No Response

```text
[Gentle ping] Following up on my message from yesterday about the auth
service timeout configuration. If you are busy, no rush — just want to
make sure this does not get lost. Happy to discuss async or in our next
1:1.
```

### Example 4: Channel Thread Summary

```text
[Thread summary] The debate about log format is resolved.

Decision: We will use structured JSON logging with the fields: timestamp,
level, service, message, request_id, and error. The format matches what
the observability team uses in other services.

Action items:
- @alice will update the logging library configuration
- @bob will update the runbook for log format changes

Full discussion in thread above.
```
